import { z } from "zod";
import { AcknowledgementStatus, PURCHASE_ORDER_SOURCES } from "./purchase-order";
import { Incoterm } from "./rfq.zod";
...
function refineDirectPurchaseOrderBody(
  data: z.infer<typeof CreateDirectPurchaseOrderBodySchema>,
  ctx: z.RefinementCtx,
): void {
...
export const CreateDirectPurchaseOrderPublicSchema = CreateDirectPurchaseOrderBodySchema.superRefine(
  refineDirectPurchaseOrderBody,
);
...
export const PurchaseOrderListQuerySchema = z
  .object({
    ...
    source: z.preprocess(
      ...
      z.enum(["RFQ", "DIRECT", "REORDER", "API", "LEGACY"]).optional(),
    ),
    status: z.preprocess(
      (v) => (v === "" || v == null ? undefined : v),
      z
...
export function composeDirectPoLineDescription(line: DirectPurchaseOrderLineInput): string {
  const parts: string[] = [line.productName];
  if (line.description?.trim()) parts.push(line.description.trim());
  if (line.specification?.trim()) parts.push(`Spec: ${line.specification.trim()}`);
  if (line.packaging?.trim()) parts.push(`Pack: ${line.packaging.trim()}`);
  if (line.unit?.trim()) parts.push(`Unit: ${line.unit.trim()}`);
