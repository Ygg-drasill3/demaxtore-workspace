...
export const MC_PROGRESS_STEPS: Array<{ key: ProcurementRequestStatus; label: string }> = [
...
const STATUS_ORDER: Record<ProcurementRequestStatus, number> = {
  DRAFT: 0,
  SUBMITTED: 1,
  UNDER_PROCUREMENT: 2,
...
export function mcStateToProcurementStatus(state: string): ProcurementRequestStatus {
  switch (state) {
    case "MC_DRAFT":
    case "MC_BUILDING":
      return "DRAFT";
...
export function procurementStatusIndex(status: ProcurementRequestStatus): number {
  return STATUS_ORDER[status] ?? 0;
}

export function isProgressStepComplete(
  step: ProcurementRequestStatus,
