...
export interface FreightCommercialInsight {
  ...
  revenueByForwarder: Array<{ forwarder: string; revenueUsd: number; marginUsd: number }>;
  revenueByCountry: Array<{ country: string; revenueUsd: number; marginUsd: number }>;
  revenueByMonth: RevenueTrend[];
  marginByRoute: RouteProfitability[];
  marginByForwarder: Array<{ forwarder: string; marginUsd: number; count: number }>;
  containerCountByRoute: Array<{ route: string; count: number }>;
  forwarderScorecards: ForwarderProfitability[];
  suggestedMarginPreview?: { lane: string; suggestedMarginUsd: number; policyName: string | null };
}
