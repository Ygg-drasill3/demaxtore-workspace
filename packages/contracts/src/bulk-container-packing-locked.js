...
export const BULK_CONTAINER_PACKING_CATALOG_VERSION = "1.0";
...
export const BULK_CONTAINER_LOCKED_PACKING_CODES = new Set(BULK_CONTAINER_LOCKED_PACKING_TYPES.map((p) => p.code));
/** Default packing code per BulkContainer category slug. */
export const BULK_CONTAINER_CATEGORY_DEFAULT_PACKING = {
...
export function lockedPackingCodesForCategory(slug) {
    return BULK_CONTAINER_LOCKED_PACKING_TYPES
        .filter((p) => p.categorySlug === slug)
        .map((p) => p.code);
}
export function isLockedBulkContainerPackingCode(code) {
    return BULK_CONTAINER_LOCKED_PACKING_CODES.has(code);
}
export function isBulkContainerPackingPrefix(code) {
    return code.startsWith("PT-BC-");
}
/** Throws-compatible guard result — returns error message or null if valid. */
export function validateBulkContainerPackingAssignment(categorySlug, packingCode) {
...
//# sourceMappingURL=bulk-container-packing-locked.js.map