// Sprint 6A — FreightIQ commercialization (margin + revenue ledger; not a marketplace)

export const FreightLedgerStatus = ["PENDING", "REALIZED", "CANCELLED"] as const;
...
export interface FreightCommercialMetrics {
  ...
  topRoutes: Array<{ route: string; count: number; marginUsd: number }>;
  topForwarders: Array<{ forwarder: string; count: number; marginUsd: number }>;
}
...
export interface FreightCommercialReport {
  ...
  revenueByMonth: Array<{ month: string; realizedUsd: number; pendingUsd: number }>;
  revenueByRoute: Array<{ route: string; realizedUsd: number; pendingUsd: number }>;
  revenueByForwarder: Array<{ forwarder: string; realizedUsd: number; pendingUsd: number }>;
  marginPerContainer: Array<{ shipmentId: string; orderRef: string; marginUsd: number }>;
  topShipments: Array<{ shipmentId: string; orderId: string; marginUsd: number; status: FreightLedgerStatus }>;
}
