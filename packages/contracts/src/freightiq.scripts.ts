...
export type FreightIqScriptPhase =
  | "not_eligible"
  | "empty"
  | "REQUESTED"
  | "QUOTING"
...
export function freightPhase(
  eligible: boolean,
  status: FreightStatus | null,
): FreightIqScriptPhase {
  if (!eligible && !status) return "not_eligible";
  if (!status) return "empty";
  return status as FreightIqScriptPhase;
}

export const FREIGHTIQ_SCRIPTS: Record<FreightIqScriptPhase, WorkspaceScript> = {
  not_eligible: {
    mood: "waiting",
...
export function freightiqScriptFor(phase: FreightIqScriptPhase, _role: WorkspaceScriptRole): WorkspaceScript {
  return FREIGHTIQ_SCRIPTS[phase];
}

export function freightMilestones(phase: FreightIqScriptPhase): Array<{ key: string; label: string; status: "done" | "current" | "pending" }> {
  const steps: Array<{ key: string; label: string; phases: FreightIqScriptPhase[] }> = [
