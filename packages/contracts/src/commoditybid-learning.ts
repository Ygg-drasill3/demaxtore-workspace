// Sprint 9A Revision — CommodityBid learning & guidance content (no FSM/runtime changes)
...
export const COMMODITYBID_STEP_LABELS: Record<CommodityBidOnboardingStep, string> = {
...
export const COMMODITYBID_STEP_DESCRIPTIONS: Record<CommodityBidOnboardingStep, string> = {
...
export const COMMODITYBID_LEARNING = {
  summary:
    "CommodityBid is a competitive reverse-auction engine for commodity procurement. Buyers schedule auctions; suppliers compete in real time; the lowest valid bid wins after the auction closes.",
  whatIs:
...
export function commodityBidWorkspaceGuidance(state: string): CommodityBidStageGuidance {
  const map: Partial<Record<CommodityBidState, CommodityBidStageGuidance>> = {
    BID_DRAFT: {
  ...
  return map[state as CommodityBidState] ?? {
...
export function commodityBidChecklistProgress(state: string): CommodityBidOnboardingStep[] {
  const completed: CommodityBidOnboardingStep[] = ["create_commoditybid"];
  const s = state as CommodityBidState;

  if (s !== "BID_DRAFT") completed.push("auction_scheduled");
  ...
  if (s === "ORDERS_SPAWNED") completed.push("order_created");
