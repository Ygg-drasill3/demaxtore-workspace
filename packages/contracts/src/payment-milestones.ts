// Faz 3 — Payment milestones and financial gates
export const PAYMENT_MILESTONE_KINDS = [
  "DEPOSIT_REQUIRED",
  "DEPOSIT_PAID",
  "PRODUCTION_PAYMENT_REQUIRED",
  "BALANCE_REQUIRED",
  "BALANCE_PAID",
  "PAYMENT_OVERDUE",
  "PAYMENT_DISPUTED",
  "PAYMENT_CONFIRMED",
] as const;
export type PaymentMilestoneKind = (typeof PAYMENT_MILESTONE_KINDS)[number];

export const PAYMENT_PLAN_STATUSES = ["DRAFT", "ACTIVE", "COMPLETED", "CANCELLED"] as const;
export type PaymentPlanStatus = (typeof PAYMENT_PLAN_STATUSES)[number];

export const PAYMENT_HOLD_REASONS = [
  "DEPOSIT_UNPAID",
  "BALANCE_UNPAID",
  "PAYMENT_DISPUTE",
  "MANUAL_HOLD",
] as const;
export type PaymentHoldReason = (typeof PAYMENT_HOLD_REASONS)[number];

/** Order actions gated by payment milestones when PAYMENT_GATES_ENABLED. */
export const PAYMENT_GATED_ORDER_ACTIONS: Partial<Record<string, PaymentMilestoneKind>> = {
  start_production: "DEPOSIT_PAID",
  book_shipment: "DEPOSIT_PAID",
  mark_delivered: "BALANCE_PAID",
};

export interface PaymentMilestoneDto {
  id: string;
  kind: PaymentMilestoneKind;
  status: "PENDING" | "SATISFIED" | "OVERDUE" | "WAIVED";
  amount: number | null;
  currency: string;
  dueAt: string | null;
  paidAt: string | null;
}

export interface PaymentPlanDto {
  orderId: string;
  status: PaymentPlanStatus;
  milestones: PaymentMilestoneDto[];
  holds: Array<{ reason: PaymentHoldReason; active: boolean }>;
  financialStatus: "CLEAR" | "HOLD" | "OVERDUE" | "DISPUTED";
}
