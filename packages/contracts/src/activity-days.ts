...
export function isUnknownActivityDays(days: number): boolean {
...
export function formatDaysSinceActivity(days: number): string {
  if (isUnknownActivityDays(days)) return "no recorded activity";
...
export function exceedsInactivityThreshold(days: number, thresholdDays: number): boolean {
  if (isUnknownActivityDays(days)) return false;
