import { SHIPMENT_TRANSITIONS, isShipmentTerminal, isShipmentActive, } from "./shipment.fsm";
const LABELS = {
...
export function computeShipmentNextActions(ctx) {
    if (isShipmentTerminal(ctx.state))
        return [];
    if (ctx.state === "EXCEPTION" && !ctx.hasOpenException)
        return [];
    const seen = new Set();
    const out = [];
    for (const t of SHIPMENT_TRANSITIONS) {
        if (t.action === "create_shipment")
            continue;
        if (t.from === "ANY_ACTIVE" && !isShipmentActive(ctx.state))
            continue;
        if (t.from !== "ANY_ACTIVE" && t.from !== "*" && t.from !== ctx.state)
            continue;
        if (!t.allowedRoles.includes(ctx.actorRole))
            continue;
        if (!satisfiesParticipant(t, ctx))
            continue;
        if (ctx.state === "EXCEPTION" && t.action !== "resolve_exception" && t.action !== "cancel_shipment" && t.action !== "upload_document")
            continue;
        if (ctx.hasOpenException && t.action !== "resolve_exception" && t.action !== "upload_document")
            continue;
...
//# sourceMappingURL=shipment.next-actions.js.map