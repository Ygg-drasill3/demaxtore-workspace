...
export interface NextActionContext {
  state: RfqState;
  actorRole: ActorRole;
  isOwner: boolean;                 // user is the workspace OWNER (creator)
  isCounterparty: boolean;          // user is a workspace COUNTERPARTY (supplier)
  isSelectedSupplier?: boolean;     // for proforma actions
  hasQuotationFromUser?: boolean;   // for supplier quotation actions in RFQ_OPEN
}
...
const LABELS: Record<RfqAction, { label: string; description: string; variant: NextAction["variant"]; confirm?: string }> = {
...
export function computeRfqNextActions(ctx: NextActionContext): NextAction[] {
...
function satisfiesSemanticConstraint(t: RfqTransition, ctx: NextActionContext): boolean {
...
function toNextAction(t: RfqTransition): NextAction {
