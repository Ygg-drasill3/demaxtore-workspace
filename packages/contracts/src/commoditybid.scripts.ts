...
export const COMMODITYBID_BUYER_SCRIPTS: Partial<Record<CommodityBidState, WorkspaceScript>> = {
  BID_DRAFT: {
    mood: "action",
...
export function commoditybidScriptFor(state: CommodityBidState, role: WorkspaceScriptRole): WorkspaceScript | undefined {
  if (role === "BUYER") return COMMODITYBID_BUYER_SCRIPTS[state];
  return undefined;
}
