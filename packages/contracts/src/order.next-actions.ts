...
const LABELS: Partial<Record<OrderAction, { label: string; description: string; variant: NextAction["variant"]; confirm?: string }>> = {
...
function satisfiesParticipant(t: OrderTransition, ctx: OrderNextActionContext): boolean {
...
function satisfiesSemantic(t: OrderTransition, ctx: OrderNextActionContext): boolean {
  if (t.action === "proceed_to_freight" && ctx.inspectionResult && ctx.inspectionResult !== "PASS") return false;
  if (t.preconditions?.includes("assertInspectionPass") && ctx.inspectionResult !== "PASS") return false;
...
function toNextAction(t: OrderTransition): NextAction {
...
export function computeOrderNextActions(ctx: OrderNextActionContext): NextAction[] {
  if (isOrderTerminal(ctx.state)) return [];

  const candidates = ORDER_TRANSITIONS.filter((t) => {
    if (t.allowedRoles.every((r) => r === "SYSTEM")) return false;
    if (!t.allowedRoles.includes(ctx.actorRole)) return false;
    if (t.from === "ANY_ACTIVE") return isOrderActive(ctx.state);
  ...
  for (const t of candidates) {
    ...
    if (!satisfiesSemantic(t, ctx)) continue;
    if (!LABELS[t.action]) continue;
