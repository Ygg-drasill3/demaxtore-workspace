...
import {
  COMMODITYBID_TRANSITIONS, isCommodityBidTerminal,
...
const LABELS: Partial<Record<CommodityBidAction, { label: string; description: string; variant: NextAction["variant"]; confirm?: string }>> = {
...
export function computeCommodityBidNextActions(ctx: CommodityBidNextActionContext): NextAction[] {
  if (isCommodityBidTerminal(ctx.state)) return [];
...
function satisfiesParticipantConstraint(t: CommodityBidTransition, ctx: CommodityBidNextActionContext): boolean {
...
function satisfiesSemanticConstraint(t: CommodityBidTransition, ctx: CommodityBidNextActionContext): boolean {
  switch (t.action) {
    case "submit_bid_lot":
...
function toNextAction(t: CommodityBidTransition): NextAction {
