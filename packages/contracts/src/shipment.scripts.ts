...
export const SHIPMENT_SCRIPTS: Partial<Record<ShipmentState, WorkspaceScript>> = {
  SHIPMENT_CREATED: {
    mood: "waiting",
...
export function shipmentMilestones(state: ShipmentState): Array<{ key: string; label: string; status: "done" | "current" | "pending" }> {
  const steps = [
  ...
  if (state === "EXCEPTION") {
    return steps.map((s, i) => ({ ...s, status: i <= 2 ? "done" : i === 2 ? "current" : "pending" as const }));
  }
...
export function shipmentProgressPercent(state: ShipmentState): number {
  const map: Partial<Record<ShipmentState, number>> = {
