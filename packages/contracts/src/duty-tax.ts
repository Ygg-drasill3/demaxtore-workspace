...
export const DUTY_TAX_V1_EVALUABLE_COMPONENTS = ["CUSTOMS_DUTY", "VAT"] as const;
...
export const DUTY_TAX_FREIGHT_ALLOCATION_METHODS = ["VALUE", "NONE"] as const;
...
export function dutyTaxCompletenessLabel(input: {
  ...
  if (input.missingCritical || input.status === "INCOMPLETE") return "LOW";
  if (input.provisional || input.status === "PROVISIONAL") return "PROVISIONAL";
  return "COMPLETE";
}
