...
export const ORDER_SCRIPTS: Partial<Record<OrderState, WorkspaceScript>> = {
  ORDER_CREATED: {
    mood: "waiting",
...
const SUPPLIER_ORDER_SCRIPTS: Partial<Record<OrderState, WorkspaceScript>> = {
  ORDER_CREATED: {
    mood: "action",
...
export function orderScriptFor(state: OrderState, role: WorkspaceScriptRole): WorkspaceScript | undefined {
  if (role === "SUPPLIER" && SUPPLIER_ORDER_SCRIPTS[state]) return SUPPLIER_ORDER_SCRIPTS[state];
  return ORDER_SCRIPTS[state];
}

export function orderMilestones(state: OrderState): Array<{ key: string; label: string; status: "done" | "current" | "pending" }> {
  const steps = [
