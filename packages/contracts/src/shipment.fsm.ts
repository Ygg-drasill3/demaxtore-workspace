...
export const SHIPMENT_ACTIVE_STATES: ShipmentState[] = [
  "SHIPMENT_CREATED",
  "BOOKING_PENDING",
  "BOOKING_CONFIRMED",
  "CONTAINER_ASSIGNED",
...
export const SHIPMENT_TRANSITIONS: ShipmentTransition[] = [
...
export function isShipmentActive(state: ShipmentState): boolean {
  return SHIPMENT_ACTIVE_STATES.includes(state);
}

export function findShipmentTransition(
  from: ShipmentState,
  action: ShipmentAction,
  actorRole?: ActorRole,
): ShipmentTransition | undefined {
  const matches = SHIPMENT_TRANSITIONS.filter((t) => {
    if (t.action !== action) return false;
    if (t.from === "ANY_ACTIVE") return isShipmentActive(from);
    if (t.from === "*") return action === "create_shipment";
    return t.from === from;
  });
  if (actorRole) {
...
export function resolveShipmentTargetState(from: ShipmentState, transition: ShipmentTransition): ShipmentState {
  if (transition.action === "upload_document" || transition.action === "ready_delivery") return from;
