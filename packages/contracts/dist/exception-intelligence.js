...
export function mapImpactToIssueCategory(impact) {
    switch (impact) {
        case "DELIVERY_RISK":
        case "SHIPMENT_DELAY":
...
export function evaluateCustomsBrokerMissing(missing) {
    if (!missing)
        return null;
    return {
...
export function evaluateCustomsOriginMissing(missing) {
    if (!missing)
        return null;
    return {
...
export function evaluateCustomsClassificationMissing(missing) {
    if (!missing)
        return null;
    return {
...
export function evaluateCustomsHold(onHold) {
    if (!onHold)
        return null;
    return {
...
export function evaluateCustomsPreparationAtRisk(input) {
    ...
    return {
        ...
        description: input.daysToArrival != null
...
export function evaluateEtaDeliveryRisk(input) {
    ...
    const deliveryAtRisk = !!eta && !!required && !Number.isNaN(eta.getTime()) && !Number.isNaN(required.getTime()) && eta > required;
    if (deliveryAtRisk) {
        return {
...
export function evaluateBookingStalled(input) {
    ...
    if (status !== "REQUESTED" && status !== "PENDING")
        return null;
    if (!input.bookingRequestedAt)
        return null;
    const requested = new Date(input.bookingRequestedAt);
    const now = input.now ?? new Date();
    if (Number.isNaN(requested.getTime()))
        return null;
    if (now.getTime() - requested.getTime() < BOOKING_STALLED_THRESHOLD_MS)
        return null;
    return {
...
export function evaluateBookingCutoffRisk(input) {
    ...
    if (Number.isNaN(ready.getTime()) || Number.isNaN(cutoff.getTime()))
        return null;
...
export function evaluateDocumentMissing(input) {
...
export function evaluateDocumentRejected(input) {
    return {
...
export function evaluateInspectionFailed() {
    return {
...
export function evaluateMilestoneOverdue(input) {
