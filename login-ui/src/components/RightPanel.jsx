...
const PILOT_DEMO_ACCOUNTS = [
  { email: "buyer.utest@demaxtore.local", label: "Alıcı", path: "/buyer/dashboard" },
  { email: "supplier.utest@demaxtore.local", label: "Türk Tedarikçi", path: "/supplier/dashboard" },
  { email: "supplier.foreign.utest@demaxtore.local", label: "Yurt Dışı Tedarikçi", path: "/supplier/freightiq" },
  { email: "broker.utest@demaxtore.local", label: "Gümrük", path: null },
  { email: "trucker.utest@demaxtore.local", label: "Nakliyeci", path: null },
  { email: "origin.utest@demaxtore.local", label: "Origin", path: null },
];
...
export default function RightPanel({ lang, setLang }) {
  ...
  const handleDemoLogin = async (demoEmail, demoPath) => {
    setDemoBusy(demoEmail);
    setLoginError("");
    try {
      const user = await loginWithCredentials(demoEmail, PILOT_DEMO_PASSWORD);
      window.location.assign(demoPath ?? postLoginPath(user.role));
    } catch (err) {
      const msg = err instanceof Error ? err.message : t.missingFieldsDesc;
      setLoginError(msg);
        ...
        <div className="w-full max-w-md">
          ...
          <div data-testid="pilot-demo-shortcuts" className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {PILOT_DEMO_ACCOUNTS.map(({ email, label, path }) => (
                <button
                  key={email}
                  type="button"
                  data-testid={`pilot-demo-${email.split("@")[0]}`}
                  disabled={submitting || !!demoBusy}
                  onClick={() => handleDemoLogin(email, path)}
                  title={email}
