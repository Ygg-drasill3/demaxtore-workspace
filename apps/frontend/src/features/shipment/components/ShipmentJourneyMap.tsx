...
import { LiveShipmentMap } from "./LiveShipmentMap";
...
export function ShipmentJourneyMap({ state, eta, isDelayed, originPort, destinationPort }: Props) {
  const milestones = shipmentMilestones(state);
  const progress = shipmentProgressPercent(state);

  return (
    <div data-testid="shipment-journey-map" data-guide="shipment-map" className="space-y-4">
      <LiveShipmentMap
        testId="shipment-journey-live-map"
        eyebrow="Live map"
        title="Route preview"
        origin={originPort}
