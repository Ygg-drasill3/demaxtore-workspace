...
export default function MixedContainerRequestsPage() {
  ...
  return (
    <div data-testid="mc-requests-page" className="max-w-[1200px] mx-auto space-y-6 animate-fade-in">
          ...
          <tbody>
            ...
            {(data?.items ?? []).map((row) => (
              <tr key={row.id} data-testid={`mc-request-row-${row.externalRef}`} className="border-t border-zinc-100">
                ...
                <td className="px-4 py-3"><span className="text-xs uppercase">{stateLabel(row.state)}</span></td>
            ...
            {!isLoading && (data?.items ?? []).length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-zinc-400">No containers yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
