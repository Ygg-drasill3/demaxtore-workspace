...
export default function GeneralMessagesPage() {
  ...
  const { data: thread, isLoading: threadLoading } = useQuery({
    ...
    refetchInterval: () => (getSocket().connected ? false : 4_000),
  });
  ...
  useEffect(() => {
    ...
    const onMessage = (payload: { conversationId?: string }) => {
      ...
      void qc.invalidateQueries({ queryKey: ["chat", "thread", conversationId] });
      void qc.invalidateQueries({ queryKey: ["chat", "conversations"] });
    };
    sock.on("chat:message:new", onMessage);
    return () => {
      sock.off("chat:message:new", onMessage);
    };
  }, [conversationId, qc]);
