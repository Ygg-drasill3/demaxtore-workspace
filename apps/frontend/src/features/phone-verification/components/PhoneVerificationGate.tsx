...
export function PhoneVerificationGate({ children }: { children: React.ReactNode }) {
  const { data, isLoading, refetch } = usePhoneVerificationMe();
  ...
  const pending =
    data?.phoneVerificationStatus === "PENDING_PHONE_VERIFICATION" || submit.isSuccess;
          ...
          <>
            {!openForm ? (
              ...
              <form
                ...
                onSubmit={(e) => {
                  e.preventDefault();
                  void submit.mutateAsync({ phone }).then(() => refetch());
                }}
              >
