...
export class JobService {
  ...
  async getHistory(limit = 100, jobName?: string): Promise<JobExecutionRecord[]> {
  ...
  async getJobStatuses(): Promise<JobStatusSummary[]> {
    const out: JobStatusSummary[] = [];
  ...
  async getFailedJobs(): Promise<FailedJobSummary[]> {
  ...
  async getStuckRunning(olderThanMs = 30 * 60_000): Promise<JobExecutionRecord[]> {
    const rows = await this.db.jobExecution.findMany({
      where: {
        status: "RUNNING",
  ...
  async getSchedulerStatuses(): Promise<SchedulerStatus[]> {
