...
import type { HealthComponentStatus, StorageHealthReport } from "@dmx/contracts/enterprise-readiness";
...
export class StorageHealthService {
  ...
  async scan(): Promise<StorageHealthReport> {
    ...
    const refs: Array<{ kind: string; id: string; storageKey: string }> = [];
    ...
    for (const a of rfqAtt) refs.push({ kind: "rfq_attachment", id: a.id, storageKey: a.storageKey });
    for (const a of orderDoc) refs.push({ kind: "order_document", id: a.id, storageKey: a.storageKey });
    for (const a of shipDoc) refs.push({ kind: "shipment_document", id: a.id, storageKey: a.storageKey });
    for (const a of commAtt) refs.push({ kind: "comm_attachment", id: a.id, storageKey: a.storageKey });

    let missingFiles = 0;
    let brokenReferences = 0;
    const samples: StorageHealthReport["samples"] = [];
    ...
    const totalReferences =
      (await this.db.rfqAttachment.count()) +
  ...
  async componentHealth(): Promise<HealthComponentStatus> {
