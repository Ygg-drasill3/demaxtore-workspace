...
const STALE_MESSAGE =
  "Marked FAILED by stale RUNNING reconciler (worker crash, hot reload, or lock timeout)";

export interface StaleJobReconcileResult {
  reconciled: number;
  jobIds: string[];
...
export async function reconcileStaleRunningJobs(
  db: PrismaClient,
  maxAgeMs: number,
): Promise<StaleJobReconcileResult> {
  ...
  logger.warn(
    { count: stale.length, jobNames: [...new Set(stale.map((s) => s.jobName))] },
    "Reconciled stale RUNNING job executions",
  );

  return {
    reconciled: stale.length,
    jobIds: stale.map((s) => s.id),
    oldestStartedAt: stale.reduce(
