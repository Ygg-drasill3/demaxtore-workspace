...
export async function exportSystemCsv(
  reportType: string,
  db: PrismaClient,
): Promise<string> {
  ...
  switch (reportType) {
    ...
    case "system-health": {
      ...
      const lines = snap.components.map((c) =>
        [c.key, c.status, c.detail ?? ""].map(csvEscape).join(","),
      );
      return [header, ...lines].join("\n");
    }
    case "backup-history": {
      ...
      const lines = rows.map((r) =>
        [r.checkType, r.status, r.verifiedAt.toISOString(), r.notes ?? ""]
          .map(csvEscape)
          .join(","),
      );
      return [header, ...lines].join("\n");
    ...
    case "storage-health": {
      ...
      const lines = rep.samples.map((s) =>
        [s.kind, s.id, s.storageKey, s.ok].map(csvEscape).join(","),
      );
      return [header, ...lines].join("\n");
    }
    case "scheduler-health": {
      ...
      const lines = rows.map((r) =>
        [r.name, r.lockId, r.healthy, r.stale, r.lastTickAt ?? "", r.missedExecutions]
          .map(csvEscape)
          .join(","),
      );
      return [header, ...lines].join("\n");
