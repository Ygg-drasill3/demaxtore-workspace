/**
 * Sprint 38 — Pre-Arrival Customs Engine.
 * Reuses CustomsCase ensure + readiness + Exception Intelligence. No second platform.
 */
...
import {
  ...
  daysUntil,
  type PreArrivalCustomsConfig,
  type PreArrivalEtaSource,
  type PreArrivalSummary,
} from "@dmx/contracts/pre-arrival-customs";
...
export type PreArrivalEvalResult = {
  shipmentWorkspaceId: string;
  ensured: boolean;
  skipped: boolean;
  skipReason: string | null;
  customsCaseId: string | null;
  preArrival: PreArrivalSummary | null;
};

export function createPreArrivalCustomsService(db: PrismaClient) {
  const customs = createCustomsService(db);
  const ei = new ExceptionIntelligenceService(db);

  async function loadConfig(): Promise<PreArrivalCustomsConfig> {
  ...
  async function resolveOperationalEta(shipmentWorkspaceId: string): Promise<{
    eta: Date | null;
    etaSource: PreArrivalEtaSource;
    bookingEta: Date | null;
    maritimeEta: Date | null;
    ata: Date | null;
  }> {
  ...
  async function isTurkeyEligible(shipmentWorkspaceId: string, orderWorkspaceId: string): Promise<boolean> {
    const sw = await db.shipmentWorkspace.findUnique({
      where: { workspaceId: shipmentWorkspaceId },
    ...
    const snap = po?.revisions[0]?.snapshotJson as
      | { header?: { destinationCountryCode?: string | null; destinationCountry?: string | null } }
      | null
      | undefined;
    const fromPo =
      snap?.header?.destinationCountryCode ?? snap?.header?.destinationCountry ?? null;
    if (isTurkeyCountryCode(fromPo)) return true;
    const route = resolveFreightRoute(sw.originPort || "", sw.destinationPort || "");
    return isTurkeyCountryCode(route.countryTo);
  }

  function nextActionFor(summary: PreArrivalSummary, caseStatus: string | null): string | null {
    if (summary.phase === "CLEARED") return null;
    if (summary.phase === "READY_BEFORE_ARRIVAL") return "Monitor until arrival — preparation ready";
    if (summary.phase === "ARRIVED") return "Immediate customs preparation action required";
    if (summary.blockingCount > 0) return "Resolve blocking readiness items";
    if (caseStatus === "READY_FOR_BROKER") return "Start broker review";
  ...
  async function evaluateShipment(
    ...
    if (!existing) {
      ...
      const shouldEnsure =
        !!etaInfo.ata || (days != null && days <= config.caseEnsureDays) || opts?.force;
      if (shouldEnsure) {
    ...
    const classificationMissing = readiness.checks.some(
      (c) => c.code === "GTIP_CLASSIFICATION" && c.status === "FAIL",
    );
    const classificationCandidate = readiness.checks.some(
      (c) => c.code === "GTIP_CLASSIFICATION" && c.status === "WARNING" && c.reason === "CANDIDATE",
    );
    const invoiceMissing = readiness.checks.some(
      (c) => c.code === "COMMERCIAL_INVOICE" && c.status === "FAIL",
    );
    const packingMissing = readiness.checks.some(
      (c) => c.code === "PACKING_LIST" && c.status === "FAIL",
    );
    ...
    const preparationAtRisk =
      nearHighRisk && readiness.status !== "READY_FOR_BROKER" && caseRow.status !== "CLEARED";
    const brokerReviewPending =
      inBrokerWindow
      && !brokerMissing
      && readiness.status === "READY_FOR_BROKER"
      && ["DRAFT", "PREPARING", "READY_FOR_BROKER"].includes(caseRow.status);
    const arrivedNotReady =
      !!etaInfo.ata
      && readiness.status !== "READY_FOR_BROKER"
    ...
    await ei.onCustomsReadiness({
      ...
      classificationMissing:
        classificationMissing && (inBrokerWindow || nearHighRisk || !!etaInfo.ata),
  ...
  async function safeEvaluateShipment(shipmentWorkspaceId: string): Promise<PreArrivalEvalResult> {
    try {
   ...
   */
  async function runScan(limit = 40): Promise<{ evaluated: number; ensured: number; skipped: number }> {
    const config = await loadConfig();
    if (!config.enabled) return { evaluated: 0, ensured: 0, skipped: 0 };
        ...
        AND: [
          {
            OR: [
              ...
              { arrivedAt: { not: null }, customsCompletedAt: null },
              {
                workspaceId: {
                  in: (
                    await db.shipmentTrackingSnapshot.findMany({
    ...
    for (const row of candidates) {
      ...
      evaluated++;
      if (r.ensured) ensured++;
      if (r.skipped) skipped++;
    }
    return { evaluated, ensured, skipped };
  }

  async function summarizeForCase(customsCaseId: string): Promise<PreArrivalSummary | null> {
...
export type PreArrivalCustomsService = ReturnType<typeof createPreArrivalCustomsService>;
