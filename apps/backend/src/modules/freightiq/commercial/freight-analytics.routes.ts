...
freightAnalyticsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.status(201).json(
      await policies.createPolicy(req.user!, body.payload ?? req.body ?? {}, {
        ip: req.ip,
...
freightAnalyticsRouter.patch(
  "/margin/policies/:policyId",
  requireAuth,
  requireRole("ADMIN"),
  asyncHandler(async (req, res) => {
    assertAdminCommercial(req.user!);
    const body = (req.body ?? {}) as { payload?: Record<string, unknown> };
    res.json(
      await policies.updatePolicy(req.params.policyId, req.user!, body.payload ?? req.body ?? {}, {
        ip: req.ip,
        userAgent: req.headers["user-agent"],
      }),
    );
