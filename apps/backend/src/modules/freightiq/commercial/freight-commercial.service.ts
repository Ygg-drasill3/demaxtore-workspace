...
import type {
  ...
  FreightCommercialSummary,
  FreightMargin,
  FreightRevenueLedgerEntry,
...
export class FreightCommercialService {
  ...
  applyRoleToSummary(summary: FreightSummary, actor: AuthUser): FreightSummary {
  ...
  sanitizeOffer<T extends { price: number; commercial?: FreightMargin }>(
    offer: T,
    includeCommercial: boolean,
  ): T {
    if (includeCommercial) return offer;
    const { commercial: _c, ...rest } = offer as T & { commercial?: FreightMargin };
    return rest as T;
  }

  mapDbOfferForRole(
    o: Parameters<typeof commercialFromOffer>[0] & {
      id: string;
    ...
    if (!canViewOfferCommercial(actor)) return base;
    return { ...base, commercial: commercialFromOffer(o) };
  }

  async buildCommercialSummary(orderId: string, summary: FreightSummary): Promise<FreightCommercialSummary | null> {
    ...
    let displayFreight: number | null = null;
  ...
  async setOfferMargin(
    ...
    if (offer.marginLockedAt) throw new AppError(409, "MARGIN_LOCKED");
    ...
    const suggestion = await policySvc.suggestMargin(
      offer.freightRequest.pol,
      offer.freightRequest.pod,
    );

    await this.db.$transaction(async (tx) => {
      await policySvc.recordMarginOverride(
        tx,
        {
          orderId,
          actor,
      ...
      await tx.auditLog.create({
        data: {
          ...
          fromState: ws.state,
          toState: ws.state,
          payload: { offerId, ...input, displayPriceUsd } as Prisma.InputJsonValue,
          ipAddress: ctx?.ip,
      ...
      await tx.timelineEvent.create({
        data: {
          ...
          eventType: action,
          actorUserId: actor.id,
          payload: { offerId, displayPriceUsd } as Prisma.InputJsonValue,
        },
      });
    });
  ...
  async realizeRevenueForShipment(shipmentId: string, actor?: AuthUser) {
      ...
      await this.db.$transaction(async (tx) => {
        ...
        await tx.timelineEvent.create({
          data: {
            ...
            actorUserId: actor?.id ?? "00000000-0000-0000-0000-000000000001",
            payload: { ledgerId: entry.id, shipmentId } as Prisma.InputJsonValue,
          },
        });
      });
      realized += 1;
  ...
  async getMetrics(): Promise<FreightCommercialMetrics> {
    ...
    const routeMap = new Map<string, { count: number; marginUsd: number }>();
    const forwarderMap = new Map<string, { count: number; marginUsd: number }>();
    let marginSum = 0;
    let marginCount = 0;
  ...
  async getReport(): Promise<FreightCommercialReport> {
    ...
    const mapEntry = (r: (typeof rows)[number]): FreightRevenueLedgerEntry => ({
      id: r.id,
      ...
      currency: r.currency,
      status: r.status as FreightRevenueLedgerEntry["status"],
    ...
    const monthMap = new Map<string, { realizedUsd: number; pendingUsd: number }>();
    const routeRev = new Map<string, { realizedUsd: number; pendingUsd: number }>();
    const fwdRev = new Map<string, { realizedUsd: number; pendingUsd: number }>();
    ...
    for (const r of rows) {
      ...
      if (r.status === "REALIZED") mo.realizedUsd += m;
      else if (r.status === "PENDING") mo.pendingUsd += m;
      monthMap.set(month, mo);

      const fr = r.offer.freightRequest;
      const route = `${fr.pol}→${fr.pod}`;
      const rr = routeRev.get(route) ?? { realizedUsd: 0, pendingUsd: 0 };
      if (r.status === "REALIZED") rr.realizedUsd += m;
      else rr.pendingUsd += m;
      ...
      if (r.status === "REALIZED") frw.realizedUsd += m;
      else frw.pendingUsd += m;
      fwdRev.set(fwd, frw);
    }
