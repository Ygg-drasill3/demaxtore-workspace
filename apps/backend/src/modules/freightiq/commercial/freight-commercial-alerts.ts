...
export async function scanFreightCommercialAlerts(db: PrismaClient): Promise<number> {
  let n = 0;
  ...
  const routeMargin = new Map<string, { margin: number; count: number }>();
  for (const row of ledger) {
    if (!row.offer?.freightRequest) continue;
  ...
  const globalAvg =
    [...routeMargin.values()].reduce((s, v) => s + v.margin, 0) /
...
async function auditCommercialAlert(
  db: PrismaClient,
  workspaceId: string,
  alertKey: string,
  offerId: string,
