...
import type { AnalyticsFilterQuery } from "@dmx/contracts/operational-analytics.zod";
...
export function resolveAnalyticsRange(
  ...
  const preset = (query.preset ?? "LAST_30_DAYS") as AnalyticsTimePreset;
  let from: Date;
  let to = now;
...
function previousRange(range: AnalyticsTimeRangeDto): { from: Date; to: Date } {
...
export class OperationalKPIService {
  ...
  private assertView(role: Role) {
    const perms = computeAnalyticsPermissions(role);
    if (!perms.canView) throw Forbidden("Analytics dashboard requires ops viewer access");
    return perms;
  }

  private assertSuppliers(role: Role) {
    const perms = this.assertView(role);
    if (!perms.canViewSuppliers) throw Forbidden("Supplier KPIs require manager access");
    return perms;
  }

  private assertExport(role: Role) {
    const perms = this.assertView(role);
    if (!perms.canExport) throw Forbidden("Analytics export requires manager/admin access");
    return perms;
  }
  ...
  async summary(
    ...
    query: AnalyticsFilterQuery,
  ): Promise<OperationalAnalyticsSummaryDto> {
    ...
    const payload = await cached(cacheKey, SUMMARY_CACHE_TTL_MS, async () => {
      const [orders, shipments, inspections, tasks, issues, completion, trends] =
        await Promise.all([
  ...
  async orders(
    actor: { id: string; role: Role },
    query: AnalyticsFilterQuery,
  ): Promise<{ range: AnalyticsTimeRangeDto; kpis: OrdersKpiDto }> {
  ...
  async shipments(
    actor: { id: string; role: Role },
    query: AnalyticsFilterQuery,
  ): Promise<{ range: AnalyticsTimeRangeDto; kpis: ShipmentsKpiDto }> {
    this.assertView(actor.role);
    const range = resolveAnalyticsRange(query);
  ...
  async inspections(
    actor: { id: string; role: Role },
    query: AnalyticsFilterQuery,
  ): Promise<{ range: AnalyticsTimeRangeDto; kpis: InspectionsKpiDto }> {
    this.assertView(actor.role);
    const range = resolveAnalyticsRange(query);
  ...
  async tasks(
    actor: { id: string; role: Role },
    query: AnalyticsFilterQuery,
  ): Promise<{ range: AnalyticsTimeRangeDto; kpis: TasksKpiDto }> {
    this.assertView(actor.role);
    const range = resolveAnalyticsRange(query);
  ...
  async issues(
    actor: { id: string; role: Role },
    query: AnalyticsFilterQuery,
  ): Promise<{ range: AnalyticsTimeRangeDto; kpis: IssuesKpiDto }> {
    this.assertView(actor.role);
    const range = resolveAnalyticsRange(query);
  ...
  async completion(
    actor: { id: string; role: Role },
    query: AnalyticsFilterQuery,
  ): Promise<{ range: AnalyticsTimeRangeDto; kpis: CompletionKpiDto }> {
    this.assertView(actor.role);
    const range = resolveAnalyticsRange(query);
  ...
  async export(
    actor: { id: string; role: Role },
    query: AnalyticsFilterQuery & { format?: AnalyticsExportFormat; scope?: AnalyticsExportScope },
  ): Promise<{ filename: string; contentType: string; body: Buffer | string }> {
    this.assertExport(actor.role);
    if (query.scope === "suppliers") this.assertSuppliers(actor.role);
  ...
  private async ordersKpis(range: AnalyticsTimeRangeDto): Promise<OrdersKpiDto> {
    const from = new Date(range.from);
    const to = new Date(range.to);
    ...
    const openOrders = orders.filter(
      (o) => !["CLOSED", "CANCELLED", "REJECTED"].includes(o.state),
    ).length;
    ...
    const closedInRange = orders.filter((o) =>
      ["CLOSED", "DELIVERED", "COMPLETED"].includes(o.state),
    ).length;
  ...
  private async shipmentsKpis(range: AnalyticsTimeRangeDto): Promise<ShipmentsKpiDto> {
    const from = new Date(range.from);
    const to = new Date(range.to);
    ...
    const delayedIds = new Set(
      delayedMilestones.map((m) => m.shipmentWorkspace.workspaceId),
    );
    ...
    if (deliveries.length) {
      const shipmentWorkspaceIds = deliveries.map((d) => d.shipmentId).filter(Boolean) as string[];
      ...
      const planBySw = new Map(
        deliveryMs.map((m) => [m.shipmentWorkspaceId, m.estimatedAt ?? m.plannedAt]),
      );
      for (const d of deliveries) {
        ...
        if (!plan) continue;
        withPlan += 1;
        if (d.deliveredAt.getTime() <= plan.getTime()) onTime += 1;
      }
    }
  ...
  private async inspectionsKpis(range: AnalyticsTimeRangeDto): Promise<InspectionsKpiDto> {
    const from = new Date(range.from);
    const to = new Date(range.to);
    ...
    const passed = rows.filter(
      (r) =>
        (r.decision && PASS_DECISIONS.has(r.decision.toUpperCase()))
        || r.status === "APPROVED",
    ).length;
    const failed = rows.filter(
      (r) =>
        (r.decision && FAIL_DECISIONS.has(r.decision.toUpperCase()))
        || r.status === "FAILED",
    ).length;
  ...
  private async tasksKpis(range: AnalyticsTimeRangeDto): Promise<TasksKpiDto> {
    const from = new Date(range.from);
    const to = new Date(range.to);
    const now = new Date();
  ...
  private async issuesKpis(range: AnalyticsTimeRangeDto): Promise<IssuesKpiDto> {
    const from = new Date(range.from);
    const to = new Date(range.to);
    const now = new Date();
    const todayStart = startOfDay(now);
  ...
  private async completionKpis(range: AnalyticsTimeRangeDto): Promise<CompletionKpiDto> {
    const from = new Date(range.from);
    const to = new Date(range.to);
  ...
  private async trends(range: AnalyticsTimeRangeDto): Promise<AnalyticsTrendPoint[]> {
    const prev = previousRange(range);
    const prevDto: AnalyticsTimeRangeDto = {
      preset: "CUSTOM",
      from: prev.from.toISOString(),
      to: prev.to.toISOString(),
    };

    const [curOrders, prevOrders, curShip, prevShip, curTasks, prevTasks, curIssues, prevIssues] =
      await Promise.all([
        this.ordersKpis(range),
  ...
  private async supplierRows(range: AnalyticsTimeRangeDto): Promise<SupplierKpiRowDto[]> {
    const from = new Date(range.from);
    const to = new Date(range.to);
    ...
    const shipsByOrder = new Map<string, { id: string; state: string }[]>();
    for (const s of shipments) {
      if (!s.spawnedFromId) continue;
    ...
    for (const o of orderDetails) {
      ...
      if (!["CLOSED", "CANCELLED", "REJECTED"].includes(state)) acc.openOrders += 1;
      ...
      if (completion?.status === "COMPLETED" || state === "CLOSED") {
        ...
        if (end) acc.completionHours.push(hoursBetween(o.workspace.createdAt, end));
      }

      if (o.inspectionResult) {
        acc.inspTotal += 1;
        if (PASS_DECISIONS.has(o.inspectionResult.toUpperCase())) acc.inspPass += 1;
      }

      const ships = shipsByOrder.get(o.workspaceId) ?? [];
      for (const s of ships) {
        acc.shipTotal += 1;
        if (delayedShipIds.has(s.id)) acc.delayHits += 1;
      }
  ...
  private async exportTable(
    ...
    role: Role,
  ): Promise<{ headers: string[]; rows: unknown[][]; sheet: string }> {
    switch (scope) {
      case "orders": {
