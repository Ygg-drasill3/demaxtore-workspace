...
function lots(ws: PreconditionInput["workspace"]) {
  return (ws.commodityBidLots as Array<{ id: string; noAwardReason?: string | null }>) ?? [];
}
function invitations(ws: PreconditionInput["workspace"]) {
  return (ws.commodityBidInvitations as Array<{ supplierUserId: string; removedAt: Date | null }>) ?? [];
}
function submissions(ws: PreconditionInput["workspace"]) {
  return (ws.commodityBidSubmissions as Array<{
    id: string; lotId: string; supplierUserId: string; withdrawnAt: Date | null; validUntil: Date; unitPrice?: unknown;
  }>) ?? [];
}
function awards(ws: PreconditionInput["workspace"]) {
  return (ws.commodityBidAwards as Array<{ lotId: string; supplierUserId: string; status: string }>) ?? [];
}
...
export const PRECONDITIONS: Record<string, PreconditionFn> = {
  assertLotFieldsValid: ({ workspace, payload }) => {
    ...
    if (!workspace.currency) throw new AppError(400, "COMMODITYBID_NO_CURRENCY");
  },
  ...
  assertAtLeastOneLotRemains: ({ workspace }) => {
    if (lots(workspace).length <= 1) throw new AppError(400, "COMMODITYBID_LAST_LOT");
  },
  assertSubmitBidPreconditions: ({ workspace }) => {
    if (!lots(workspace).length) throw new AppError(400, "COMMODITYBID_NO_LOTS");
    if (!workspace.deadlineAt || new Date(workspace.deadlineAt as string) <= new Date())
  ...
  assertAtLeastOneSupplier: ({ payload }) => {
    ...
    if (!ids.length) throw new AppError(400, "COMMODITYBID_NO_SUPPLIERS");
  },
  assertNotYetTriaged: ({ workspace }) => {
    if (workspace.state !== "BID_SUBMITTED") throw new AppError(409, "COMMODITYBID_ALREADY_TRIAGED");
  },
  ...
  assertSupplierNew: ({ workspace, payload }) => {
    ...
    if (incoming.some((id) => existing.has(id))) throw new AppError(409, "COMMODITYBID_SUPPLIER_EXISTS");
  },
  ...
  assertDeadlineExtensionAllowed: ({ workspace, payload }) => {
    const count = (workspace.deadlineExtensionCount as number) ?? 0;
    if (count >= MAX_DEADLINE_EXTENSIONS) throw new AppError(409, "COMMODITYBID_DEADLINE_EXTENSION_LIMIT");
    const newDeadline = new Date(payload.newDeadline as string);
    const old = new Date(workspace.deadlineAt as string);
    const addedDays = Math.ceil((newDeadline.getTime() - old.getTime()) / 86400_000);
    const total = ((workspace.deadlineExtensionTotalDays as number) ?? 0) + addedDays;
    if (total > MAX_DEADLINE_EXTENSION_DAYS) throw new AppError(409, "COMMODITYBID_DEADLINE_EXTENSION_DAYS_LIMIT");
  },
  assertHasBids: ({ workspace }) => {
    if (!submissions(workspace).some((b) => !b.withdrawnAt)) throw new AppError(409, "COMMODITYBID_NO_BIDS");
  },
  assertNoBids: ({ workspace }) => {
    if (submissions(workspace).some((b) => !b.withdrawnAt)) throw new AppError(409, "COMMODITYBID_HAS_BIDS");
  },
  assertNewDeadline: ({ payload }) => {
    if (!payload.newDeadline || new Date(payload.newDeadline as string) <= new Date())
      throw new AppError(400, "COMMODITYBID_DEADLINE_PAST");
  },
  assertBidNotWithdrawn: ({ workspace, payload }) => {
    const sub = submissions(workspace).find((b) => b.id === payload.submissionId);
    if (!sub || sub.withdrawnAt) throw new AppError(409, "COMMODITYBID_BID_WITHDRAWN");
  },
  assertBidValidityNotExpired: ({ workspace, payload }) => {
    const sub = submissions(workspace).find((b) => b.id === payload.submissionId);
    if (sub && new Date(sub.validUntil) < new Date()) throw new AppError(409, "COMMODITYBID_BID_EXPIRED");
  },
  assertAllLotsDecided: ({ workspace }) => {
    for (const lot of lots(workspace)) {
      ...
      if (!hasNoAward && !hasDraft) throw new AppError(409, "COMMODITYBID_LOT_UNDECIDED", { lotId: lot.id });
    }
  },
  assertPublishedAwardForSupplierOnLot: ({ workspace, payload, actor }) => {
    const ok = awards(workspace).some(
      (a) => a.lotId === payload.lotId && a.supplierUserId === actor.id && a.status === "PUBLISHED",
    );
    if (!ok) throw new AppError(404, "COMMODITYBID_AWARD_NOT_FOUND");
  },
  assertAwardNotYetAccepted: ({ workspace, payload, actor }) => {
    const lotId = payload.lotId as string;
    const award = awards(workspace).find(
      (a) => a.lotId === lotId && a.supplierUserId === actor.id,
    );
  ...
  assertPreviousAwardTerminal: ({ workspace, payload }) => {
    ...
    const blocking = awards(workspace).find(
      (a) => a.lotId === lotId && (a.status === "PUBLISHED" || a.status === "ACCEPTED"),
    );
    if (blocking) throw new AppError(409, "COMMODITYBID_PRIOR_AWARD_NOT_TERMINAL", { lotId });
    const stale = awards(workspace).find(
      (a) => a.lotId === lotId && !terminal.has(a.status) && a.status !== "DRAFT",
    );
    if (stale) throw new AppError(409, "COMMODITYBID_PRIOR_AWARD_NOT_TERMINAL", { lotId });
  },
  assertEligibleReplacementBid: ({ workspace, payload }) => {
    ...
    if (!sub || sub.withdrawnAt) throw new AppError(404, "COMMODITYBID_REPLACEMENT_BID_NOT_FOUND");
    if (sub.lotId !== lotId) throw new AppError(400, "COMMODITYBID_BID_LOT_MISMATCH");
    if (new Date(sub.validUntil) < new Date()) throw new AppError(409, "COMMODITYBID_BID_EXPIRED");
    const declined = awards(workspace).find(
      (a) => a.lotId === lotId && ["DECLINED", "WITHDRAWN", "EXPIRED"].includes(a.status),
    );
    if (!declined) throw new AppError(409, "COMMODITYBID_NO_TERMINAL_AWARD_FOR_REAWARD");
    const sameSupplier = awards(workspace).find(
      (a) => a.lotId === lotId && a.supplierUserId === sub.supplierUserId && a.status !== "RE_AWARDED",
    );
  ...
  assertContractNumbersUnique: ({ payload }) => {
    const refs = payload.contractRefs as Record<string, string> | undefined;
    if (!refs) return;
