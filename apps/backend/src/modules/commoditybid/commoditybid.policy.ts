...
export async function canAccessCommodityBid(
  prisma: PrismaClient,
  user: AuthUser,
  workspaceId: string,
): Promise<boolean> {
  ...
  if (user.role === "SUPPLIER") {
    ...
    if (!ws || ws.type !== "COMMODITYBID") return false;
...
export function canViewAnonymousComparison(user: AuthUser): boolean {
  return user.role === "BUYER" || user.role === "ADMIN";
}

/** Only admin resolves bidder code ↔ supplier identity. */
export function canViewSupplierIdentity(user: AuthUser): boolean {
