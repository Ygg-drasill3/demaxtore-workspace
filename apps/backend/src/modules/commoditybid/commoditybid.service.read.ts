...
import { ListCommodityBidQuery } from "@dmx/contracts/commoditybid.zod";
import type { z } from "zod";
type ListCommodityBidQueryT = z.infer<typeof ListCommodityBidQuery>;
import type { CommodityBidNextActionContext } from "@dmx/contracts/commoditybid.next-actions";
import { CommodityBidService } from "./commoditybid.service.js";
import { AppError } from "../../utils/httpErrors.js";
import type { AuthUser } from "./commoditybid.policy.js";
import { canViewAnonymousComparison, canViewSupplierIdentity } from "./commoditybid.policy.js";
...
declare module "./commoditybid.service" {
  interface CommodityBidService {
    createDraft(input: CreateCommodityBidDraftInput, actor: AuthUser): Promise<unknown>;
    list(query: ListCommodityBidQueryT, actor: AuthUser): Promise<unknown>;
    toDTO(ws: WsFull, viewer: AuthUser): Promise<unknown>;
    fetchDTO(wsId: string, viewer: AuthUser): Promise<unknown>;
    timeline(wsId: string, query: unknown): Promise<unknown>;
    buildNextActionContext(ws: WsFull, actor: AuthUser): Promise<CommodityBidNextActionContext>;
    getComparison(wsId: string, actor: AuthUser): Promise<unknown>;
    getAdminIdentityMap(wsId: string): Promise<unknown>;
    getOwnBids(wsId: string, actor: AuthUser): Promise<unknown>;
    getMyAwards(wsId: string, actor: AuthUser): Promise<unknown>;
    getSpawnedOrders(wsId: string): Promise<unknown>;
    adminQueue(): Promise<unknown>;
  }
}

function dtoFromWorkspace(ws: WsFull) {
...
async function nextExternalRef(prisma: CommodityBidService["prisma"]): Promise<string> {
...
CommodityBidService.prototype.createDraft = async function (input, actor) {
  ...
  const id = await this.prisma.$transaction(async (tx) => {
    const externalRef = await nextExternalRef(tx as CommodityBidService["prisma"]);
      ...
      data: {
        ...
        commodityBidLots: {
          create: input.lots.map((lot, i) => ({
            ...
            uom: lot.uom,
            specs: (lot.specs ?? {}) as Prisma.InputJsonValue,
...
CommodityBidService.prototype.list = async function (query, actor) {
  const where: Prisma.WorkspaceWhereInput = { type: "COMMODITYBID" };
  if (query.state) where.state = query.state;
  if (actor.role === "BUYER") where.createdById = actor.id;
  ...
  return { items: rows.map((r) => dtoFromWorkspace(r as WsFull)), total: rows.length };
};
...
CommodityBidService.prototype.buildNextActionContext = async function (ws, actor) {
  ...
  return {
    state: ws.state as CommodityBidNextActionContext["state"],
    actorRole: actor.role as CommodityBidNextActionContext["actorRole"],
    isOwner: ws.createdById === actor.id,
    isCounterparty: (ws.participants ?? []).some(
...
CommodityBidService.prototype.getComparison = async function (wsId, actor) {
  if (!canViewAnonymousComparison(actor)) throw new AppError(403, "FORBIDDEN");
  const ws = await withRlsUser(actor.id, (tx) =>
  ...
  const codeBySupplier = new Map(
    ws.commodityBidInvitations.map((i) => [i.supplierUserId, i.bidderCode]),
  );
  return {
...
CommodityBidService.prototype.getMyAwards = async function (wsId, actor) {
  if (actor.role !== "SUPPLIER") throw new AppError(403, "FORBIDDEN");
...
CommodityBidService.prototype.getOwnBids = async function (wsId, actor) {
  if (actor.role !== "SUPPLIER") throw new AppError(403, "FORBIDDEN");
  const bids = await withRlsUser(actor.id, (tx) =>
    tx.commodityBidSubmission.findMany({
