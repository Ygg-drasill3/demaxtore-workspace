...
async function tick(): Promise<void> {
  ...
  if (!ran) await recordSkippedJob(prisma, "commoditybid_auction_engine", "lock_held");
}

export async function runCommodityBidSchedulerTick(): Promise<void> {
  await tick();
}

export function startCommodityBidScheduler(): NodeJS.Timeout {
