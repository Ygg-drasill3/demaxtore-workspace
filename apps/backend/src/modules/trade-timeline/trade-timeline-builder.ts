...
import type {
  TradeMilestoneType,
  TradeTimelineCategory,
  TradeTimelineSeverity,
  TradeTimelineSourceModule,
} from "@dmx/contracts/trade-timeline";
import type { TradeGraph } from "../trade/trade.resolver.js";

export interface RawTimelineInput {
  id: string;
  eventType: string;
  workspaceId: string;
  createdAt: Date;
...
export interface BuiltTimelineEvent {
  ...
  visibility: "ALL" | "BUYER" | "ADMIN";
}

interface EventRule {
  eventType: string;
  milestoneType?: TradeMilestoneType;
  eventCategory: TradeTimelineCategory;
  sourceModule: TradeTimelineSourceModule;
  severity: TradeTimelineSeverity;
  title: string;
...
const PREFIX_RULES: Array<{ prefix: string; rule: Partial<EventRule> }> = [
...
function resolveRule(rawEventType: string): EventRule {
...
export function buildTimelineEvents(
  inputs: RawTimelineInput[],
  graph: TradeGraph,
): BuiltTimelineEvent[] {
  const wsTypeMap = new Map<string, string>();
  wsTypeMap.set(graph.root.id, graph.root.type);
  for (const id of graph.orderIds) wsTypeMap.set(id, "ORDER");
  for (const id of graph.shipmentIds) wsTypeMap.set(id, "SHIPMENT");
