...
export function isExceptionEngineV2Enabled(): boolean {
...
export class ExceptionEngineService {
  ...
  async upsertFromAlert(input: {
    ...
    if (!isExceptionEngineV2Enabled()) return null;
  ...
  private mapAlertToType(alertKey: string): string {
  ...
  private normalizeSeverity(s: string): ExceptionSeverity {
    if (s === "CRITICAL") return "Critical";
    if (s === "WARNING") return "Medium";
    return "High";
  }

  private defaultOwnerRole(alertKey: string): string {
    if (alertKey.includes("PAYMENT") || alertKey.includes("PO")) return "Finance";
    if (alertKey.includes("SHIPMENT") || alertKey.includes("TRACKING")) return "Logistics";
    if (alertKey.includes("DOC")) return "Document Controller";
    return "Operations";
  }
}
