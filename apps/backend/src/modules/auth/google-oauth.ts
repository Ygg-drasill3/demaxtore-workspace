...
async function exchangeCodeForTokens(code: string): Promise<GoogleTokenResponse> {
  ...
  const data = (await res.json().catch(() => ({}))) as GoogleTokenResponse & { error?: string };
  if (!res.ok) {
...
async function fetchGoogleUserInfo(accessToken: string): Promise<GoogleUserInfo> {
  ...
  const data = (await res.json().catch(() => ({}))) as GoogleUserInfo & { error?: string };
  if (!res.ok || !data.sub) {
...
function defaultOrganisationName(email: string, displayName: string): string {
