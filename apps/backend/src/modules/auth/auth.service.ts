...
export async function login(
  ...
  password: string,
  ip: string,
): Promise<IssueTokensResult & { user: ReturnType<typeof toUserDTO> }> {
...
export async function refresh(rawRefreshToken: string): Promise<IssueTokensResult> {
  let payload;
  try {
...
export async function forgotPassword(rawEmail: string): Promise<string | null> {
  const email = rawEmail.trim().toLowerCase();
  const user  = await prisma.user.findUnique({ where: { email } });
  if (!user) return null; // generic — no enumeration
