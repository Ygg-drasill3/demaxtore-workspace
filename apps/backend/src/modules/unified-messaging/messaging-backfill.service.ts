...
export type BackfillSource = "workspace" | "direct" | "whatsapp" | "clarification";

export interface BackfillReport {
  mode: "dry-run" | "apply";
  sources: BackfillSource[];
  conversationsFound: Record<BackfillSource, number>;
  messagesFound: Record<BackfillSource, number>;
  ...
  unmatchedWorkspaceContext: number;
  missingPhone: number;
  missingAuthor: number;
  conflictingMetaMessageId: number;
  conflictingLegacyId: number;
...
export function computeBackfillConfirmToken(report: Omit<BackfillReport, "confirmToken">): string {
...
function emptyCounts(): Record<BackfillSource, number> {
...
export class MessagingBackfillService {
  ...
  private async backfillWorkspace(
    ...
    const where: Prisma.WorkspaceConversationWhereInput = options.conversationId
    ...
    for (const conv of convs) {
      ...
      const hasContext = conv.contexts.some(
        (c) => c.contextType === contextType && c.contextId === conv.workspaceId,
      );
      if (!hasContext) {
        if (!options.dryRun) {
      ...
      const authorIds = new Set(
        conv.messages.map((m) => m.authorUserId).filter((id): id is string => Boolean(id)),
      );
  ...
  private async backfillDirect(
    report: BackfillReport,
    options: BackfillOptions,
    batchSize: number,
  ) {
    const where: Prisma.DirectConversationWhereInput = options.conversationId
      ? { id: options.conversationId }
      : {};
        ...
        if (!options.dryRun) {
          unifiedConv = await this.prisma.workspaceConversation.create({
            data: {
              ...
              metadata: { legacyDirectConversationId: direct.id } as Prisma.InputJsonValue,
              contexts: {
                create: {
                  contextType,
  ...
  private async ensureDirectParticipants(
    conversationId: string,
    direct: { buyerUserId: string; peerUserId: string | null },
  ) {
    for (const [userId, role] of [
      [direct.buyerUserId, "OWNER"],
      ...(direct.peerUserId ? [[direct.peerUserId, "MEMBER"]] as const : []),
    ]) {
  ...
  private async backfillWhatsapp(
    ...
    const where: Prisma.WhatsAppConversationWhereInput = options.conversationId
      ? { id: options.conversationId }
      : {};
    ...
    for (const wa of convs) {
      ...
      if (!unifiedConv) {
        ...
        if (wa.userId) report.participantsCreated += 1;
      }
  ...
  private async backfillClarification(
    report: BackfillReport,
    options: BackfillOptions,
    batchSize: number,
  ) {
