...
export class UnifiedShadowProjector {
  ...
  async projectWorkspaceCommunication(
    workspaceType: string,
    workspaceId: string,
    actor: AuthUser,
  ): Promise<NormalizedMessagingResult> {
    ...
    if (!conv) return emptyNormalized("workspace_communication");
    ...
    for (const m of conv.messages) {
      ...
      if (m.messageType === "SYSTEM_EVENT" || m.audienceScope === "SYSTEM") systemCount += 1;
    ...
    const visibleMessages =
      actor.role === "BUYER" || actor.role === "SUPPLIER"
  ...
  async projectDirectChat(conversationId: string): Promise<NormalizedMessagingResult> {
    const conv = await this.prisma.directConversation.findUnique({
      where: { id: conversationId },
      include: { messages: true },
    });
    if (!conv) return emptyNormalized("direct_chat");
    return {
  ...
  async projectWhatsAppConversation(conversationId: string): Promise<NormalizedMessagingResult> {
    const conv = await this.prisma.whatsAppConversation.findUnique({
      where: { id: conversationId },
      include: { messages: true },
    });
    if (!conv) return emptyNormalized("whatsapp_inbox");
  ...
  async projectClarifications(workspaceId: string): Promise<NormalizedMessagingResult> {
    const thread = await this.prisma.clarificationThread.findUnique({
      where: { workspaceId },
      include: { messages: true },
    });
    if (!thread) return emptyNormalized("rfq_clarifications");
    return {
