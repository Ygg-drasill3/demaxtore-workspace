...
export function isWhatsAppServiceWindowOpen(lastInboundAt: Date | null): boolean {
...
export function formatWhatsAppOutboundBody(senderName: string | null | undefined, body: string): string {
...
function coldOutreachBlockedError(buyerDisplayPhone?: string): AppError {
  ...
  return new AppError(400, "WHATSAPP_COLD_OUTREACH_BLOCKED", {
    message:
      `WhatsApp 24 saat penceresi kapalı — ilk mesaj Meta tarafından reddedildi (131047). ${hint} Şimdilik Workspace kanalını kullanın.`,
    businessLine: line,
  });
}

function toSendCredentials(credentials: WhatsAppTenantCredentials) {
  return {
...
async function sendApprovedTemplate(
  prisma: PrismaClient,
  phone: string,
  rfqRef: string | null | undefined,
  messageBody: string,
  credentials: WhatsAppTenantCredentials,
  templateOverride?: { templateName: string; templateLanguage: string },
): Promise<{ metaMessageId: string; body: string }> {
  const buyerTemplate =
    templateOverride ??
  ...
  if (!resolvedTemplate) {
    if (isBuyerConnectionWhatsAppMode()) {
      throw new AppError(400, "WHATSAPP_TEMPLATE_REQUIRED", {
        message:
          "An approved WhatsApp template is required for cold outreach. Configure a template on your connected WhatsApp Business Account.",
      });
    }
...
export type WhatsAppOutboundResult = {
  ...
  mode: "text" | "template" | "interactive";
  body: string;
};
...
async function sendRfqInteractiveSupplierOutreach(
  input: {
    phone: string;
...
export async function assertWhatsAppOutboundAllowed(
  ...
  if (!normalized) throw new AppError(400, "INVALID_PHONE");

  const lastInboundAt = await resolveLatestInboundAt(prisma, phone, phoneNumberId);
  if (isWhatsAppServiceWindowOpen(lastInboundAt)) return;
  ...
  if (buyerTemplate?.templateName ?? env.WHATSAPP_RFQ_TEMPLATE_NAME?.trim()) return;
...
export async function sendWhatsAppOutbound(
  ...
  if (isWhatsAppServiceWindowOpen(lastInboundAt)) {
    ...
    if (!result.metaMessageId) {
      await connectionService.handleMetaSendError(
        input.credentials.buyerId,
        result.errorCode,
        result.error,
      );
      throw new AppError(502, "WHATSAPP_SEND_FAILED", {
  ...
  if (resolvedTemplate) {
    const templated = await sendApprovedTemplate(
      prisma,
      input.phone,
      input.rfqRef,
      input.body,
...
export async function sendRfqWhatsAppOpeningInvite(
  prisma: PrismaClient,
  ...
  if (!unifiedConv) return false;

  const meta =
    typeof unifiedConv.metadata === "object" && unifiedConv.metadata && !Array.isArray(unifiedConv.metadata)
      ? (unifiedConv.metadata as Record<string, unknown>)
      : {};
  if (meta.rfqWhatsAppInviteSentAt) return false;
  ...
  try {
    ...
    if (env.WHATSAPP_RFQ_TEMPLATE_NAME?.trim()) {
      const templated = await sendApprovedTemplate(
        ...
        normalized,
        rfqRef,
        "DeMaxtore üzerinde yeni bir teklif talebiniz var.",
        credentials,
      );
