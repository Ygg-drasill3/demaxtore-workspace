...
export const DELAYED_SUPPLIER_REPLY_BODY =
  "Your supplier has replied to your message. Open your DeMaxtore Workspace to view the latest response.";
 ...
 */
export async function maybeNotifyBuyerOfDelayedSupplierReply(
  prisma: PrismaClient,
  conversationId: string,
  inboundMessageId: string,
  inboundCreatedAt: Date = new Date(),
): Promise<void> {
  try {
    ...
    const meta =
      typeof conv.metadata === "object" && conv.metadata && !Array.isArray(conv.metadata)
        ? ({ ...(conv.metadata as Record<string, unknown>) } as Record<string, unknown>)
        : {};

    const notifiedFor = Array.isArray(meta.delayedReplyNotifiedFor)
      ? (meta.delayedReplyNotifiedFor as unknown[]).filter((x): x is string => typeof x === "string")
      : [];
    if (notifiedFor.includes(lastBuyerOutbound.id)) return;

    const buyerUserId =
      (typeof meta.rfqBuyerUserId === "string" && meta.rfqBuyerUserId) ||
      (
    ...
    if (result.error && !result.demo) {
      logger.warn(
        { conversationId, error: result.error, errorCode: result.errorCode },
        "[delayed-reply] WhatsApp send failed",
      );
      return;
    }
