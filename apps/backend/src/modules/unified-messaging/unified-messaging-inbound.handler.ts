...
export class UnifiedMessagingInboundHandler {
  ...
  private async resolveUnifiedConversation(whatsappConversationId: string) {
    ...
    if (supplierWa) {
      ...
      if (matches.length > 1) {
        const alreadyLinked = matches.find((m) => {
          const meta =
            typeof m.metadata === "object" && m.metadata && !Array.isArray(m.metadata)
        ...
        logger.warn(
          { supplierWa, candidateCount: matches.length, whatsappConversationId },
          "ambiguous RFQ WhatsApp inbound — refusing phone-only match",
        );
      }
    }
  ...
  private async linkWhatsAppConversation(
    rfqConv: { id: string; metadata: unknown },
    whatsappConversationId: string,
  ) {
    const metadata =
      typeof rfqConv.metadata === "object" && rfqConv.metadata && !Array.isArray(rfqConv.metadata)
        ? (rfqConv.metadata as Record<string, unknown>)
        : {};
    if (metadata.whatsappConversationId === whatsappConversationId) return;
  ...
  async mirrorWhatsAppInboxResult(inboxResult: {
    ...
    if (!inboxResult.conversationId || !inboxResult.messageId) return null;
    ...
    try {
      ...
      if (!unifiedConv) {
        logger.debug(
          { whatsappConversationId: inboxResult.conversationId },
          "unified whatsapp inbound mirror — no RFQ conversation link",
        );
        return null;
      }
