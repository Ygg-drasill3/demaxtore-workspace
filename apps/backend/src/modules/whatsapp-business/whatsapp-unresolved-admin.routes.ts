...
router.post(
  "/whatsapp-unresolved-events/:id/resolve",
  asyncHandler(async (req, res) => {
    const body = z.object({ workspaceConversationId: z.string().uuid() }).parse(req.body);
    const result = await service().resolveToConversation(
      req.params.id!,
      body.workspaceConversationId,
      req.user!.id,
    );
    res.json(result);
  }),
...
router.post(
  "/whatsapp-unresolved-events/:id/reprocess",
  asyncHandler(async (req, res) => {
...
router.post(
  "/whatsapp-unresolved-events/:id/ignore",
  asyncHandler(async (req, res) => {
...
router.get(
  "/whatsapp-unresolved-events/:id/audit",
  asyncHandler(async (req, res) => {
