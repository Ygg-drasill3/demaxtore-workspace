import type { PrismaClient, WhatsAppBusinessConnection, WhatsAppBusinessConnectionStatus } from "@prisma/client";
...
import {
  ...
  verifyAccessTokenHealth,
} from "./whatsapp-business-embedded-signup.service.js";
import { logWhatsAppConnectionAudit, maskMetaId, maskPhoneForAdmin } from "./whatsapp-business-audit.service.js";
import { upsertDefaultBuyerTemplate } from "./whatsapp-template.service.js";
import { env } from "../../config/env.js";
import type {
  ConnectWhatsAppBusinessInput,
  HealthCheckResult,
  WhatsAppConnectionAdminDto,
  WhatsAppConnectionBuyerDto,
} from "./whatsapp-business.types.js";

type ConnectionRow = WhatsAppBusinessConnection;

function healthFromStatus(
  status: WhatsAppBusinessConnectionStatus,
  lastErrorMessage?: string | null,
): WhatsAppConnectionBuyerDto["healthStatus"] {
  switch (status) {
    case "CONNECTED":
...
function toBuyerDto(row: ConnectionRow | null): WhatsAppConnectionBuyerDto {
  if (!row) {
    return {
      status: "DISCONNECTED",
      connected: false,
...
function toAdminDto(
  row: ConnectionRow & { buyer?: { email: string; displayName: string } | null },
): WhatsAppConnectionAdminDto {
...
export class WhatsAppBusinessConnectionService {
  ...
  private assertBuyer(actor: AuthUser): string {
    if (actor.role !== "BUYER") {
  ...
  async getConnectionForBuyer(actor: AuthUser): Promise<WhatsAppConnectionBuyerDto> {
  ...
  async listConnectionsForAdmin(): Promise<WhatsAppConnectionAdminDto[]> {
  ...
  async connect(actor: AuthUser, input: ConnectWhatsAppBusinessInput): Promise<WhatsAppConnectionBuyerDto> {
    const buyerId = this.assertBuyer(actor);
  ...
  prepareReconnect(actor: AuthUser): { ready: true; message: string } {
  ...
  async disconnect(actor: AuthUser): Promise<{ ok: true; unsubscribe: { ok: boolean; message: string } }> {
    const buyerId = this.assertBuyer(actor);
  ...
  async testConnection(actor: AuthUser): Promise<HealthCheckResult> {
    const buyerId = this.assertBuyer(actor);
    const row = await this.db.whatsAppBusinessConnection.findUnique({ where: { buyerId } });
  ...
  async markRevoked(buyerId: string): Promise<void> {
    const row = await this.db.whatsAppBusinessConnection.findUnique({ where: { buyerId } });
    if (!row) return;
  ...
  async handleMetaSendError(buyerId: string, errorCode?: string, errorMessage?: string): Promise<void> {
