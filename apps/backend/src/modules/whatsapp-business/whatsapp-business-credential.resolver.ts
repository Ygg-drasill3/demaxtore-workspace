...
import type { WhatsAppTenantCredentials } from "./whatsapp-business.types.js";
import { logWhatsAppConnectionAudit } from "./whatsapp-business-audit.service.js";

const ACTIVE_STATUSES = new Set<WhatsAppBusinessConnectionStatus>(["CONNECTED", "PENDING"]);

export function mapConnectionToCredentials(
  row: {
    id?: string;
    buyerId: string;
...
export async function findConnectionByPhoneNumberId(db: PrismaClient, phoneNumberId: string) {
  return db.whatsAppBusinessConnection.findUnique({ where: { phoneNumberId } });
}

export async function resolveBuyerWhatsAppCredentials(
  db: PrismaClient,
  buyerId: string,
): Promise<WhatsAppTenantCredentials | null> {
  const row = await db.whatsAppBusinessConnection.findUnique({ where: { buyerId } });
  if (!row) return null;

  if (row.status === "EXPIRED" || row.status === "REVOKED") return null;
...
export async function requireBuyerWhatsAppCredentials(
  db: PrismaClient,
  buyerId: string,
): Promise<WhatsAppTenantCredentials> {
  const row = await db.whatsAppBusinessConnection.findUnique({ where: { buyerId } });
...
export async function resolveBuyerIdForConversation(
  db: PrismaClient,
  conversationId: string,
  sender: AuthUser,
): Promise<string | null> {
  if (sender.role === "BUYER") return sender.id;
  ...
  if (!ownerParticipant?.userId) return null;

  const owner = await db.user.findUnique({
    where: { id: ownerParticipant.userId },
    select: { id: true, role: true },
  });
  if (owner?.role === "BUYER") return owner.id;
  return null;
}

export async function assertBuyerCredentialIsolation(
  _db: PrismaClient,
