/**
 * Sprint 36B — Product Master service (tenant-scoped canonical Product).
 */
import type { Prisma, PrismaClient, ProductClassificationSource, ProductClassificationStatus } from "@prisma/client";
import type {
  CreateProductInput,
  ProductDto,
  ProductListQuery,
  UpdateProductInput,
  UpsertProductSupplierReferenceInput,
...
import {
  ...
  canBrowseProducts,
  canManageProducts,
  isProductMasterDeniedRole,
  resolveActorOrganisationId,
} from "./product-master.policy.js";
...
function mapProduct(
  row: {
    ...
    classificationSource: ProductClassificationSource | null;
...
export class ProductMasterService {
  ...
  async create(user: AuthUser, input: CreateProductInput): Promise<ProductDto> {
    this.denyIfPartner(user);
    if (!canManageProducts(user)) throw new AppError(403, "PRODUCT_MASTER_FORBIDDEN");

    const orgId = await resolveActorOrganisationId(this.prisma, user, input.organisationId);
    if (!orgId) throw new AppError(400, "ORGANISATION_REQUIRED");
    ...
    if (existing) throw new AppError(409, "PRODUCT_SKU_EXISTS");
    ...
    const row = await this.prisma.$transaction(async (tx) => {
      const created = await tx.product.create({
        data: {
          ...
          classificationUpdatedById:
            input.gtipCode || input.classificationStatus ? user.id : null,
          createdById: user.id,
          updatedById: user.id,
        },
      });
  ...
  async update(user: AuthUser, id: string, input: UpdateProductInput): Promise<ProductDto> {
    this.denyIfPartner(user);
    if (!canManageProducts(user)) throw new AppError(403, "PRODUCT_MASTER_FORBIDDEN");
    await assertProductAccess(this.prisma, user, id);
    ...
    if (nextSku !== before.sku) {
      ...
      if (clash && clash.id !== id) throw new AppError(409, "PRODUCT_SKU_EXISTS");
    }
    ...
    const row = await this.prisma.$transaction(async (tx) => {
      const updated = await tx.product.update({
        ...
        data: {
          ...
          customsDescription:
            input.customsDescription !== undefined
              ? input.customsDescription?.trim() || null
              : undefined,
          manufacturer:
            input.manufacturer !== undefined ? input.manufacturer?.trim() || null : undefined,
          ...
          dimensionUnit:
            input.dimensionUnit !== undefined ? input.dimensionUnit?.trim() || null : undefined,
          countryOfOrigin:
  ...
  async upsertSupplierReference(
    user: AuthUser,
    productId: string,
    input: UpsertProductSupplierReferenceInput,
  ) {
    this.denyIfPartner(user);
    if (!canManageProducts(user)) throw new AppError(403, "PRODUCT_MASTER_FORBIDDEN");
    ...
    if (!supplier || supplier.role !== "SUPPLIER") throw new AppError(404, "SUPPLIER_NOT_FOUND");
  ...
  async relatedPurchaseOrders(user: AuthUser, productId: string, page = 1, pageSize = 25) {
    this.denyIfPartner(user);
    await assertProductAccess(this.prisma, user, productId);
  ...
  async relatedShipments(user: AuthUser, productId: string, page = 1, pageSize = 25) {
    this.denyIfPartner(user);
    await assertProductAccess(this.prisma, user, productId);
   ...
   */
  async assertProductForBuyerOrg(
    productId: string,
    buyerOrganisationId: string | null,
  ): Promise<void> {
    if (!buyerOrganisationId) throw new AppError(403, "ORGANISATION_REQUIRED");
...
export function createProductMasterService(prisma: PrismaClient) {
