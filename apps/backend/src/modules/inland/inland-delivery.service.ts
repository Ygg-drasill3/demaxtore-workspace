/**
 * Sprint 41 — Turkey Inland Execution V1.
 * After Customs CLEARED → pickup → gate-out → transit → delivered + POD.
 */
import type { InlandDelivery, Prisma, PrismaClient } from "@prisma/client";
import {
  INLAND_CUSTOMS_GATED_STATUSES,
  canTransitionInland,
  type InlandCancelInput,
  type InlandConfirmInput,
  type InlandCostInput,
  type InlandDeliveryDto,
  type InlandDeliveryStatus,
  type RequestInlandDeliveryInput,
...
function sourceFor(actor: AuthUser): string {
  const r = String(actor.role);
  if (r === "TRUCKER") return "TRUCKER";
...
export function createInlandDeliveryService(db: PrismaClient) {
  const issues = new OperationalIssueService(db);
  const tasks = new OperationalTaskService(db);

  async function isCustomsCleared(shipmentWorkspaceId: string): Promise<{
    cleared: boolean;
    ...
    if (cc?.status === "CLEARED") return { cleared: true, customsCaseId: cc.id };
  ...
  async function assertAccess(
    ...
    if (role === "TRUCKER") {
      ...
      if (!a) throw new AppError(403, "PARTNER_NOT_ASSIGNED");
      return "TRUCKER";
    }
    ...
    if (role === "BUYER" || role === "DOCUMENT_CONTROLLER") {
      ...
      if (!ok) throw new AppError(403, "INLAND_FORBIDDEN");
      return "BUYER";
    }
    throw new AppError(403, "INLAND_FORBIDDEN");
  }

  async function enrich(row: InlandDelivery, actor?: AuthUser): Promise<InlandDeliveryDto> {
    const sw = await db.shipmentWorkspace.findUnique({
    ...
    if (access === "BUYER" || access === "OPS") {
      if (["DRAFT", "REQUESTED"].includes(row.status)) allowed.push("ASSIGN_TRUCKER");
      if (row.status === "TRUCKER_ASSIGNED" || row.status === "PICKUP_SCHEDULED") {
        allowed.push("SCHEDULE_PICKUP");
      }
      if (row.status === "PICKUP_SCHEDULED" && customs.cleared) allowed.push("READY_FOR_PICKUP");
      if (access === "OPS" && row.status === "IN_TRANSIT") allowed.push("MARK_DELIVERED");
      if (row.status !== "DELIVERED" && row.status !== "CANCELLED") allowed.push("CANCEL");
      if (access === "OPS" || access === "BUYER") allowed.push("RECORD_COST");
    }
    if (access === "TRUCKER" && (row.status === "TRUCKER_ASSIGNED" || row.status === "PICKUP_SCHEDULED")) {
      allowed.push("SCHEDULE_PICKUP");
    }
    if (access === "TRUCKER" || access === "OPS") {
      if (row.status === "READY_FOR_PICKUP") allowed.push("CONFIRM_PICKUP");
      if (row.status === "PICKED_UP") allowed.push("GATE_OUT");
      if (row.status === "GATE_OUT") allowed.push("IN_TRANSIT");
      if (row.status === "IN_TRANSIT") allowed.push("MARK_DELIVERED");
    }
    if (
      (access === "TRUCKER" || access === "OPS" || access === "BUYER")
      && row.status === "DELIVERED"
    ...
    return {
      ...
      status: row.status as InlandDeliveryStatus,
      statusSource: row.statusSource as InlandDeliveryDto["statusSource"],
      ...
      podStatus: row.podStatus as InlandDeliveryDto["podStatus"],
      podTradeDocumentId: row.podTradeDocumentId,
      inlandCostAmount:
        showCost && row.inlandCostAmount != null ? Number(row.inlandCostAmount) : null,
  ...
  async function transition(
  ...
  ): Promise<InlandDelivery> {
    const from = row.status as InlandDeliveryStatus;
    ...
    if (INLAND_CUSTOMS_GATED_STATUSES.includes(to)) {
      ...
      if (!cleared) throw new AppError(409, "CUSTOMS_NOT_CLEARED");
    }
  ...
  async function syncExceptions(row: InlandDelivery) {
    const { cleared } = await isCustomsCleared(row.shipmentWorkspaceId);
    ...
    const resolve = async (automationKey: string) => {
  ...
  async function syncTruckerFromAssignment(row: InlandDelivery) {
    const a = await db.partnerAssignment.findFirst({
      where: {
        workspaceId: row.shipmentWorkspaceId,
        partnerRole: "TRUCKER",
  ...
  return {
    async request(actor: AuthUser, input: RequestInlandDeliveryInput): Promise<InlandDeliveryDto> {
      const role = String(actor.role);
      if (
        !["BUYER", "ADMIN", "SUPER_ADMIN", "OPS_MANAGER", "LOGISTICS_OPERATOR"].includes(role)
      ) {
        throw new AppError(403, "INLAND_FORBIDDEN");
      }
      const sw = await db.shipmentWorkspace.findUnique({
    ...
    async get(actor: AuthUser, id: string): Promise<InlandDeliveryDto> {
      const row = await db.inlandDelivery.findUnique({ where: { id } });
      if (!row) throw new AppError(404, "INLAND_DELIVERY_NOT_FOUND");
      await assertAccess(actor, row);
      return enrich(row, actor);
    },

    async getByShipment(actor: AuthUser, shipmentWorkspaceId: string) {
    ...
    async list(actor: AuthUser, query: { attention?: boolean; status?: string } = {}) {
      const role = String(actor.role);
      if (role === "TRUCKER") throw new AppError(403, "INLAND_FORBIDDEN");
    ...
    async syncTrucker(actor: AuthUser, id: string): Promise<InlandDeliveryDto> {
      const row = await db.inlandDelivery.findUnique({ where: { id } });
      if (!row) throw new AppError(404, "INLAND_DELIVERY_NOT_FOUND");
      await assertAccess(actor, row);
    ...
    async readyForPickup(actor: AuthUser, id: string): Promise<InlandDeliveryDto> {
      const row = await db.inlandDelivery.findUnique({ where: { id } });
      if (!row) throw new AppError(404, "INLAND_DELIVERY_NOT_FOUND");
      await assertAccess(actor, row);
    ...
    async confirmPickup(
      actor: AuthUser,
      id: string,
      input: InlandConfirmInput = {},
    ): Promise<InlandDeliveryDto> {
      const row = await db.inlandDelivery.findUnique({ where: { id } });
      if (!row) throw new AppError(404, "INLAND_DELIVERY_NOT_FOUND");
      const access = await assertAccess(actor, row);
      if (access !== "TRUCKER" && access !== "OPS") throw new AppError(403, "INLAND_FORBIDDEN");
      if (row.status === "PICKED_UP") return enrich(row, actor);
    ...
    async gateOut(
      ...
      const access = await assertAccess(actor, row);
      if (access !== "TRUCKER" && access !== "OPS") throw new AppError(403, "INLAND_FORBIDDEN");
      if (row.status === "GATE_OUT") return enrich(row, actor);
      const ts = input.timestamp ? new Date(input.timestamp) : new Date();
    ...
    async inTransit(
      ...
      const access = await assertAccess(actor, row);
      if (access !== "TRUCKER" && access !== "OPS") throw new AppError(403, "INLAND_FORBIDDEN");
      if (row.status === "IN_TRANSIT") return enrich(row, actor);
      const ts = input.timestamp ? new Date(input.timestamp) : new Date();
    ...
    async markDelivered(
      ...
      const access = await assertAccess(actor, row);
      if (access !== "TRUCKER" && access !== "OPS") throw new AppError(403, "INLAND_FORBIDDEN");
      if (row.status === "DELIVERED") return enrich(row, actor);
    ...
    async linkPod(actor: AuthUser, id: string, tradeDocumentId: string) {
      const row = await db.inlandDelivery.findUnique({ where: { id } });
      if (!row) throw new AppError(404, "INLAND_DELIVERY_NOT_FOUND");
      await assertAccess(actor, row);
      ...
      if (
        !(
          (doc.workspaceType === "SHIPMENT" && doc.workspaceId === row.shipmentWorkspaceId)
    ...
    async recordCost(actor: AuthUser, id: string, input: InlandCostInput) {
      const row = await db.inlandDelivery.findUnique({ where: { id } });
      if (!row) throw new AppError(404, "INLAND_DELIVERY_NOT_FOUND");
      const access = await assertAccess(actor, row);
      if (access === "TRUCKER") throw new AppError(403, "INLAND_COST_FORBIDDEN");
      const updated = await db.inlandDelivery.update({
        where: { id },
        data: {
    ...
    async cancel(actor: AuthUser, id: string, input: InlandCancelInput) {
      const row = await db.inlandDelivery.findUnique({ where: { id } });
      if (!row) throw new AppError(404, "INLAND_DELIVERY_NOT_FOUND");
      await assertAccess(actor, row);
      ...
      if (row.status === "CANCELLED") return enrich(row, actor);
...
export type InlandDeliveryService = ReturnType<typeof createInlandDeliveryService>;
