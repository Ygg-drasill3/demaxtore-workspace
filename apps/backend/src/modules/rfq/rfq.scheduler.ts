...
async function processRfqDeadlines(): Promise<void> {
  const now = new Date();
...
async function processProformaSlaExpiry(): Promise<void> {
  const now = new Date();
  const due = await prisma.workspace.findMany({
    where: {
      type: "RFQ",
...
async function tick(): Promise<void> {
  ...
  if (!ran) await recordSkippedJob(prisma, "rfq_deadline_scheduler", "lock_held");
}

export async function runRfqSchedulerTick(): Promise<void> {
  await tick();
}

export function startRfqScheduler(): NodeJS.Timeout {
