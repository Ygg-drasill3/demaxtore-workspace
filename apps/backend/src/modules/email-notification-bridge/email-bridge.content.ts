import type { NotificationMetadata } from "../notification-engine/notification-engine.mapper.js";
import type { ResolvedOperationalShape } from "../notification-engine/notification-engine.mapper.js";
...
export function resolveEmailSafeMessage(input: {
  ...
  if (input.metadata.sensitiveContent === true) return GENERIC_SUMMARY;
  if (input.metadata.messageVisibility && input.metadata.messageVisibility !== "ALL_PARTICIPANTS") {
    return GENERIC_SUMMARY;
  }
  if (input.eventType && SENSITIVE_EVENT_TYPES.has(input.eventType)) return GENERIC_SUMMARY;
  if (
    SENSITIVE_CENTER_TYPES.has(input.shape.centerType)
