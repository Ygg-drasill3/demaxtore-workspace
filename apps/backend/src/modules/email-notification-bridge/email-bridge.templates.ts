...
const PRIORITY_STYLES: Record<NotificationPriority, { bg: string; fg: string; label: string }> = {
...
export interface EmailBridgeTemplateContext {
...
export function buildOperationalEmailTemplate(ctx: EmailBridgeTemplateContext): {
  subject: string;
  html: string;
  text: string;
  templateKey: string;
...
export function buildOpenTrackingUrl(deliveryId: string): string | undefined {
  if (env.EMAIL_BRIDGE_OPEN_TRACKING_ENABLED === false) return undefined;
