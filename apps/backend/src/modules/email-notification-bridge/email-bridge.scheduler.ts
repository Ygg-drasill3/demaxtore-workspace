...
export async function runEmailBridgeRetryTick(): Promise<void> {
  ...
  if (!ran) await recordSkippedJob(prisma, "email_bridge_retry", "lock_held");
}

export function startEmailBridgeRetryWorker(): NodeJS.Timeout {
  const tick = () => {
