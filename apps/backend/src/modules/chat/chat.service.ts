...
import type { ChatContextType } from "./chat.types.js";
...
export class TradeChatService {
  private link: ConversationLinkService;

  constructor(private readonly db: PrismaClient) {
    this.link = new ConversationLinkService(db);
  }

  private async assertRfqAccess(actor: AuthUser, rfqWorkspaceId: string) {
    if (!(await canAccessCommWorkspace(this.db, actor, "RFQ", rfqWorkspaceId))) {
      throw new AppError(403, "FORBIDDEN");
    }
  }

  private async assertOrderAccess(actor: AuthUser, orderWorkspaceId: string) {
    if (!(await canAccessCommWorkspace(this.db, actor, "ORDER", orderWorkspaceId))) {
      throw new AppError(403, "FORBIDDEN");
    }
  }

  private canAccessConv(
    conv: { buyerUserId: string; peerUserId: string | null },
    userId: string,
    role: string,
  ) {
    if (isAdminChatRole(role)) return true;
  ...
  private emitMessageEvent(
    ...
    message: ReturnType<typeof mapMessageRow>,
  ) {
    ...
    socketBus.scheduleEmit(() => {
      ...
      if (conv.peerUserId) socketBus.emitToUser(conv.peerUserId, "chat:message:new", payload);
      socketBus.emitToRole("ADMIN", "chat:message:new", payload);
    });
  }

  private async lastMessageMeta(conversationId: string) {
  ...
  async listForWorkspace(contextType: ChatContextType, contextWorkspaceId: string, actor: AuthUser) {
    if (contextType === "RFQ") await this.assertRfqAccess(actor, contextWorkspaceId);
    else await this.assertOrderAccess(actor, contextWorkspaceId);
  ...
  async listByRfqLink(opts: {
    ...
    if (!isAdminChatRole(opts.actor.role)) {
      const allowed = rows.filter(
        (c) => c.buyerUserId === opts.actor.id || c.peerUserId === opts.actor.id,
      );
      if (!allowed.length) throw new AppError(403, "FORBIDDEN");
  ...
  async ensureRfqConversations(rfqWorkspaceId: string, actor: AuthUser, freightToken?: string) {
    ...
    const freightIqRfqId = await this.link.syncFreightIqRfqId(
      rfqWorkspaceId,
      actor.id,
      freightToken,
      actor.role,
    ...
    for (const a of assignments) {
      ...
      const data = {
        ...
        peerPhone: phone,
        whatsappPhone: phone,
        forwarderPhone: null as string | null,
        status: "active",
      };
  ...
  async ensureOrderFreightConversation(
    ...
    if (!ws?.orderWorkspace) throw new AppError(404, "ORDER_NOT_FOUND");

    const forwarder = await this.db.forwarderContact.findUnique({
      where: { id: forwarderContactId },
    });
    if (!forwarder) throw new AppError(404, "FORWARDER_NOT_FOUND");

    const workspaceRfqId = ws.spawnedFrom?.id ?? null;
    let freightIqRfqId: string | null = null;
    if (workspaceRfqId) {
  ...
  async syncOrderFreightFromCommunications(orderWorkspaceId: string, actor: AuthUser) {
    await this.assertOrderAccess(actor, orderWorkspaceId);
    ...
    for (const comm of req.communications) {
      if (!comm.forwarderContact) continue;
      convs.push(
        await this.ensureOrderFreightConversation(orderWorkspaceId, comm.forwarderContactId, actor),
      );
    }
    return convs;
  }

  async getConversation(conversationId: string, userId: string, role: string) {
  ...
  async sendMessage(conversationId: string, actor: AuthUser, body: string) {
    ...
    const targetPhone =
      actor.id === conv.buyerUserId || isAdminChatRole(actor.role)
        ? counterpartyPhone
        : conv.peerUserId
          ? (
    ...
    let source: "platform" | "whatsapp" = "platform";
    ...
    if (normalized) {
      ...
      const wa = await sendTextMessage(
        normalized,
        `${prefix}${sender?.displayName ?? actor.email.split("@")[0] ?? "User"}: ${text}`,
      );
      if (wa.id) {
        source = "whatsapp";
  ...
  async ingestInbound(fromPhone: string, messageText: string, whatsappMessageId: string | null) {
    ...
    const phoneMatch = (c: (typeof conversations)[0]) =>
      normalizePhone(c.whatsappPhone) === normalized ||
      normalizePhone(c.forwarderPhone) === normalized ||
      normalizePhone(c.peerPhone) === normalized;

    const matchedConv = conversations.find((c) => {
      if (phoneMatch(c)) return true;
      if (matchedUser && c.peerUserId === matchedUser.id) return true;
      if (matchedUser && c.buyerUserId === matchedUser.id) return true;
      return false;
    });
    if (!matchedConv) {
    ...
    let senderType: "supplier" | "forwarder" | "buyer" = "supplier";
    if (matchedUser) {
