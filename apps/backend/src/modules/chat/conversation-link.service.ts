...
export class ConversationLinkService {
  ...
  async findFreightIqRfqId(workspaceRfqId: string): Promise<string | null> {
  ...
  async findByFreightIqRfqId(freightIqRfqId: string) {
  ...
  async linkFreightIqRfqId(workspaceRfqId: string, freightIqRfqId: string) {
  ...
  async syncFreightIqRfqId(
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logger.warn(
          { workspaceRfqId, status: res.status, body: body.slice(0, 300) },
          "[Chat] FreightIQ RFQ sync failed — conversations remain workspace-scoped",
        );
        return null;
      }
      const data = (await res.json()) as { rfqId?: string };
      if (!data.rfqId) return null;
  ...
  }): Promise<{ workspaceRfqId: string | null; freightIqRfqId: string | null }> {
    if (opts.workspaceRfqId) {
      const freightIqRfqId =
        opts.freightIqRfqId ?? (await this.findFreightIqRfqId(opts.workspaceRfqId));
