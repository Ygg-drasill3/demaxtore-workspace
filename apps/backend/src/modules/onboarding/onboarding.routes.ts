...
onboardingRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const steps = TOUR_STEPS_BY_ROLE[role as keyof typeof TOUR_STEPS_BY_ROLE] ?? [];
    res.json({ steps });
  }),
);
...
onboardingRouter.get(
  "/guidance/:workspaceType/:workspaceId",
  asyncHandler(async (req, res) => guidanceHandler(req, res, prisma)),
);
