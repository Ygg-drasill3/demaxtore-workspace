...
export async function getWorkspaceGuidance(
  db: PrismaClient,
  workspaceType: string,
  workspaceId: string,
  userId: string,
  userRole: string,
): Promise<WorkspaceGuidanceDTO> {
  ...
  if (workspaceType === "rfq" || ws.type === "RFQ") {
    ...
    const actions = computeRfqNextActions({
      state: ws.state as never,
      actorRole: userRole as never,
      isOwner,
      isCounterparty,
...
export async function guidanceHandler(req: Request, res: Response, db: PrismaClient) {
