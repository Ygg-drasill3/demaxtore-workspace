...
import type { OnboardingRole } from "@dmx/contracts/onboarding";
import type { OnboardingJourneyContext } from "@dmx/contracts/onboarding";
...
export async function buildJourneyContext(
  ...
  userId: string,
  role: OnboardingRole,
): Promise<OnboardingJourneyContext> {
  if (role === "BUYER") return buildBuyerContext(db, userId);
  if (role === "SUPPLIER") return buildSupplierContext(db, userId);
  return buildOperatorContext(db, userId, role);
}

async function buildBuyerContext(db: PrismaClient, userId: string): Promise<OnboardingJourneyContext> {
...
async function buildSupplierContext(db: PrismaClient, userId: string): Promise<OnboardingJourneyContext> {
...
async function buildOperatorContext(
  db: PrismaClient,
  userId: string,
  role: Extract<OnboardingRole, "ADMIN" | "SALES_CONTROL"> = "ADMIN",
): Promise<OnboardingJourneyContext> {
...
export async function buildTradeMilestoneSignals(
  db: PrismaClient,
  userId: string,
  role: OnboardingRole,
  existingCtx?: OnboardingJourneyContext,
) {
