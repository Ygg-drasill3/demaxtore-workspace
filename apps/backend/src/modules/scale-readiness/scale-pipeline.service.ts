...
import type { PipelineHealthItem, PipelineHealthSummary } from "@dmx/contracts/scale-readiness";
...
export class ScalePipelineService {
  ...
  async getPipelineHealth(): Promise<PipelineHealthSummary> {
    ...
    const alertsByWs = new Map<string, string[]>();
    ...
    const avg =
      items.length ? items.reduce((s, i) => s + i.healthScore, 0) / items.length : 100;
  ...
  private async auditHealthRefresh(count: number) {
