...
import type { AccountOwner } from "@dmx/contracts/scale-readiness";
import { AssignAccountOwnershipPayload } from "@dmx/contracts/scale-readiness.zod";
import { AppError } from "../../utils/httpErrors.js";
import type { AuthUser } from "../freightiq/freightiq.policy.js";

export class ScaleAccountService {
  constructor(private readonly db: PrismaClient) {}

  async mapOwner(orgId: string, orgName: string): Promise<AccountOwner | null> {
  ...
  async assignOwnership(
    ...
    if (!org) throw new AppError(404, "ORG_NOT_FOUND");

    if (input.operationsUserId) await this.assertAdminUser(input.operationsUserId);
    if (input.salesUserId) await this.assertAdminUser(input.salesUserId);
  ...
  private async assertAdminUser(userId: string) {
    ...
    if (!u || u.role !== "ADMIN") throw new AppError(400, "INVALID_ACCOUNT_OWNER");
  }
}
