...
scaleRouter.get(
  "/portfolio/buyers/:organisationId",
  asyncHandler(async (req, res) => {
    const row = await portfolio.getBuyerHealth(req.params.organisationId);
    if (!row) throw new AppError(404, "BUYER_NOT_FOUND");
    res.json(row);
  }),
);
...
scaleRouter.get(
  "/portfolio/suppliers/:organisationId",
  asyncHandler(async (req, res) => {
    const row = await portfolio.getSupplierHealth(req.params.organisationId);
    if (!row) throw new AppError(404, "SUPPLIER_NOT_FOUND");
    res.json(row);
  }),
);

scaleRouter.post(
  "/accounts/:organisationId/assign",
  asyncHandler(async (req, res) => {
    const body = (req.body ?? {}) as { payload?: Record<string, unknown> };
    res.json(
      await accounts.assignOwnership(
...
scaleRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await forecast.getForecast(q.days as 30 | 60 | 90));
  }),
);
