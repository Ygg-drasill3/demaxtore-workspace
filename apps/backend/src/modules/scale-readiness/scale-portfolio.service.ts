...
import type { AccountActivity, BuyerHealth, SupplierHealth } from "@dmx/contracts/scale-readiness";
import { ScaleAccountService } from "./scale-account.service.js";

function activityFrom(date: Date | null, eventType?: string | null): AccountActivity {
  if (!date) {
...
function commercialScore(params: {
  ...
  if (!isUnknownActivityDays(params.daysSince)) {
    if (params.daysSince <= 7) score += 15;
    else if (params.daysSince <= 30) score += 5;
...
export class ScalePortfolioService {
  private readonly accounts: ScaleAccountService;

  constructor(private readonly db: PrismaClient) {
    this.accounts = new ScaleAccountService(db);
  }

  async listBuyerHealth(): Promise<BuyerHealth[]> {
    ...
    for (const [orgId, users] of byOrg) {
      ...
      const lastWs = [...rfqs, ...orders, ...shipments].sort(
  ...
  async getBuyerHealth(organisationId: string): Promise<BuyerHealth | null> {
    const list = await this.listBuyerHealth();
    return list.find((b) => b.organisationId === organisationId) ?? null;
  }

  async listSupplierHealth(): Promise<SupplierHealth[]> {
  ...
  async getSupplierHealth(organisationId: string): Promise<SupplierHealth | null> {
