...
async function nextBcExecRef(tx: Prisma.TransactionClient): Promise<string> {
...
async function resolveSupplierUserId(tx: Prisma.TransactionClient, supplierCode: string): Promise<string> {
...
export class BulkContainerExecutionService {
  ...
  async spawnExecutionOrders(id: string, actor: AuthUser) {
    ...
    const incomplete = ws.bulkContainerLines.filter(
      (l) => !lineFullyAllocated(l.id, Number(l.quantityMt), ws.bcSupplierAllocations),
    );
    if (incomplete.length > 0) throw new AppError(409, "LINES_NOT_ALLOCATED", { count: incomplete.length });
    if (ws.bcSupplierAllocations.length === 0) throw new AppError(409, "NO_ALLOCATIONS");

    const allHaveProformas = ws.bcSupplierAllocations.every((a) => a.proformas.length > 0);
    if (!allHaveProformas) throw new AppError(409, "PROFORMAS_MISSING");

    const allPaid = ws.bcSupplierAllocations.every((a) =>
      a.payments.some((p) => p.status === "PAYMENT_CONFIRMED"),
    );
      ...
      return {
        ...
        supplierOrders: ws.bcOrderLinks.map((l) => ({
          allocationRef: allocationRef(
            ws.bcSupplierAllocations.find((a) => a.id === l.allocationId)!.sortOrder,
          ),
  ...
  async tryMarkExecutionComplete(bulkContainerId: string) {
    const ws = await this.prisma.workspace.findUnique({
      where: { id: bulkContainerId },
      include: { bcOrderLinks: true },
    });
    if (!ws || ws.state !== "BC_EXECUTION_ACTIVE" || ws.bcOrderLinks.length === 0) return;
  ...
  async recordFreightStarted(bulkContainerId: string, orderId: string) {
  ...
  async recordShipmentStarted(bulkContainerId: string, orderId: string, shipmentId: string) {
    const link = await this.prisma.bcOrderLink.findFirst({
      where: { workspaceId: bulkContainerId, supplierOrderId: orderId },
    });
    if (!link) return;
