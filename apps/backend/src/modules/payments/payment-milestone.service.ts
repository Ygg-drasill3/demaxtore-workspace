...
export function isPaymentGatesEnabled(): boolean {
...
export class PaymentMilestoneService {
  ...
  async ensurePlan(orderId: string, currency = "USD") {
  ...
  async getPlanDto(orderId: string): Promise<PaymentPlanDto> {
    ...
    return {
      orderId,
      status: full.status as PaymentPlanDto["status"],
      financialStatus: disputed ? "DISPUTED" : overdue ? "OVERDUE" : hasHold ? "HOLD" : "CLEAR",
      holds: full.holds.map((h) => ({ reason: h.reason as never, active: h.active })),
      milestones: full.milestones.map((m) => ({
        id: m.id,
        kind: m.kind as PaymentMilestoneKind,
  ...
  async satisfyMilestone(orderId: string, kind: PaymentMilestoneKind, externalEventId?: string) {
    const plan = await this.ensurePlan(orderId);
    if (externalEventId) {
   ...
   */
  async recordDispute(orderId: string, externalEventId?: string): Promise<void> {
    const plan = await this.ensurePlan(orderId);
  ...
  async assertOrderActionAllowed(action: string, orderId: string): Promise<void> {
    if (!isPaymentGatesEnabled()) return;
    ...
    if (!plan) throw new AppError(409, "PAYMENT_PLAN_REQUIRED");
    if (plan.holds.some((h) => h.active)) throw new AppError(409, "PAYMENT_HOLD_ACTIVE");
