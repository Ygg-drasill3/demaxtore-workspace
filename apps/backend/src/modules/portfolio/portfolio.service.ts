...
import type { ListPortfolioQuery } from "@dmx/contracts/portfolio.zod";
...
type ParticipantFilter = Prisma.WorkspaceWhereInput;

function workspaceAccessFilter(actor: AuthUser): ParticipantFilter {
...
export class PortfolioService {
  ...
  async listPurchaseOrders(actor: AuthUser, query: ListPortfolioQuery) {
    const access = workspaceAccessFilter(actor);
    const orderWhere: Prisma.WorkspaceWhereInput = {
      type: "ORDER",
      ...access,
    };
    ...
    const poWhere: Prisma.PurchaseOrderWhereInput = { orderId: { in: ids } };
  ...
  async listShipments(actor: AuthUser, query: ListPortfolioQuery) {
    ...
    const parentIds = rows.map((r) => r.spawnedFromId).filter((id): id is string => !!id);
  ...
  async listTradeDocuments(actor: AuthUser, query: ListPortfolioQuery) {
    const orderWhere: Prisma.WorkspaceWhereInput = {
      type: "ORDER",
  ...
  async listMessages(actor: AuthUser, query: ListPortfolioQuery) {
    const types = actor.role === "SUPPLIER"
      ? (["RFQ", "ORDER", "COMMODITYBID"] as const)
      : (["RFQ", "ORDER", "COMMODITYBID"] as const);
    ...
    const convByKey = new Map(
      convs.map((c) => [`${c.workspaceType}:${c.workspaceId}`, c]),
    );
