...
async function loadAuthUser(userId: string): Promise<AuthUser> {
  ...
  if (!row) throw new AppError(401, "USER_NOT_FOUND");
...
export const freightiqDemoVesselOffer: RequestHandler[] = [
  embedCors,
  asyncHandler(async (req, res) => {
    ...
    if (!summary.request) {
      ...
      summary = await freightIq.applyFreightAction(
        order.id,
        "create_request",
        authUser,
        {
    ...
    const result = await comms.intakeOffer(
      order.id,
      authUser,
      {
        forwarderContactId,
