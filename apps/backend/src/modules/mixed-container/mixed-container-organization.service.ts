...
export class MixedContainerOrganizationService {
  constructor(
    ...
    private readonly execution = new MixedContainerExecutionService(prisma),
  ) {}
  ...
  async createOrganization(
    ...
    if (details.organizationRef) return details.organizationRef;

    const organizationRef = await nextOrRef(tx);
    const status: OrganizationStatus = "ORGANIZATION_STARTED";
    const now = new Date();
  ...
  async getOrganization(workspaceId: string, actor: AuthUser, includeInternalNotes = false) {
    ...
    let opsManagerName: string | null = null;
    ...
    const modules = [
      ...
      {
        key: "PROFORMA_INVOICES" as const,
        label: "Proforma Invoices",
        status:
          ws.mcSupplierProformas.length === 0
            ? "Pending"
      ...
      {
        key: "SHIPMENT_TRACKING" as const,
    ...
    const outstandingTasks: Array<{
      id: string;
      title: string;
      moduleKey: (typeof modules)[number]["key"] | null;
    ...
    const orgStatus = d.organizationStatus as OrganizationStatus;
    const upcomingMilestones = ORGANIZATION_STATUSES.filter(
      (s) => !isOrganizationStepComplete(s, orgStatus) && s !== orgStatus,
    )
      .slice(0, 5)
      .map((key) => ({
        key,
    ...
    const synchronizationStatus =
      orgEvents.length > 0 && lastSyncedAt && Date.now() - new Date(lastSyncedAt).getTime() < 120_000
        ? ("LIVE" as const)
        : orgEvents.length > 0
          ? ("SYNCED" as const)
          : ("PENDING" as const);
    ...
    let internalNotes: Array<{ id: string; authorName: string; body: string; createdAt: string }> | undefined;
    if (includeInternalNotes && actor.role === "ADMIN" && "mcInternalNotes" in ws) {
      const notes = ws.mcInternalNotes as Array<{ id: string; authorId: string; body: string; createdAt: Date }>;
      const noteAuthors =
        notes.length > 0
  ...
  async updateStatus(workspaceId: string, input: UpdateOrganizationStatusInput, actor: AuthUser) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (!ws.mixedContainerDetails?.organizationRef) throw new AppError(404, "ORGANIZATION_NOT_FOUND");

      const fromStatus = (ws.mixedContainerDetails.organizationStatus ?? "ORGANIZATION_STARTED") as OrganizationStatus;
  ...
  async assignOperationsManager(workspaceId: string, managerId: string, actor: AuthUser) {
    await assertCanManageOrganization(this.prisma, actor, workspaceId);
