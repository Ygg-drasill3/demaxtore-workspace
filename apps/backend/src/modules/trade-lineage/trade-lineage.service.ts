/**
 * Sprint 31 — Trade Entity Lineage service.
 * Header links (TradeShipmentLink) + line qty (ShipmentLineAllocation).
 */
import type { Prisma, PrismaClient } from "@prisma/client";
import type { UpsertShipmentLineAllocationInput } from "@dmx/contracts/trade-lineage";
...
import { canAccessShipment, type AuthUser } from "../shipment/shipment.policy.js";

function canMutateLineAllocation(role: string): boolean {
  return (
...
export class TradeLineageService {
   ...
   */
  async discoverAndLinkForOrder(orderWorkspaceId: string, source: string = "DISCOVERED") {
  ...
  async linkPoToShipment(user: AuthUser, purchaseOrderId: string, shipmentWorkspaceId: string) {
    ...
    if (!sw) throw NotFound("Shipment not found");
    if (
      sw.orderWorkspaceId !== po.orderId &&
      user.role !== "ADMIN" &&
  ...
  async upsertAllocation(user: AuthUser, input: UpsertShipmentLineAllocationInput) {
    ...
    if (!line) throw NotFound("PO line not found");
    ...
    if (!sw) throw NotFound("Shipment not found");
    let containerDbId: string | null = null;
    if (input.shipmentContainerId) {
      ...
      if (!c) throw Validation("Container does not belong to shipment");
      containerDbId = c.id;
    }
    ...
    const exact = existing.find(
      (a) =>
        a.shipmentWorkspaceId === input.shipmentWorkspaceId &&
    ...
    const uncontaineredOnShipment =
      containerDbId == null
        ? undefined
        : existing.find(
            (a) => a.shipmentWorkspaceId === input.shipmentWorkspaceId && a.shipmentContainerId == null,
          );
    const match = exact ?? uncontaineredOnShipment;
    const otherSum = await this.sumActiveAllocated(
      this.prisma,
      existing.filter((a) => a.id !== match?.id),
    );
   ...
   */
  async allocateRemainingLinesForShipment(
    db: DbClient,
    input: {
  ...
  ): Promise<{ created: number; skipped: number; allocated: Array<{ lineId: string; quantity: number }> }> {
    ...
    let created = 0;
    let skipped = 0;
    const allocated: Array<{ lineId: string; quantity: number }> = [];
    ...
    for (const line of po.lines) {
      ...
      const otherSum = await this.sumActiveAllocated(
        db,
        existing.filter((a) => a.shipmentWorkspaceId !== input.shipmentWorkspaceId),
      );
      const ordered = dec(line.quantity);
  ...
  async backfillRemainingAllocations(user: AuthUser, shipmentWorkspaceId: string) {
    if (!(await canAccessShipment(this.prisma, user, shipmentWorkspaceId))) {
      throw Forbidden("Cannot access shipment");
    }
    const sw = await this.prisma.shipmentWorkspace.findUnique({
  ...
  private async sumActiveAllocated(
    db: DbClient,
    rows: Array<{ quantity: unknown; shipmentWorkspaceId: string }>,
  ): Promise<number> {
    ...
    const inactive = new Set(
      workspaces.filter((w) => INACTIVE_SHIPMENT_STATES.has(w.state)).map((w) => w.id),
    );
    return rows
  ...
  async relatedForPurchaseOrder(user: AuthUser, poId: string) {
    ...
    const allowedShipmentIds: string[] = [];
    for (const sid of shipmentIds) {
      if (await canAccessShipment(this.prisma, user, sid)) allowedShipmentIds.push(sid);
    }
  ...
  async relatedForShipment(user: AuthUser, shipmentWorkspaceId: string) {
    if (!(await canAccessShipment(this.prisma, user, shipmentWorkspaceId))) {
      throw Forbidden("Cannot access shipment");
    }
    const sw = await this.prisma.shipmentWorkspace.findUnique({
    ...
    if (primaryPo) poIds.add(primaryPo.id);
    const allowedPoIds: string[] = [];
    for (const id of poIds) {
      if (await canAccessPo(this.prisma, user, id)) allowedPoIds.push(id);
    }
    return this.buildDto({
  ...
  async relatedForContainer(user: AuthUser, shipmentWorkspaceId: string, containerId: string) {
    if (!(await canAccessShipment(this.prisma, user, shipmentWorkspaceId))) {
      throw Forbidden("Cannot access shipment");
    }
    const sw = await this.prisma.shipmentWorkspace.findUnique({
    ...
    return {
      ...
      poLines: base.poLines.filter((l) =>
        base.allocations.some(
          (a) => a.purchaseOrderLineId === l.id && a.shipmentContainerId === containerId,
        ),
      ),
    };
  }

  private async buildDto(opts: { purchaseOrderIds: string[]; shipmentWorkspaceIds: string[] }) {
    ...
    const allocations =
      purchaseOrderIds.length === 0 && shipmentWorkspaceIds.length === 0
        ? []
            ...
            where: {
              OR: [
                ...
                shipmentWorkspaceIds.length
                  ...
                  : undefined,
              ].filter(Boolean) as Prisma.ShipmentLineAllocationWhereInput[],
            },
            select: {
              id: true,
    ...
    const containers = shipmentRows.flatMap((s) =>
      s.containers.map((c) => ({
        id: c.id,
        shipmentWorkspaceId: s.workspaceId,
  ...
  private async buildSourceContext(
    purchaseOrders: Array<{ id: string; orderId: string; source: string | null }>,
  ) {
    ...
    let rfqWorkspaceId: string | null = null;
    let rfqExternalRef: string | null = null;
    let commodityBidWorkspaceId: string | null = null;
    let commodityBidExternalRef: string | null = null;
...
export function createTradeLineageService(prisma: PrismaClient) {
