...
async function shouldDeliverWhatsApp(userId: string, centerType: string): Promise<boolean> {
...
export async function processWhatsAppBridgeDelivery(notificationId: string): Promise<void> {
  ...
  if (!isWhatsAppDeliverableType(shape.centerType)) return;
  ...
  await prisma.whatsAppNotificationDelivery.update({
    ...
    data: {
      ...
      providerResponse: (result.raw ?? { error: result.error }) as Prisma.InputJsonValue,
    },
  });
  logger.warn(
    { notificationId, error: result.error, retryCount },
    "[WA Bridge] delivery failed — notification remains in Workspace",
  );
}

export async function retryFailedWhatsAppDeliveries(): Promise<number> {
  const now = new Date();
...
export async function updateDeliveryStatusFromWebhook(
  ...
  const now = new Date();
  const data: Prisma.WhatsAppNotificationDeliveryUpdateInput = {
    providerResponse: { ...(row.providerResponse as object), lastWebhook: raw ?? {} } as Prisma.InputJsonValue,
  };
...
export async function getDeliveryForNotification(notificationId: string) {
...
export async function loadNotificationDto(notificationId: string) {
