...
export async function runWhatsAppBridgeRetryTick(): Promise<void> {
  ...
  if (!ran) await recordSkippedJob(prisma, "whatsapp_bridge_retry", "lock_held");
}

export function startWhatsAppBridgeRetryWorker(): NodeJS.Timeout {
  const tick = () => {
