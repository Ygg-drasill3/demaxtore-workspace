...
import type { WhatsAppProvider, WhatsAppSendResult, WhatsAppTemplateMessage } from "./whatsapp-provider.types.js";
...
export class MetaCloudWhatsAppProvider implements WhatsAppProvider {
  ...
  async sendTemplateMessage(msg: WhatsAppTemplateMessage): Promise<WhatsAppSendResult> {
    ...
    if (!token || !phoneNumberId) {
      logger.info(
        { to: normalized, preview: msg.bodyText.slice(0, 100), url: msg.buttonUrl },
        "[WA Bridge] demo send",
      );
      return {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        return { providerMessageId: null, demo: false, error: err, raw: data as Record<string, unknown> };
      }
      return {
