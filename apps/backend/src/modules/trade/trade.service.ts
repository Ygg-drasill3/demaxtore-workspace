...
import type {
  ...
  TradeDocumentItem,
  TradeFreightPanelItem,
  TradeExecutionMilestone,
  TradeOrderPanelItem,
  TradePoPanelItem,
  TradeRelatedRecord,
  TradeShipmentPanelItem,
  TradeSummaryPanel,
  TradeTimelineItem,
  TradeType,
  TradeWorkspaceHeader,
  TradeWorkspacePayload,
} from "@dmx/contracts/trade-workspace";
import { AppError } from "../../utils/httpErrors.js";
import { canAccessTrade } from "./trade.policy.js";
import type { AuthUser } from "./trade.policy.js";
import {
  collectTradeGraph,
  resolveTradeRoot,
  tradeRefFromRoot,
} from "./trade.resolver.js";
...
const DOC_CATEGORY_MAP: Record<string, TradeDocumentCategory> = {
  PROFORMA: "Proforma",
...
function mapDocCategory(documentType: string): TradeDocumentCategory {
  return DOC_CATEGORY_MAP[documentType] ?? "Other";
}

function workspaceUrl(type: string, id: string): string {
  if (type === "RFQ") return `/workspace/rfq/${id}`;
  if (type === "COMMODITYBID") return `/workspace/commoditybid/${id}`;
  if (type === "ORDER") return `/workspace/order/${id}`;
  if (type === "SHIPMENT") return `/workspace/shipment/${id}`;
  if (type === "MIXED_CONTAINER") return `/buyer/mixed-container/requests/${id}`;
  if (type === "BULK_CONTAINER") return `/buyer/bulk-container/requests/${id}`;
...
function deriveMilestone(
  ...
  if (shipmentStates.some((s) => s === "DELIVERED" || s === "CLOSED")) return "Delivered";
  if (shipmentStates.length > 0) return "Shipment";
  if (orderStates.some((s) =>
    ["PRODUCTION_STARTED", "PRODUCTION_COMPLETED", "INSPECTION_PENDING", "INSPECTION_COMPLETED"].includes(s),
  )) return "Production";
  if (orderStates.some((s) => s !== "ORDER_CREATED" && s !== "CANCELLED")) return "Order Confirmed";
  if (hasPo) return "PO Created";
...
export class TradeWorkspaceService {
  ...
  async getWorkspace(actor: AuthUser, workspaceId: string): Promise<TradeWorkspacePayload> {
    ...
    const tradeType = root.type as TradeType;

    const [orders, shipments, pos, freightReqs, timelineEvents, alertRows, rfqDetails, rfqLines] =
      await Promise.all([
    ...
    const header: TradeWorkspaceHeader = {
      tradeId: tradeRef,
    ...
    const summary: TradeSummaryPanel = {
    ...
    const purchaseOrders: TradePoPanelItem[] = pos.map((po) => ({
      poId: po.id,
    ...
    const orderPanels: TradeOrderPanelItem[] = orders.map((o) => {
      let productionStatus = "Not started";
      if (o.state === "PRODUCTION_STARTED") productionStatus = "In progress";
    ...
    const freightPanels: TradeFreightPanelItem[] = freightReqs.map((fr) => {
    ...
    const shipmentPanels: TradeShipmentPanelItem[] = shipments.map((s) => {
    ...
    const refMap = new Map<string, string>([
      ...
      ...orders.map((o) => [o.id, o.externalRef] as const),
      ...shipments.map((s) => [s.id, s.externalRef] as const),
    ]);
    ...
    const documents: TradeDocumentItem[] = [
      ...tradeDocs.map((d) => ({
        id: d.id,
        detailId: `TRADE:${d.id}`,
        fileName: d.fileName,
      ...
      ...rfqAttachments.map((d) => ({
        ...
        category: "Other" as TradeDocumentCategory,
        status: "UPLOADED",
        workspaceType: "RFQ",
        workspaceId: d.workspaceId,
        workspaceRef: refMap.get(d.workspaceId) ?? "",
    ...
    const timeline: TradeTimelineItem[] = timelineEvents.map((e) => ({
      id: e.id,
    ...
    const alertItems: TradeAlertItem[] = alertRows.map((a) => ({
      id: a.id,
      severity: a.severity,
      category: a.category,
      title: a.title,
    ...
    const relatedRecords: TradeRelatedRecord[] = [
      {
        type: root.type,
        id: root.id,
