...
const CATEGORY_MAP: Record<string, DocumentCenterCategory> = {
...
const STATUS_MAP: Record<string, DocumentCenterStatus> = {
  MISSING: "Missing",
...
type RawDoc = DocumentCenterRow & {
  _uploadedAt?: Date | null;
  _expiresAt?: Date | null;
  _buyerId?: string | null;
  _supplierId?: string | null;
  _uploadedBy?: string | null;
  _workspaceId?: string;
};

export class DocumentCenterService {
  private readonly tradeDocs: TradeDocumentsService;

  constructor(private readonly db: PrismaClient) {
    this.tradeDocs = new TradeDocumentsService(db);
  }

  async list(actor: AuthUser, query: DocumentCenterQuery): Promise<DocumentCenterPayload> {
  ...
  async getDetail(actor: AuthUser, compositeId: string): Promise<DocumentCenterDetail> {
  ...
  async getTradeDocuments(actor: AuthUser, tradeRootId: string): Promise<TradeDocumentsPanelPayload> {
    ...
    const checklist = requirements.map((r) => {
      ...
      return {
        ...
        required: r.required,
        status: doc ? mapStatus(doc.status) : "Missing" as DocumentCenterStatus,
  ...
  async streamDownload(actor: AuthUser, compositeId: string, res: import("express").Response) {
    ...
    if (!detail.fileId) throw new AppError(404, "FILE_NOT_AVAILABLE");
  ...
  async approve(actor: AuthUser, compositeId: string, reason?: string) {
    const { source, id } = parseCompositeId(compositeId);
    if (source !== "TRADE") throw new AppError(400, "ACTION_NOT_SUPPORTED");
    const doc = await this.db.tradeDocument.findUnique({ where: { id } });
    if (!doc) throw new AppError(404, "DOCUMENT_NOT_FOUND");
    await this.tradeDocs.applyDocumentAction(
      doc.workspaceType as "ORDER" | "SHIPMENT",
      doc.workspaceId,
      "approve_document",
      actor,
      { documentId: id, reason },
    );
    await this.emitDocAlert(doc, AlertKey.TRADE_DOC_REQUIRED_MISSING, "resolve");
    return this.getDetail(actor, compositeId);
  }

  async reject(actor: AuthUser, compositeId: string, reason: string) {
    const { source, id } = parseCompositeId(compositeId);
    if (source !== "TRADE") throw new AppError(400, "ACTION_NOT_SUPPORTED");
    const doc = await this.db.tradeDocument.findUnique({ where: { id } });
    if (!doc) throw new AppError(404, "DOCUMENT_NOT_FOUND");
  ...
  async requestRevision(actor: AuthUser, compositeId: string, reason: string) {
    const { source, id } = parseCompositeId(compositeId);
    if (source !== "TRADE") throw new AppError(400, "ACTION_NOT_SUPPORTED");
    const doc = await this.db.tradeDocument.findUnique({ where: { id } });
    if (!doc) throw new AppError(404, "DOCUMENT_NOT_FOUND");
    ...
    const workspaceType = doc.workspaceType as "ORDER" | "SHIPMENT";
  ...
  private async collectAccessible(actor: AuthUser): Promise<RawDoc[]> {
    const items: RawDoc[] = [];

    const staffView = hasPortfolioVisibility(actor.role);
    const shipmentWhere: Prisma.WorkspaceWhereInput = staffView
      ? { type: "SHIPMENT" }
      : { type: "SHIPMENT", participants: { some: { userId: actor.id, leftAt: null } } };
    const orderWhere: Prisma.WorkspaceWhereInput = staffView
      ? { type: "ORDER" }
      : { type: "ORDER", participants: { some: { userId: actor.id, leftAt: null } } };
    const rfqWhere: Prisma.WorkspaceWhereInput = staffView
      ? { type: "RFQ" }
    ...
    for (const d of tradeDocs) if (d.uploadedById) userIds.add(d.uploadedById);
  ...
  private async emitDocAlert(
    doc: { workspaceId: string; workspaceType: string; documentType: string },
    _key: string,
    _action: "resolve",
  ) {
...
function mapCategory(documentType: string): DocumentCenterCategory {
  return CATEGORY_MAP[documentType] ?? "Other";
}

function mapStatus(status: string): DocumentCenterStatus {
  return STATUS_MAP[status] ?? "Uploaded";
}

function parseCompositeId(compositeId: string): { source: DocumentCenterSource; id: string } {
  const idx = compositeId.indexOf(":");
  if (idx <= 0) throw new AppError(400, "INVALID_DOCUMENT_ID");
  const source = compositeId.slice(0, idx) as DocumentCenterSource;
...
function computeKpis(rows: RawDoc[]): DocumentCenterKpis {
  const now = Date.now();
...
export function shipmentDocumentStatusSummary(docs: DocumentCenterRow[]): string {
  if (docs.some((d) => ["Rejected", "Revision Requested"].includes(d.status))) return "Rejected Documents";
  if (docs.some((d) => d.status === "Missing")) return "Missing Documents";
  if (docs.some((d) => ["Uploaded", "Under Review"].includes(d.status))) return "Pending Review";
  if (docs.length > 0 && docs.every((d) => d.status === "Approved" || d.status === "Uploaded")) return "All Approved";
  return "No Documents";
}
