...
import type { FunnelStageMetrics } from "@dmx/contracts/commercial-funnel";
...
export const FUNNEL_LABELS: Record<(typeof FUNNEL_STAGE_KEYS)[number], string> = {
...
export function buildFunnelStages(counts: number[], avgHours: (number | null)[]): FunnelStageMetrics[] {
...
export function classifyBuyer(params: {
  ...
  daysSince: number;
}): "Cold" | "Warm" | "Active" | "Power Buyer" {
  if (params.shipmentsCompleted >= 3 && params.ordersCreated >= 3) return "Power Buyer";
  if (params.ordersCreated >= 1 && params.daysSince <= 90) return "Active";
  if (params.rfqsSubmitted >= 1 && params.daysSince <= 90) return "Warm";
  return "Cold";
}
...
export function classifySupplier(params: {
  ...
  revenue: number;
  daysSince: number;
}): "Inactive" | "Emerging" | "Active" | "Top Performer" {
  if (params.quotationsSubmitted === 0) return "Inactive";
  if (isUnknownActivityDays(params.daysSince)) return "Emerging";
  if (params.daysSince > 90) return "Inactive";
  if (params.selectionRate >= 0.25 && params.revenue >= 500) return "Top Performer";
  if (params.quotationsSubmitted >= 3 || params.selectionRate > 0) return "Active";
  return "Emerging";
}
...
export function buyerActivationScore(params: {
  ...
  if (!isUnknownActivityDays(params.daysSince)) {
    if (params.daysSince <= 7) s += 10;
    else if (params.daysSince <= 30) s += 5;
