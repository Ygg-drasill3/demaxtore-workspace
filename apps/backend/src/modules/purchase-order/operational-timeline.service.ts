...
import type {
  ...
  OperationalTimelineListResponse,
...
type RawEvent = OperationalTimelineEvent & { actorUserId?: string | null };
...
interface EventRule {
  ...
  icon: string;
  description?: (payload: Record<string, unknown>) => string | null;
  related?: (ctx: {
    payload: Record<string, unknown>;
    workspaceId: string;
    shipmentIds: Set<string>;
    revisionByNumber: Map<number, string>;
  }) => OperationalTimelineEvent["relatedEntity"];
}
...
const EVENT_RULES: Record<string, EventRule> = {
  ...
  "po.amendment.approved": {
    ...
    icon: "revision",
    description: (p) => {
      if (p.revisionNumber != null) return `Revision ${String(p.revisionNumber)}`;
  ...
  "inspection.requested": {
    ...
    icon: "clipboard",
    related: ({ payload }) =>
      payload.inspectionId ? { type: "INSPECTION", id: String(payload.inspectionId) } : null,
  },
  "inspection.assigned": {
    category: "INSPECTION",
  ...
  "finding.created": {
    ...
    icon: "clipboard",
    description: (p) =>
      [p.category, p.severity].filter(Boolean).map(String).join(" · ") || null,
  },
  "finding.updated": {
    category: "INSPECTION",
  ...
  "task.created": {
    ...
    related: ({ payload }) =>
      payload.taskId ? { type: "ORDER", id: String(payload.taskId) } : null,
  },
  "task.assigned": {
    category: "TASK",
    title: "Task Assigned",
  ...
  "issue.created": {
    ...
    description: (p) => (p.title ? String(p.title) : null),
    related: ({ payload }) =>
      payload.issueId ? { type: "ORDER", id: String(payload.issueId) } : null,
  },
  "issue.updated": {
    category: "ISSUE",
    title: "Issue Updated",
  ...
  "shipment.created": {
    ...
    severity: "info",
    icon: "ship",
    related: ({ workspaceId, shipmentIds }) =>
      shipmentIds.has(workspaceId) ? { type: "SHIPMENT", id: workspaceId } : null,
  },
  "shipment.spawned": {
    category: "SHIPMENT",
    title: "Shipment Created",
...
function fileDesc(p: Record<string, unknown>): string | null {
  const name =
...
function docRelated(payload: Record<string, unknown>): OperationalTimelineEvent["relatedEntity"] {
...
function defaultRule(eventType: string): EventRule {
...
export class OperationalTimelineService {
  ...
  async list(
    poId: string,
    actor: AuthUser,
    query: OperationalTimelineListQuery,
  ): Promise<OperationalTimelineListResponse> {
    ...
    const availableCategories = [...new Set(all.map((e) => e.category))] as OperationalEventCategory[];
  ...
  private async aggregate(poId: string): Promise<OperationalTimelineEvent[]> {
    ...
    if (!po) throw new AppError(404, "PO_NOT_FOUND");
    ...
    const hasIssuedTimeline = raw.some(
      (e) =>
        e.metadata?.eventType === "PURCHASE_ORDER_ISSUED"
        || e.metadata?.eventType === "po.issued"
        || e.metadata?.eventType === "po.submitted",
    );
    const approvedRevNumbers = new Set(
      raw
        .filter((e) =>
          e.metadata?.eventType === "po.amendment.approved"
  ...
  private async enrichActors(events: RawEvent[]): Promise<OperationalTimelineEvent[]> {
    const ids = [...new Set(events.map((e) => e.actorUserId).filter(Boolean))] as string[];
    const users =
      ids.length === 0
        ? []
    ...
    const byId = new Map(
      users.map((u) => [u.id, { id: u.id, name: u.displayName || u.email || u.id }]),
    );
  ...
  private applyFilters(
    ...
    if (query.actorId) out = out.filter((e) => e.actor?.id === query.actorId);
    if (query.from) {
      const from = Date.parse(query.from);
      if (Number.isFinite(from)) out = out.filter((e) => Date.parse(e.occurredAt) >= from);
    }
    if (query.to) {
      const to = Date.parse(query.to);
      if (Number.isFinite(to)) out = out.filter((e) => Date.parse(e.occurredAt) <= to);
    }
...
function prettyDocCategory(category: string): string {
  return category
    .split("_")
