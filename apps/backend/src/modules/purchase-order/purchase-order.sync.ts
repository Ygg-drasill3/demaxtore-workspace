...
export async function autoAcknowledgePoOnSupplierConfirm(
  tx: Prisma.TransactionClient,
  orderId: string,
  actor: PoSyncActor,
  notes = "Auto-acknowledged on order confirmation",
): Promise<boolean> {
  ...
  await tx.auditLog.create({
    data: {
      ...
      fromState: ws.state,
      toState: ws.state,
      payload: { poId: po.id, status: "ACCEPTED", auto: true } as Prisma.InputJsonValue,
    },
  });
