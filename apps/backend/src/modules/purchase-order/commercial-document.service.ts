...
import type {
  ...
  CommercialDocumentReplaceMeta,
  CommercialDocumentUploadMeta,
...
import { canAccessPo, type AuthUser } from "./purchase-order.policy.js";

type RawDoc = CommercialDocumentDto & { _storageKey?: string | null };
...
function parseCompositeId(id: string): { source: CommercialDocumentSource | "COMMERCIAL"; rawId: string } {
  const idx = id.indexOf(":");
  if (idx <= 0) throw new AppError(400, "INVALID_DOCUMENT_ID");
  const prefix = id.slice(0, idx);
  const rawId = id.slice(idx + 1);
  if (prefix === "COMMERCIAL") return { source: "COMMERCIAL", rawId };
  if (
...
function apiPaths(poId: string, compositeId: string) {
...
export class CommercialDocumentService {
  ...
  async list(
    poId: string,
    actor: AuthUser,
    query: CommercialDocumentListQuery,
  ): Promise<CommercialDocumentListResponse> {
    ...
    ] as CommercialDocumentCategory[];
    const availableSources = [...new Set(all.map((d) => d.source))] as CommercialDocumentSource[];
    return {
      items,
      page: query.page,
      ...
      availableSources: availableSources.length
        ? availableSources
        : (["PURCHASE_ORDER", "ORDER_WORKSPACE"] as CommercialDocumentSource[]),
    };
  }

  async get(poId: string, documentId: string, actor: AuthUser): Promise<CommercialDocumentDto> {
    if (!(await canAccessPo(this.db, actor, poId))) throw new AppError(403, "FORBIDDEN");
    const all = await this.aggregate(poId, actor);
    const found = all.find((d) => d.id === documentId);
    if (!found) throw new AppError(404, "DOCUMENT_NOT_FOUND");
    return found;
  }
  ...
  async replace(
    ...
    actor: AuthUser,
    file: { originalname: string; mimetype: string; size: number; buffer: Buffer },
    meta: CommercialDocumentReplaceMeta,
  ): Promise<CommercialDocumentDto> {
    if (!(await canAccessPo(this.db, actor, poId))) throw new AppError(403, "FORBIDDEN");
    this.assertCanMutate(actor);
    ...
    if (!existing) throw new AppError(404, "DOCUMENT_NOT_FOUND");
    if (actor.role === "SUPPLIER" && existing.uploadedById !== actor.id && actor.role === "SUPPLIER") {
      // suppliers may only replace own uploads
      if (existing.uploadedById !== actor.id) throw new AppError(403, "FORBIDDEN");
    }
    ...
    try {
      const updated = await this.db.purchaseOrderCommercialDocument.update({
        ...
        data: {
          ...
          referenceNumber:
            meta.referenceNumber !== undefined
              ? (meta.referenceNumber?.trim() || null)
              : existing.referenceNumber,
          documentDate:
            meta.documentDate !== undefined
  ...
  async remove(poId: string, documentId: string, actor: AuthUser): Promise<{ ok: true }> {
    if (!(await canAccessPo(this.db, actor, poId))) throw new AppError(403, "FORBIDDEN");
    this.assertCanMutate(actor);
    const parsed = parseCompositeId(documentId);
  ...
  async stream(
    ...
    if (!resolved) throw new AppError(404, "DOCUMENT_NOT_FOUND");
  ...
  private async aggregate(poId: string, actor: AuthUser): Promise<CommercialDocumentDto[]> {
    const po = await this.db.purchaseOrder.findUniqueOrThrow({ where: { id: poId } });
    ...
    const uploaderIds = [
      ...new Set([
        ...
        ...tradeDocs.map((d) => d.uploadedById).filter(Boolean) as string[],
    ...
    const actorById = new Map(
      users.map((u) => [u.id, { id: u.id, name: u.displayName?.trim() || u.email || "Unknown user" }]),
    );
    ...
    for (const row of latestOrder) {
      ...
      const source: CommercialDocumentSource =
        row.documentType.toUpperCase() === "FREIGHT" ? "FREIGHT" : "ORDER_WORKSPACE";
      push({
        id,
        purchaseOrderId: poId,
        orderId: po.orderId,
  ...
  private applyFilters(docs: CommercialDocumentDto[], query: CommercialDocumentListQuery) {
    return docs.filter((d) => {
      if (query.category && d.category !== query.category) return false;
      if (query.source && d.source !== query.source) return false;
      if (query.uploadedFrom && d.uploadedAt.slice(0, 10) < query.uploadedFrom) return false;
      if (query.uploadedTo && d.uploadedAt.slice(0, 10) > query.uploadedTo) return false;
      if (query.search) {
  ...
  private sortDocs(
    docs: CommercialDocumentDto[],
    sort: CommercialDocumentListQuery["sort"],
    direction: CommercialDocumentListQuery["direction"],
  ) {
  ...
  private async resolveStorage(
    poId: string,
    documentId: string,
  ): Promise<{ storageKey: string; fileName: string; mimeType: string; fileSizeBytes?: number } | null> {
    const parsed = parseCompositeId(documentId);
    ...
    if (parsed.source === "ORDER_WORKSPACE") {
      if (parsed.rawId.startsWith("trade-")) {
        ...
        if (!row?.fileId || !row.fileName) return null;
  ...
  private async replaceCanonicalPoPdf(
    poId: string,
    actor: AuthUser,
    file: { originalname: string; mimetype: string; size: number; buffer: Buffer },
  ): Promise<CommercialDocumentDto> {
  ...
  private assertCanMutate(actor: AuthUser) {
    if (!this.canMutate(actor)) throw new AppError(403, "FORBIDDEN");
  }

  private canMutate(actor: AuthUser): boolean {
    return ["BUYER", "SUPPLIER", "ADMIN", "SUPER_ADMIN"].includes(actor.role);
  }

  private async auditAndTimeline(
    orderId: string,
    actor: AuthUser,
    action: string,
...
function latestByType<T extends { documentType: string; version: number }>(rows: T[]): T[] {
  const map = new Map<string, T>();
  for (const row of rows) {
    const cur = map.get(row.documentType);
    if (!cur || row.version > cur.version) map.set(row.documentType, row);
  }
  return [...map.values()];
}
