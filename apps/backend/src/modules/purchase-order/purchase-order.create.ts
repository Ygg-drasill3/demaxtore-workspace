...
export interface CreatePurchaseOrderLineInput {
  ...
  quantity: number;
  unitPrice: number;
  /** Sprint 36A — RFQ line lineage (optional). */
  rfqLineItemId?: string | null;
  quotationLineId?: string | null;
  /** Sprint 36B — Product Master reference (optional; snapshot stays on line). */
...
export interface CreatePurchaseOrderInput {
  ...
  /** When omitted, generated via generatePoNumber(). */
  ...
  actorEmail: string;
  actorRole: string;
  /** Extra commercial fields stored on revision snapshot (Sprint 28 direct entry). */
  revisionMetadata?: Record<string, unknown>;
  /** Skip pending acknowledgement row (rare). Default false. */
  skipPendingAcknowledgement?: boolean;
}
 ...
 */
export async function createPurchaseOrderForOrderTx(
  tx: Prisma.TransactionClient,
  input: CreatePurchaseOrderInput,
): Promise<string> {
  let source: PurchaseOrderSource;
  {
    const raw = String(input.source ?? "").trim();
    const upper = raw.toUpperCase();
    const aliases: Record<string, PurchaseOrderSource> = {
      AUTO: "RFQ",
      RFQ: "RFQ",
      MANUAL: "DIRECT",
      DIRECT: "DIRECT",
  ...
  if (input.organizationWorkspaceId) {
    ...
    if (!orgWs) throw new AppError(400, "ORGANIZATION_MISMATCH");
  }
  ...
  try {
    ...
    if (input.organizationWorkspaceId) {
      const { bridgeModuleEventToOrganization, bindOrganizationToPurchaseOrder } = await import(
