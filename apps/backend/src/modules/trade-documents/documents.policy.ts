...
import type { TradeDocumentAction, TradeWorkspaceType } from "@dmx/contracts/trade-documents";
...
async function partnerAssignedToShipment(
  prisma: PrismaClient,
  userId: string,
  shipmentWorkspaceId: string,
  partnerRole: "CUSTOMS_BROKER" | "TRUCKER",
): Promise<boolean> {
...
export async function canAccessTradeWorkspace(
  ...
  if (role !== "CUSTOMS_BROKER" && role !== "TRUCKER") return false;
...
export function assertDocumentActionRole(action: TradeDocumentAction, role: AuthUser["role"]): void {
  const rules: Record<TradeDocumentAction, Array<AuthUser["role"] | string>> = {
...
export function assertTruckerUploadDocumentType(
