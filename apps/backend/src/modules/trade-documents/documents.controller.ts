...
const ACTION_MAP: Record<string, TradeDocumentAction> = {
...
function parseWorkspaceType(raw: string): TradeWorkspaceType {
  if (raw === "ORDER" || raw === "SHIPMENT") return raw;
...
export const documentsController = {
  ...
  async action(req: Request, res: Response) {
    ...
    res.json(
      await service.applyDocumentAction(
        workspaceType,
        workspaceId,
        actionKey,
        req.user!,
