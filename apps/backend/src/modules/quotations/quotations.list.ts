// GET /api/rfq/:id/quotations — buyer comparison + supplier "mine" row.
import type { Quotation, QuotationLineItem, User } from "@prisma/client";
...
type RowWithLines = Quotation & { lineItems: QuotationLineItem[] };

function unitPriceAvgFromLines(
...
function toRow(
  q: RowWithLines,
  supplier: Pick<User, "id" | "displayName"> & { organisation?: { name: string } | null },
): QuotationRowDTO {
  const unitPriceAvg = unitPriceAvgFromLines(q.lineItems, q.unitPriceAvg ? Number(q.unitPriceAvg) : null);
  const status = q.withdrawnAt ? "WITHDRAWN" as const : q.status === "REVISED" ? "REVISED" as const : "SUBMITTED" as const;
...
export async function listQuotationsForWorkspace(
  workspaceId: string,
  actor: AuthUser,
): Promise<QuotationRowDTO[]> {
