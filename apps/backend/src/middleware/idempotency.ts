...
function peekUserId(req: Request): string | null {
  const header = req.headers.authorization;
  if (!header) return null;
  const [scheme, token] = header.split(" ");
  if (scheme?.toLowerCase() !== "bearer" || !token) return null;
  try { return verifyAccessToken(token.trim()).sub; } catch { return null; }
}

export async function idempotency(req: Request, res: Response, next: NextFunction): Promise<void> {
  const key = req.header("Idempotency-Key");
  if (!key || SAFE_METHODS.has(req.method)) { next(); return; }
  const userId = peekUserId(req);
  if (!userId) { next(); return; }   // requireAuth will issue the 401
  ...
  res.json = (body: unknown) => {
    ...
    void prisma.idempotencyKey
      .update({
        where: { key },
        data:  { statusCode: res.statusCode, response: body as object },
      })
  ...
  res.on("finish", () => { /* no-op; res.json path handles it */ });
