 ...
 */
export function submittedEmailKey(req) {
...
export function refreshCookieKey(req) {
    const fromParser = req.cookies?.dmx_refresh;
    const raw = fromParser ??
        (typeof req.headers.cookie === "string"
            ? req.headers.cookie.match(/(?:^|;\s*)dmx_refresh=([^;]+)/)?.[1]
            : undefined);
    if (!raw)
        return `ip:${clientKey(req)}`;
    const decoded = jwt.decode(decodeURIComponent(raw));
    if (decoded?.sub)
        return `refresh:${decoded.sub}:${decoded.jti ?? "legacy"}`;
    return `ip:${clientKey(req)}`;
}
...
export async function checkRedisSocketHandshakeLimit(ip, max) {
    if (!redisUrl())
        return false;
