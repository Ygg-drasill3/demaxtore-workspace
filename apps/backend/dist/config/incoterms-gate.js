...
export function isIncotermsPreconditionsEnabled() {
...
export async function getOrderIncotermProfile(db, orderId) {
...
export function assertIncotermDocuments(profile, approvedDocTypes) {
    if (!isIncotermsPreconditionsEnabled())
        return [];
...
export async function assertOrderIncotermPreconditions(db, orderId, action) {
    if (!isIncotermsPreconditionsEnabled())
        return;
    if (!INCOTERM_GATED_ORDER_ACTIONS.has(action))
        return;
    ...
    const missing = assertIncotermDocuments(profile, approved.map((d) => d.documentType));
    if (missing.length > 0) {
...
//# sourceMappingURL=incoterms-gate.js.map