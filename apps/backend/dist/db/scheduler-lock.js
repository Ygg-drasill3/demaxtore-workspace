...
async function getSchedulerPool(pg) {
    if (!schedulerPool) {
...
async function withDedicatedConnection(lockId, fn) {
    const pg = await loadPg();
    if (!pg)
        return withPrismaTransactionFallback(lockId, fn);
    const pool = await getSchedulerPool(pg);
    const client = await pool.connect();
    try {
        const acquired = await client.query("SELECT pg_try_advisory_lock($1::bigint) AS ok", [lockId.toString()]);
...
async function withPrismaTransactionFallback(lockId, fn) {
    return prisma.$transaction(async (tx) => {
        const acquired = await tx.$queryRaw(Prisma.sql `SELECT pg_try_advisory_lock(${lockId}) AS ok`);
        if (!acquired[0]?.ok)
            return false;
        try {
            await fn();
 ...
 */
export async function withSchedulerLock(lockId, fn) {
    return withDedicatedConnection(lockId, fn);
}
export async function closeSchedulerPool() {
    if (schedulerPool) {
