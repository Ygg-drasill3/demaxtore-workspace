...
import { ConversationSearchQuerySchema, CreateTimelineItemSchema, MarkTimelineDeliveredSchema, MarkTimelineReadSchema, PinTimelineItemSchema, } from "@dmx/contracts/conversation-hub.zod";
...
conversationHubRouter.get("/", asyncHandler(async (req, res) => {
    const workspaceType = req.params.workspaceType.toUpperCase();
    ...
    res.json(await adapters().conversationHub.getHub(() => hub().getHub(workspaceType, workspaceId, actor), workspaceType, workspaceId, toMessagingActor(actor)));
}));
conversationHubRouter.get("/search", asyncHandler(async (req, res) => {
    ...
    res.json(await adapters().conversationHub.search(() => hub().search(workspaceType, workspaceId, actor, query), workspaceType, workspaceId, toMessagingActor(actor)));
}));
conversationHubRouter.get("/participants", asyncHandler(async (req, res) => {
    ...
    const data = await adapters().conversationHub.getHub(() => hub().getHub(workspaceType, workspaceId, actor), workspaceType, workspaceId, toMessagingActor(actor));
    res.json(data.participants);
}));
...
conversationHubRouter.post("/attachments", uploadLimiter, uploadSingle, asyncHandler(async (req, res) => {
    if (!req.file) {
...
conversationHubRouter.get("/attachments/:attachmentId/download", asyncHandler(async (req, res) => {
    const workspaceType = req.params.workspaceType.toUpperCase();
    const workspaceId = req.params.workspaceId;
    const file = await comm().getAttachmentForDownload(workspaceType, workspaceId, req.params.attachmentId, req.user);
    await streamStoredFileToResponse(file.storageKey, res, file);
}));
//# sourceMappingURL=conversation-hub.routes.js.map