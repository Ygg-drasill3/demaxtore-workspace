...
const DECISION_SYSTEM_EVENTS = new Set([
    "SUPPLIER_SELECTED",
    "PURCHASE_ORDER_ISSUED",
    "COMMODITYBID_CLOSED",
    "SHIPMENT_DELIVERED",
]);
const DECISION_EVENT_TITLES = {
...
function categorizeAttachment(fileName, mimeType) {
    const name = fileName.toLowerCase();
    if (mimeType.startsWith("image/"))
        return "PHOTO";
    if (mimeType.startsWith("video/"))
        return "VIDEO";
    if (/quot|quote|bid/.test(name))
        return "QUOTATION";
    if (/\bpo[-_]|purchase.?order|^po\./.test(name))
        return "PURCHASE_ORDER";
    if (/inspection|survey|qc/.test(name))
        return "INSPECTION";
    if (/certificate|cert|coa|origin/.test(name))
        return "CERTIFICATE";
    if (/invoice|proforma|commercial/.test(name))
        return "INVOICE";
    if (/packing|pack.?list/.test(name))
        return "PACKING_LIST";
    if (/bill.?of.?lading|\bbol\b|bl[-_]/.test(name))
        return "BILL_OF_LADING";
    return "OTHER";
}
export function buildAttachmentLibrary(timeline) {
    const items = [];
...
export function buildDecisionLog(timeline) {
    const entries = [];
    for (const item of timeline) {
...
function unansweredQuestions(timeline) {
    const answeredParents = new Set(timeline.filter((t) => t.itemType === "ANSWER" && t.parentMessageId).map((t) => t.parentMessageId));
    return timeline.filter((t) => t.itemType === "QUESTION" && !answeredParents.has(t.id));
}
export function buildPendingActions(timeline) {
    const actions = [];
    let seq = 0;
    const push = (kind, title, description, timelineItemId, createdAt, priority) => {
        actions.push({
            id: `pending-${kind}-${seq++}`,
            kind,
            title,
    ...
    for (const q of unansweredQuestions(timeline)) {
        push("UNANSWERED_QUESTION", "Waiting for Supplier Reply", q.body.slice(0, 120), q.id, q.createdAt, "high");
    }
    for (const item of timeline.filter((t) => t.itemType === "ACTION_REQUIRED")) {
        const title = /approval/i.test(item.body) ? "Buyer Approval Required" : "Action Required";
        const kind = /approval/i.test(item.body)
    ...
    for (const item of timeline.filter((t) => t.systemEventType === "INSPECTION_SCHEDULED")) {
        const hasInspectionDoc = timeline.some((t) => new Date(t.createdAt) > new Date(item.createdAt) &&
            t.attachments.some((a) => categorizeAttachment(a.fileName, a.mimeType) === "INSPECTION"));
        if (!hasInspectionDoc) {
            push("INSPECTION_REPORT_WAITING", "Inspection Report Waiting", "Inspection scheduled — report not yet uploaded", item.id, item.createdAt, "high");
        }
    }
    const hasRecentDoc = timeline.some((t) => t.attachments.length > 0 && Date.now() - new Date(t.createdAt).getTime() < 7 * 86400000);
    if (!hasRecentDoc && timeline.length > 3) {
        push("DOCUMENT_MISSING", "Document Missing", "No documents uploaded in the last 7 days", null, new Date().toISOString(), "low");
    }
...
export function buildSummary(workspaceStatus, supplierName, shipmentStatus, timeline, pendingActions) {
    const decisions = buildDecisionLog(timeline);
    const lastDecision = decisions[0]?.title ?? null;
    let lastDocumentUploaded = null;
...
export function buildHeader(workspaceType, workspaceId, workspaceRef, workspaceStatus, participants, timeline, unreadCount, pendingActionsCount) {
...
export async function loadWorkspaceOperationalContext(db, workspaceType, auditWorkspaceId) {
...
export function getPinnedItems(timeline) {
    return timeline
        .filter((t) => t.pinned)
        .sort((a, b) => {
...
//# sourceMappingURL=conversation-hub.operational.js.map