...
import { redisUrl, getRedisClient } from "../../lib/redis.js";
import { logger } from "../../config/logger.js";
const MAX_ATTEMPTS = 5;
const LOCK_SEC = 15 * 60;
const ATTEMPT_TTL_SEC = 60 * 60;
const key = (ip, email) => `bf:${ip}:${email.toLowerCase()}`;
export async function checkLock(ip, email) {
    if (!redisUrl()) {
...
export async function recordFailure(ip, email) {
    if (!redisUrl())
        return;
    try {
        const redis = await getRedisClient();
...
export async function recordSuccess(ip, email) {
    if (!redisUrl())
        return;
    try {
        const redis = await getRedisClient();
