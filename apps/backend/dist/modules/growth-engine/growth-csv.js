...
export async function exportGrowthCsv(reportType, growth, db) {
    const lines = [];
    switch (reportType) {
        case "buyers": {
            ...
            for (const b of rows) {
                lines.push([b.organisationName, b.classification, b.activationScore, b.rfqsCreated, b.ordersCreated, b.shipmentsCompleted]
                    .map(esc)
                    .join(","));
            }
            break;
        ...
        case "suppliers": {
            ...
            for (const s of rows) {
                lines.push([s.organisationName, s.classification, s.growthScore, s.quotationsSubmitted, s.winRate, s.freightiqRevenueUsd]
                    .map(esc)
                    .join(","));
            }
            break;
        ...
        case "categories": {
            ...
            for (const c of rows) {
                lines.push([c.category, c.rfqCount, c.orderCount, c.shipmentCount, c.revenueUsd, c.freightiqRevenueUsd].map(esc).join(","));
            }
            break;
        }
        case "routes": {
            ...
            for (const r of rows) {
                lines.push([r.route, r.lane, r.rfqCount, r.orderCount, r.shipmentCount, r.revenueUsd, r.freightiqRevenueUsd]
                    .map(esc)
                    .join(","));
            }
            break;
        ...
        case "lost-opportunities": {
            ...
            for (const i of lost.items) {
                lines.push([i.type, i.workspaceRef, i.description, i.estimatedLostRevenueUsd, i.estimatedLostFreightiqRevenueUsd]
                    .map(esc)
                    .join(","));
            }
            break;
        ...
        case "repeat-customers": {
            ...
            for (const r of rows) {
                lines.push([r.horizonDays, r.firstTimeBuyers, r.repeatBuyers, r.repeatRate, r.repeatShipmentRate, r.repeatRevenueUsd]
                    .map(esc)
                    .join(","));
            }
            break;
...
//# sourceMappingURL=growth-csv.js.map