...
export async function scanGrowthAlerts(db) {
    let n = 0;
    const growth = new GrowthService(db);
    const buyers = await growth.getBuyerActivation();
    for (const b of buyers.filter((x) => x.classification === "Cold" && exceedsInactivityThreshold(x.daysSinceActivity, INACTIVE_DAYS))) {
    ...
    for (const b of buyers.filter((x) => x.classification === "Active" && !isUnknownActivityDays(x.daysSinceActivity) && x.daysSinceActivity >= 21)) {
        const anchor = await db.workspace.findFirst({
            where: { type: "RFQ", createdById: { in: b.buyerUserIds } },
            orderBy: { updatedAt: "desc" },
        });
...
//# sourceMappingURL=growth-alerts.js.map