import { mcStateToProcurementStatus, PROCUREMENT_STATUS_LABELS, } from "@dmx/contracts/mixed-container-procurement";
...
export async function nextPrRef(prisma) {
...
export async function nextCpRef(prisma) {
    const year = new Date().getUTCFullYear();
...
export function buyerProcurementLink(workspaceId) {
    return `/buyer/mixed-container/requests/${workspaceId}`;
}
export function buyerOrganizationWorkspaceLink(workspaceId) {
    return buyerOrganizationLink(workspaceId);
}
export function adminProcurementLink(workspaceId) {
    return `/admin/mixed-container/${workspaceId}`;
}
export function adminOrganizationWorkspaceLink(workspaceId) {
    return adminOrganizationLink(workspaceId);
}
export async function notifyBuyerProcurementStatus(tx, input) {
    ...
    if (!payload)
        return;
    const link = input.status === "ORGANIZATION_STARTED"
...
export async function notifyBuyerProposalRevised(tx, input) {
    await notifyMcEvent(tx, {
        userIds: [input.buyerUserId],
        workspaceId: input.workspaceId,
...
export async function notifyAdminsRevisionRequested(tx, input) {
...
export async function notifyAdminsNewProcurementRequest(tx, input) {
    const admins = await tx.user.findMany({ where: { role: "ADMIN" }, select: { id: true } });
    if (!admins.length)
        return;
    await notifyMcEvent(tx, {
...
export async function notifyBuyerManagerAssigned(tx, input) {
    await notifyMcEvent(tx, {
        userIds: [input.buyerUserId],
        workspaceId: input.workspaceId,
...
//# sourceMappingURL=mc-procurement.helpers.js.map