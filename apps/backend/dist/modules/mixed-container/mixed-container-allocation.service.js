import { findMcTransition, } from "@dmx/contracts/mixed-container.fsm";
...
export class MixedContainerAllocationService {
    ...
    async assertAdminMcWorkspace(id) {
    ...
    async getAllocationWorkspace(id) {
    ...
    async startAllocation(id, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
    ...
    async createAllocation(id, input, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
        ...
        if (existing)
            throw new AppError(409, "LINE_ALREADY_ALLOCATED");
    ...
    async completeAllocations(id, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
        const ws = await this.prisma.workspace.findUniqueOrThrow({
            where: { id },
    ...
    async uploadProforma(id, allocationId, input, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
        const ws = await this.assertAdminMcWorkspace(id);
        ...
        if (!alloc)
            throw new AppError(404, "ALLOCATION_NOT_FOUND");
        const existingProforma = await this.prisma.mcSupplierProforma.findFirst({
            where: { allocationId },
        });
        if (existingProforma)
            throw new AppError(409, "PROFORMA_ALREADY_EXISTS");
        await this.prisma.$transaction(async (tx) => {
    ...
    async createPayment(id, input, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
        await this.assertAdminMcWorkspace(id);
        const alloc = await this.prisma.mcSupplierAllocation.findFirst({
        ...
        if (existing)
            throw new AppError(409, "PAYMENT_ALREADY_EXISTS");
    ...
    async updatePayment(id, paymentId, input, actor) {
        const ws = await this.assertAdminMcWorkspace(id);
    ...
    async tryMarkExecutionReady(tx, id, actor) {
        const ws = await tx.workspace.findUniqueOrThrow({
            where: { id },
            include: { mcSupplierAllocations: true, mcPaymentRecords: true },
        });
        if (ws.state !== "MC_PAYMENT_TRACKING")
            return;
        if (ws.mcSupplierAllocations.length === 0)
            return;
    ...
    async getCoordination(id, actor) {
        const ws = await this.prisma.workspace.findUniqueOrThrow({
            where: { id },
            include: {
                mixedContainerDetails: true,
    ...
    async reviewProforma(id, proformaId, actor) {
        if (actor.role !== "BUYER")
            throw new AppError(403, "FORBIDDEN");
        await this.assertBuyerAccess(id, actor);
        ...
        if (!proforma)
            throw new AppError(404, "PROFORMA_NOT_FOUND");
...
//# sourceMappingURL=mixed-container-allocation.service.js.map