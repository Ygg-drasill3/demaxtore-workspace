...
export async function canAccessMixedContainer(prisma, user, workspaceId) {
    if (user.role === "ADMIN")
        return true;
    if (user.role !== "BUYER")
        return false;
...
export async function assertCanAccessMixedContainer(prisma, user, workspaceId) {
...
export async function assertCanManageProcurement(prisma, user, workspaceId) {
    if (user.role !== "ADMIN")
        throw new AppError(403, "FORBIDDEN");
...
export async function assertCanManageOrganization(prisma, user, workspaceId) {
    if (user.role !== "ADMIN")
        throw new AppError(403, "FORBIDDEN");
    const d = await prisma.mixedContainerDetails.findUnique({
        where: { workspaceId },
...
//# sourceMappingURL=mixed-container.policy.js.map