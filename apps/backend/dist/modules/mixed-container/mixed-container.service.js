import { MC_CONTAINER_CAPACITIES, findMcTransition, } from "@dmx/contracts/mixed-container.fsm";
import { MC_MAX_CONTAINERS_PER_ORDER, } from "@dmx/contracts/mixed-container.zod";
...
import { assertLinesHavePackingType, assertValidPackingTypeForProduct, } from "../packing-type/packing-type.helpers.js";
import { nextPrRef, notifyAdminsNewProcurementRequest, notifyBuyerProcurementStatus, recordProcurementStatusHistory, } from "./mc-procurement.helpers.js";
const WS_INCLUDE = {
...
export function toMixedContainerDTO(ws) {
...
async function nextMcRef(prisma) {
...
async function assertContainerCapacity(tx, workspaceId, nextTotalPallets) {
...
async function recalcDetails(tx, workspaceId) {
...
async function loadOrderContainerSlots(prisma, orderGroupId) {
    ...
    return rows.map((r) => ({
        ...
        fillPercent: r.maxPalletCapacity > 0 ? Math.round((r.currentPalletCount / r.maxPalletCapacity) * 100) : 0,
    }));
}
function enrichMixedContainerDTO(ws, base, orderContainers) {
    const d = ws.mixedContainerDetails;
...
export class MixedContainerService {
    ...
    async addSiblingContainer(sourceId, actor) {
        if (actor.role !== "BUYER")
            throw new AppError(403, "FORBIDDEN_ROLE");
        ...
        if (source.type !== "MIXED_CONTAINER")
            throw new AppError(409, "WRONG_WORKSPACE_TYPE");
    ...
    async addLine(id, input, actor) {
        ...
        if (input.packagingId) {
            ...
            if (!packaging)
                throw new AppError(400, "INVALID_PACKAGING_FOR_PRODUCT");
        ...
        const packingLink = await assertValidPackingTypeForProduct(this.prisma, "MIXED_CONTAINER", input.catalogProductId, packingTypeId);
        await this.prisma.$transaction(async (tx) => {
    ...
    async updateLine(id, lineId, input, actor) {
        await assertCanAccessMixedContainer(this.prisma, actor, id);
    ...
    async removeLine(id, lineId, actor) {
        await assertCanAccessMixedContainer(this.prisma, actor, id);
        const line = await this.prisma.containerLine.findFirst({
            where: { id: lineId, workspaceId: id, removedAt: null },
        });
    ...
    async requestPricing(id, actor, input = {}) {
        await assertCanAccessMixedContainer(this.prisma, actor, id);
        const ws = await this.prisma.workspace.findUniqueOrThrow({
            where: { id },
            include: {
        ...
        if (ws.containerLines.length === 0)
            throw new AppError(400, "EMPTY_CONTAINER");
...
//# sourceMappingURL=mixed-container.service.js.map