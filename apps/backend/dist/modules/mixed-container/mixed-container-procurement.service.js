import { findMcTransition, MC_OFFER_VALIDITY_HOURS, } from "@dmx/contracts/mixed-container.fsm";
import { mcStateToProcurementStatus, TIMELINE_EVENT_LABELS, } from "@dmx/contracts/mixed-container-procurement";
...
import { notifyBuyerManagerAssigned, notifyBuyerProcurementStatus, notifyBuyerProposalRevised, notifyAdminsRevisionRequested, nextCpRef, recordProcurementStatusHistory, } from "./mc-procurement.helpers.js";
...
async function handleProcurementSideEffects(tx, input) {
...
async function applyMcTransition(tx, workspaceId, action, actor, auditEvent, payload = {}) {
...
function toOfferDTO(ws, offer, extras) {
    const now = Date.now();
    const expiresInSeconds = offer.validityDate && offer.status === "SENT"
...
export class MixedContainerProcurementService {
    prisma;
    organizationService;
    constructor(prisma, organizationService = new MixedContainerOrganizationService(prisma)) {
        this.prisma = prisma;
        this.organizationService = organizationService;
    }
    async countFreightActive() {
    ...
    async inbox(filters = {}) {
        const stateFilter = filters.status?.startsWith("MC_")
            ? [filters.status]
            : filters.status
                ? this.statesForProcurementStatus(filters.status)
                : [
        ...
        if (filters.managerId)
            detailsWhere.assignedManagerId = filters.managerId;
    ...
    statesForProcurementStatus(status) {
        switch (status) {
            case "SUBMITTED":
    ...
    async kpis() {
        ...
        return {
            ...
            allocationsPending: (map.get("MC_APPROVED") ?? 0) + (map.get("MC_ALLOCATION_IN_PROGRESS") ?? 0),
            ...
            dashboard: {
                ...
                proposalPreparationQueue: (map.get("MC_PROCUREMENT_IN_PROGRESS") ?? 0) + (map.get("MC_OFFER_READY") ?? 0),
            },
        };
    }
    ...
    async getProcurementRequest(id, actor, includeInternalNotes = false) {
        ...
        const container = toMixedContainerDTO(ws);
        ...
        if (isAdmin && includeInternalNotes && "mcInternalNotes" in ws) {
            const noteAuthorIds = ws.mcInternalNotes.map((n) => n.authorId);
            ...
            internalNotes = ws.mcInternalNotes.map((n) => ({
                id: n.id,
                authorId: n.authorId,
    ...
    async addInternalNote(id, input, actor) {
        if (actor.role !== "ADMIN")
    ...
    async getProcurement(id) {
        ...
        return {
            container: toMixedContainerDTO(ws),
    ...
    async startProcurement(id, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
    ...
    async assignManager(id, managerId, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            if (findMcTransition(ws.state, "assign_buyer_manager")) {
    ...
    async upsertQuote(id, input, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
        ...
        if (!line)
            throw new AppError(404, "LINE_NOT_FOUND");
    ...
    async createOffer(id, input, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
        const ws = await this.prisma.workspace.findUniqueOrThrow({
            where: { id },
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            const logisticsCost = input.logisticsCost ?? (input.exportExecutionFee ?? 0) + (input.estimatedFreight ?? 0);
    ...
    async sendOffer(id, offerId, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
        await this.prisma.$transaction(async (tx) => {
            const ws = await tx.workspace.findUniqueOrThrow({
            ...
            if (offer.status !== "DRAFT")
                throw new AppError(409, "OFFER_NOT_DRAFT");
    ...
    async getBuyerOffer(offerId, actor) {
    ...
    async getCommercialProposal(workspaceId, actor, offerId) {
        ...
        if (!targetOfferId) {
            if (actor.role === "BUYER") {
                ...
                if (!visible)
                    throw new AppError(404, "NO_COMMERCIAL_PROPOSAL");
    ...
    async approveOffer(offerId, actor) {
        if (actor.role !== "BUYER")
            throw new AppError(403, "FORBIDDEN");
        const offer = await this.prisma.mcContainerOffer.findUniqueOrThrow({
            where: { id: offerId },
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            const orgState = await applyMcTransition(tx, offer.workspaceId, "begin_organization", { id: actor.id, role: "SYSTEM", email: actor.email }, "mixed_container.organization_started", { offerId });
            await handleProcurementSideEffects(tx, {
                workspaceId: offer.workspaceId,
            ...
            await this.organizationService.createOrganization(tx, offer.workspaceId, actor.id, offer.workspace.createdById);
    ...
    async requestRevision(offerId, input, actor) {
        if (actor.role !== "BUYER")
            throw new AppError(403, "FORBIDDEN");
        const offer = await this.prisma.mcContainerOffer.findUniqueOrThrow({
            where: { id: offerId },
    ...
    async expireOffers(actor) {
        const systemActor = actor.role === "ADMIN" ? actor : { ...actor, role: "ADMIN", id: actor.id };
        const now = new Date();
        ...
        for (const offer of expired) {
            if (offer.workspace.state !== "MC_BUYER_REVIEW")
                continue;
            await this.prisma.$transaction(async (tx) => {
                await tx.mcContainerOffer.update({ where: { id: offer.id }, data: { status: "EXPIRED" } });
                await applyMcTransition(tx, offer.workspaceId, "expire_offer", { id: "00000000-0000-0000-0000-000000000001", role: "SYSTEM", email: "system@demaxtore.local" }, "mixed_container.offer_expired", { offerId: offer.id });
            });
            count++;
        }
        return count;
    }
    async resumeProcurement(id, actor) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN");
        await this.prisma.$transaction(async (tx) => {
            await applyMcTransition(tx, id, "resume_procurement", actor, "mixed_container.procurement_started");
...
//# sourceMappingURL=mixed-container-procurement.service.js.map