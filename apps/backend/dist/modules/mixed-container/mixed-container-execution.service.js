...
async function resolveSupplierUserId(tx, supplierCode, supplierId) {
    if (supplierId)
        return supplierId;
...
export class MixedContainerExecutionService {
    ...
    async spawnExecutionOrders(id, actor) {
        ...
        const allPaid = ws.mcSupplierAllocations.every((a) => a.payments.some((p) => p.paymentStatus === "PAYMENT_CONFIRMED"));
        if (!allPaid)
        ...
        if (ws.mcOrderLinks.length > 0) {
            ...
            return {
                ...
                supplierOrders: ws.mcOrderLinks.map((l) => ({
                    allocationRef: allocationRef(ws.mcSupplierAllocations.find((a) => a.id === l.allocationId).sortOrder),
    ...
    async getExecution(id, actor, opts = {}) {
        const ws = await this.prisma.workspace.findUniqueOrThrow({
            where: { id },
            include: {
                mixedContainerDetails: true,
    ...
    async tryMarkExecutionComplete(smartContainerId) {
        const ws = await this.prisma.workspace.findUnique({
            where: { id: smartContainerId },
            include: { mcOrderLinks: true },
        });
        if (!ws || ws.state !== "MC_EXECUTION_ACTIVE" || ws.mcOrderLinks.length === 0)
            return;
        const orderIds = ws.mcOrderLinks.map((l) => l.supplierOrderId);
        ...
        await this.prisma.$transaction(async (tx) => {
            await applyMcTransition(tx, smartContainerId, "mark_execution_complete", { id: "00000000-0000-0000-0000-000000000001", role: "SYSTEM", email: "system@demaxtore.local" }, "smartcontainer.execution_completed");
    ...
    async recordFreightStarted(smartContainerId, orderId) {
    ...
    async recordShipmentStarted(smartContainerId, orderId, shipmentId) {
        const link = await this.prisma.mcOrderLink.findFirst({
            where: { smartContainerId, supplierOrderId: orderId },
        });
        if (!link)
...
//# sourceMappingURL=mixed-container-execution.service.js.map