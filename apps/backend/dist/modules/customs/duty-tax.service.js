...
import { MATERIAL_ESTIMATE_CHANGE_THRESHOLD, dutyTaxCompletenessLabel, roundMoney, } from "@dmx/contracts/duty-tax";
...
const DISCLAIMER = "Preliminary customs cost estimate — not an official Turkish Customs assessment or tax liability.";
const EVALUABLE = ["CUSTOMS_DUTY", "VAT"];
...
export function createDutyTaxService(db) {
    const issues = new OperationalIssueService(db);
    async function requireCaseAccess(actor, caseId) {
    ...
    async function refreshLandedCost(shipmentWorkspaceId) {
        try {
    ...
    function mapCalc(row) {
        ...
        for (const t of [...EVALUABLE, ...ALWAYS_NOT_EVALUATED]) {
            ...
            totalsByComponent[t] = roundMoney(matching.reduce((s, l) => s + (l.overrideAmount ?? l.amount ?? 0), 0));
        }
        return {
            id: row.id,
            ...
            customsValueEstimate: row.customsValueEstimate == null ? null : num(row.customsValueEstimate),
            ...
            totalEvaluatedAmount: row.totalEvaluatedAmount == null ? null : num(row.totalEvaluatedAmount),
    ...
    async function loadLineContexts(shipmentWorkspaceId) {
    ...
    async function resolveFreight(shipmentWorkspaceId, override) {
        if (override != null)
            return { amount: roundMoney(override), source: "MANUAL" };
        const sw = await db.shipmentWorkspace.findUnique({
            where: { workspaceId: shipmentWorkspaceId },
            select: { freightOfferId: true },
        });
        if (!sw?.freightOfferId)
    ...
    function gtipMatchCandidates(gtipCode) {
    ...
    async function matchRule(componentType, gtipCode, origin, at) {
        ...
        const applicable = rules.filter((r) => !r.originCountryCode || (origin && r.originCountryCode === origin));
    ...
    async function syncIssues(caseRow, dto) {
        const missingInput = dto.diagnostics.some((d) => ["CLASSIFICATION_REQUIRED", "ORIGIN_REQUIRED", "EXCHANGE_RATE_REQUIRED"].includes(d));
    ...
    return {
        ...
        async listVersions(actor, caseId) {
            await requireCaseAccess(actor, caseId);
        ...
        async getById(actor, caseId, calculationId) {
            await requireCaseAccess(actor, caseId);
            const row = await db.dutyTaxCalculation.findFirst({
            ...
            if (!row)
                throw new AppError(404, "DUTY_TAX_CALCULATION_NOT_FOUND");
            return mapCalc(row);
        },
        async calculate(actor, caseId, input) {
            ...
            const freight = await resolveFreight(caseRow.shipmentWorkspaceId, input.freightAmountOverride ?? null);
            const insurance = input.insuranceAmount == null ? null : roundMoney(input.insuranceAmount);
            const targetCurrency = (input.targetCurrency || "TRY").toUpperCase();
            const needsFx = !!sourceCurrency && sourceCurrency.toUpperCase() !== targetCurrency;
            ...
            const goodsTotalSource = roundMoney(input.goodsValueOverride != null
            ...
            for (const line of lines) {
                ...
                const share = goodsTotalSource > 0 ? line.goodsValue / goodsTotalSource : 1 / Math.max(lines.length, 1);
                const allocFreight = input.freightAllocationMethod === "NONE" || freight.amount == null
                    ? 0
                ...
                for (const componentType of EVALUABLE) {
                    ...
                    const { rule, conflict } = await matchRule(componentType, line.gtipCode, line.originCountryCode, calcDate);
                    if (conflict) {
                    ...
                    if (rule.baseFormula === "GOODS_VALUE")
                        base = goodsC;
                    if (rule.baseFormula === "GOODS_PLUS_FREIGHT")
                        base = roundMoney(goodsC + allocFreight);
                    ...
                    built.push({
                        ...
                        warning: line.classificationStatus === "CANDIDATE"
            ...
            const evaluatedTotal = roundMoney(built
        ...
        async review(actor, caseId, input) {
        ...
        async override(actor, caseId, input) {
            const { row: caseRow, access } = await requireCaseAccess(actor, caseId);
            if (access !== "BROKER" && access !== "OPS") {
            ...
            if (!line)
                throw new AppError(404, "DUTY_TAX_LINE_NOT_FOUND");
            ...
            const total = roundMoney(refreshedLines
        ...
        async listRules(actor) {
            const role = String(actor.role);
            if (!["ADMIN", "SUPER_ADMIN", "OPS_MANAGER", "LOGISTICS_OPERATOR", "BUYER", "CUSTOMS_BROKER"].includes(role)) {
                throw new AppError(403, "DUTY_TAX_RULE_FORBIDDEN");
            }
...
//# sourceMappingURL=duty-tax.service.js.map