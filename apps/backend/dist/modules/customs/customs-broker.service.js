import { computeCustomsBrokerAllowedActions, } from "@dmx/contracts/customs-broker-execution";
...
export function createCustomsBrokerService(db) {
    ...
    async function requireBrokerCase(actor, caseId) {
    ...
    async function withAllowed(dto) {
        return {
            ...dto,
    ...
    async function reevalPreArrival(shipmentWorkspaceId) {
        void import("./pre-arrival-customs.service.js")
            .then(({ createPreArrivalCustomsService }) => createPreArrivalCustomsService(db).safeEvaluateShipment(shipmentWorkspaceId))
            .catch(() => undefined);
    }
    return {
        async getWithActions(actor, caseId) {
            const dto = await customs.get(actor, caseId);
            return withAllowed(dto);
        },
        async startReview(actor, caseId, input = {}) {
        ...
        async verifyClassification(actor, caseId, input) {
            const row = await requireBrokerCase(actor, caseId);
            ...
            if (!onCase)
                throw new AppError(403, "PRODUCT_NOT_ON_CUSTOMS_CASE");
            ...
            await db.$transaction(async (tx) => {
                await tx.product.update({
                    ...
                    data: {
                        ...
                        customsDescription: input.customsDescription?.trim() || product.customsDescription,
        ...
        async requestDocument(actor, caseId, input) {
            const row = await requireBrokerCase(actor, caseId);
            ...
            try {
                await tradeDocs.applyDocumentAction("SHIPMENT", row.shipmentWorkspaceId, "request_document", actor, { documentType: input.documentType, notes: input.reason });
            }
            catch {
        ...
        async startDeclarationPreparation(actor, caseId) {
        ...
        async startCustomsProcessing(actor, caseId) {
            await requireBrokerCase(actor, caseId);
            const dto = await customs.transition(actor, caseId, {
        ...
        async markClearancePending(actor, caseId) {
            await requireBrokerCase(actor, caseId);
            const dto = await customs.transition(actor, caseId, {
        ...
        async markCleared(actor, caseId) {
            const row = await requireBrokerCase(actor, caseId);
        ...
        async placeHold(actor, caseId, input) {
            const row = await requireBrokerCase(actor, caseId);
...
//# sourceMappingURL=customs-broker.service.js.map