...
function parseMessageType(raw) {
    if (!raw)
        return "unsupported";
    return SUPPORTED_TYPES.has(raw) ? raw : "unsupported";
}
    ...
    for (const entry of asArray(body.entry)) {
        for (const change of asArray(entry.changes)) {
            ...
            for (const c of asArray(value.contacts)) {
                ...
                if (waId && name)
                    contactsByWaId.set(waId, String(name));
            }
...
export function parseStatusUpdates(body) {
    const out = [];
    if (body.object !== "whatsapp_business_account")
        return out;
    ...
    for (const entry of asArray(body.entry)) {
        for (const change of asArray(entry.changes)) {
            ...
            for (const st of asArray(value.statuses)) {
                ...
                if (!metaMessageId || !status || !validStatuses.has(status))
                    continue;
...
//# sourceMappingURL=whatsapp-inbox.parser.js.map