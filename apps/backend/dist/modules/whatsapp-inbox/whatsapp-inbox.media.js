...
export async function downloadWhatsAppMedia(db, mediaId, opts) {
    const creds = await resolveMediaAccessCredentials(db, opts?.phoneNumberId);
    if (!creds) {
        logger.warn({ mediaId, phoneNumberId: opts?.phoneNumberId ?? null }, "[WA-Inbox] media download skipped — no tenant credentials");
        return null;
    }
    ...
    try {
        ...
        if (!metaResp.ok) {
            logger.warn({ mediaId, status: metaResp.status, phoneNumberId: creds.phoneNumberId, buyerId: creds.buyerId }, "[WA-Inbox] media metadata fetch failed");
            return null;
        }
        ...
        logger.error({ err, mediaId, phoneNumberId: creds.phoneNumberId, buyerId: creds.buyerId }, "[WA-Inbox] media download failed");
        return null;
    }
}
//# sourceMappingURL=whatsapp-inbox.media.js.map