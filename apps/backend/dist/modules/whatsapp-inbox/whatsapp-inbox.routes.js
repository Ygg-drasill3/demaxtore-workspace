...
const INBOX_ROLES = ["SUPER_ADMIN", "ADMIN", "OPS_MANAGER", "SALES_CONTROL"];
router.get("/conversations", requireAuth, requireRole(...INBOX_ROLES), asyncHandler(async (req, res) => {
    ...
    res.json(await adapters().whatsappInbox.listConversations(() => inbox().listConversations(actor, { cursor, limit }), toMessagingActor(actor)));
}));
router.get("/conversations/:id/messages", requireAuth, requireRole(...INBOX_ROLES), asyncHandler(async (req, res) => {
    const cursor = typeof req.query.cursor === "string" ? req.query.cursor : undefined;
    const limit = req.query.limit ? Number(req.query.limit) : undefined;
    const actor = req.user;
    if (!shouldUseAdapterLayer()) {
    ...
    res.json(await adapters().whatsappInbox.getMessages(() => inbox().getMessages(actor, req.params.id, { cursor, limit }), req.params.id, toMessagingActor(actor)));
}));
router.post("/conversations/:id/read", requireAuth, requireRole(...INBOX_ROLES), asyncHandler(async (req, res) => {
    const data = await inbox().markRead(req.user, req.params.id);
    res.json(data);
}));
router.post("/messages", requireAuth, requireRole(...INBOX_ROLES), asyncHandler(async (req, res) => {
    const body = req.body ?? {};
...
router.get("/media/:messageId", requireAuth, requireRole(...INBOX_ROLES), asyncHandler(async (req, res) => {
...
//# sourceMappingURL=whatsapp-inbox.routes.js.map