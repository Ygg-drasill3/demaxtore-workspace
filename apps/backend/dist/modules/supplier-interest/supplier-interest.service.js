...
import { normalizeInterestLabels, resolveInterestCategorySlugs, SUPPLIER_INTEREST_CATEGORY_SLUGS, } from "@dmx/contracts/supplier-interest";
...
async function resolveActorOrganisationId(userId) {
...
export const supplierInterestService = {
    ...
    async getInterests(organisationId) {
        const org = await prisma.organisation.findUnique({
            where: { id: organisationId },
            select: { interestAreas: true },
        });
        if (!org)
            throw new AppError(404, "ORGANISATION_NOT_FOUND");
        return {
            organisationId,
    ...
    async setInterests(organisationId, labels) {
    ...
    async setMyInterests(userId, labels) {
        const organisationId = await resolveActorOrganisationId(userId);
     ...
     */
    async resolveSlugsFromText(text) {
        return resolveInterestCategorySlugs(text, SUPPLIER_INTEREST_CATEGORY_SLUGS);
    },
};
//# sourceMappingURL=supplier-interest.service.js.map