...
export async function scanScaleReadinessAlerts(db) {
    let n = 0;
    ...
    for (const b of buyers) {
        if (!exceedsInactivityThreshold(b.activity.daysSinceActivity, INACTIVE_DAYS))
            continue;
    ...
    for (const s of suppliers) {
        if (!exceedsInactivityThreshold(s.activity.daysSinceActivity, INACTIVE_DAYS))
            continue;
    ...
    for (const item of pipe.items.filter((i) => i.stalled).slice(0, 20)) {
        if (await isTestWorkspace(db, item.workspaceId))
            continue;
        if (await upsertControlTowerAlert(db, {
...
//# sourceMappingURL=scale-alerts.js.map