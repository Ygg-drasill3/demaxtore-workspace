...
export async function uploadAttachment(workspaceId, actor, file) {
    if (file.sizeBytes > MAX_SIZE_BYTES)
        throw Validation(`File too large (max ${MAX_SIZE_BYTES} bytes)`);
    ...
    if (!(await canAccessRfq(prisma, actor, workspaceId)))
        throw Forbidden("No access to workspace");
...
export async function getAttachmentForDownload(workspaceId, attachmentId, actor) {
    if (!(await canAccessRfq(prisma, actor, workspaceId)))
        throw Forbidden("No access to workspace");
...
export async function deleteAttachment(workspaceId, attachmentId, actor) {
    if (!(await canAccessRfq(prisma, actor, workspaceId)))
        throw Forbidden("No access to workspace");
    const row = await prisma.rfqAttachment.findUnique({ where: { id: attachmentId } });
    if (!row || row.workspaceId !== workspaceId)
