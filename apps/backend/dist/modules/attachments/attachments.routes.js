...
router.post("/", requireAuth, uploadLimiter, uploadSingleFile, asyncHandler(async (req, res) => {
    const file = req.file;
    if (!file)
        throw Validation("Missing 'file' field in multipart form");
...
router.get("/:attId", requireAuth, asyncHandler(async (req, res) => {
    const workspaceId = getRfqId(req);
...
router.delete("/:attId", requireAuth, asyncHandler(async (req, res) => {
    const workspaceId = getRfqId(req);
    const { attId } = req.params;
