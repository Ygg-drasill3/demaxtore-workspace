...
function orderWs(ws) {
    return ws.orderWorkspace;
}
export const PRECONDITIONS = {
    assertPlannedCompletionDate: ({ payload }) => {
        if (!payload.plannedCompletionDate)
            throw new AppError(400, "PLANNED_COMPLETION_REQUIRED");
    },
    assertProgressLabel: ({ payload }) => {
        if (!String(payload.label ?? "").trim())
            throw new AppError(400, "PROGRESS_LABEL_REQUIRED");
    },
    ...
    assertLatestProductionPercent100: ({ workspace, payload }) => {
        ...
        if (Number.isFinite(pct) && pct >= 100)
            return;
        const updates = workspace.orderStatusUpdates ?? [];
        const latest = updates.find((u) => u.updateType === "PRODUCTION") ?? updates[0];
        if (latest?.percentage != null && latest.percentage >= 100)
            return;
    ...
    assertInspectionResult: ({ payload }) => {
        if (!["PASS", "FAIL"].includes(String(payload.result)))
            throw new AppError(400, "INVALID_INSPECTION_RESULT");
        if (!payload.reportUrl)
            throw new AppError(400, "REPORT_URL_REQUIRED");
        if (!payload.inspectorName)
            throw new AppError(400, "INSPECTOR_NAME_REQUIRED");
    },
    assertInspectionPass: ({ workspace }) => {
        if (orderWs(workspace)?.inspectionResult !== "PASS")
            throw new AppError(400, "INSPECTION_NOT_PASS");
    },
    ...
    assertActualDepartureDate: ({ payload }) => {
        if (!payload.actualDepartureDate)
            throw new AppError(400, "ACTUAL_DEPARTURE_REQUIRED");
    },
    assertNewEta: ({ payload }) => {
        if (!payload.newEta)
            throw new AppError(400, "NEW_ETA_REQUIRED");
    },
    assertActualArrivalDate: ({ payload }) => {
        if (!payload.actualArrivalDate)
            throw new AppError(400, "ACTUAL_ARRIVAL_REQUIRED");
    },
    ...
    assertSettlementConfirmation: ({ payload }) => {
        if (!String(payload.settlementConfirmation ?? "").trim())
            throw new AppError(400, "SETTLEMENT_CONFIRMATION_REQUIRED");
    },
    assertDisputeCategory: ({ payload }) => {
        ...
        if (!cats.includes(String(payload.category)))
            throw new AppError(400, "DISPUTE_CATEGORY_REQUIRED");
    },
    ...
    assertFreightCoordinationReady: ({ workspace }) => {
        const freightRequests = workspace.freightRequests ?? [];
        if (!isFreightOfferSelected(freightRequests)) {
            throw new AppError(409, "FREIGHT_OFFER_NOT_SELECTED", {
                message: "FreightIQ bölümünden bir navlun teklifi seçin (veya admin teklif girsin). Seçim yapılmadan sevkiyat rezerve edilemez.",
            });
        }
    },
};
//# sourceMappingURL=order.preconditions.js.map