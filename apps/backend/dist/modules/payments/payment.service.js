...
import { getPaymentCapabilities, isPaymentIntentApiEnabled, resolvePaymentProvider, } from "./payment-provider.factory.js";
export class PaymentService {
    ...
    async createIntent(orderId, payload, actorUserId) {
        if (!isPaymentIntentApiEnabled() && !this.providerOverride) {
            throw new AppError(503, "ONLINE_PAYMENTS_DISABLED", {
                message: "Online payment collection is not currently enabled. Payment milestones can still be recorded manually by authorized users.",
            });
        }
    ...
    async handleWebhook(body) {
        ...
        if (status === "succeeded" || eventType === "payment.succeeded") {
            const kind = (body.milestoneKind ?? "DEPOSIT_PAID");
