...
export class AuctionEngine {
    prisma;
    service;
    constructor(prisma, service = new CommodityBidService(prisma)) {
        this.prisma = prisma;
        this.service = service;
    }
    /** Validate reverse-auction bid: must beat current lowest on lot. */
    async validateBid(tx, workspaceId, lotId, unitPrice, supplierUserId, isRevise) {
        ...
        if (!isRevise && existing && !existing.withdrawnAt)
            throw new Error("BID_EXISTS");
        if (isRevise && (!existing || existing.withdrawnAt))
            throw new Error("NO_BID_TO_REVISE");
    ...
    emitLowestUpdated(workspaceId, lotId, unitPrice) {
    ...
    emitAuctionEvent(event, workspaceId, extra = {}) {
        socketBus.scheduleEmit(() => {
    ...
    async processScheduledInvitations() {
    ...
    async processInvitationDeadlines() {
        const now = new Date();
        const rows = await this.prisma.workspace.findMany({
            where: {
                type: "COMMODITYBID",
    ...
    async fireWarning(wsId, action, socketEvent) {
    ...
    async inviteSuppliers(tx, workspaceId, supplierIds, invitedById) {
...
export function createAuctionEngine(prisma) {
    return new AuctionEngine(prisma);
}
//# sourceMappingURL=auction-engine.js.map