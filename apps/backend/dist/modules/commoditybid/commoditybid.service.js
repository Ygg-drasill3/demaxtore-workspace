...
import { findCommodityBidTransition, } from "@dmx/contracts/commoditybid.fsm";
...
export class CommodityBidService {
    ...
    async applyTransition(input) {
        ...
        void (async () => {
            const { emitFromFsmAuditEvent, bootstrapSpawnedOrdersForParent } = await import("../conversation-hub/conversation-hub.hooks.js");
            ...
            if (transition?.auditEvent && result.timelineEventId !== "(idempotent-replay)") {
                emitFromFsmAuditEvent(this.prisma, "COMMODITYBID", workspaceId, transition.auditEvent, actor.role === "SYSTEM" ? null : actor.id);
            }
    ...
    async checkAllAwardsFinalised(_workspaceId) { }
    /**
    ...
    async runActionSideEffects(tx, transition, ws, payload, actor) {
        ...
        switch (transition.action) {
            case "schedule_auction": {
                ...
                await tx.commodityBidDetails.update({
                    ...
                    data: {
                        ...
                        auctionRules: {
                            supplierUserIds: payload.supplierUserIds,
                            minBidImprovement: payload.minBidImprovement ?? 0,
                        },
                    },
                });
                await tx.workspace.update({
            ...
            case "start_invitations": {
                const rules = (ws.commodityBidDetails?.auctionRules ?? {});
                const supplierIds = rules.supplierUserIds ?? payload.supplierUserIds ?? [];
            ...
            case "revise_bid_lot": {
                ...
                await auctionEngine.recordBidEvent(tx, ws.id, lotId, actor.id, unitPrice, transition.action === "submit_bid_lot" ? "SUBMITTED" : "REVISED");
                await tx.commodityBidDetails.update({
                    where: { workspaceId: ws.id },
...
function generateBidderCode(used) {
    for (let i = 0; i < 200; i++) {
...
//# sourceMappingURL=commoditybid.service.js.map