...
import { issuePasswordlessLinkInternal, revokeSupersededDeliveryTokens, reuseDeliveryPasswordlessLink, } from "../passwordless-access/passwordless-access.service.js";
import { getEmailBridgeProvider } from "./providers/email-bridge-provider.factory.js";
import { buildOperationalEmailTemplate, buildOpenTrackingUrl, maskEmailForLog, } from "./email-bridge.templates.js";
...
export function computeEmailRetryAt(retryCount, now = new Date()) {
...
async function shouldDeliverEmail(userId, centerType) {
...
async function resolvePartyLabels(auditWorkspaceId) {
    if (!auditWorkspaceId)
...
async function assertActiveParticipant(userId, auditWorkspaceId) {
    if (!auditWorkspaceId)
        return false;
...
async function resolveDeliveryPasswordlessLink(deliveryId, userId, commType, commId, existingTokenId, isRetry) {
...
async function updateDeliveryStatus(deliveryId, currentStatus, target, data) {
...
export async function processEmailBridgeDelivery(notificationId) {
    if (!bridgeEnabled())
        return;
    ...
    if (!isEmailDeliverableType(shape.centerType))
        return;
    ...
    let link;
    try {
        link = await resolveDeliveryPasswordlessLink(delivery.id, notification.userId, commType, commId, delivery.passwordlessTokenId, isRetry);
    }
    catch (err) {
    ...
    await updateDeliveryStatus(delivery.id, delivery.status, "FAILED", {
        ...
        providerResponse: sanitizeProviderResponse({ error: result.error, ...sanitizedRaw }),
    });
    logger.warn({ notificationId, error: result.error, retryCount }, "[Email Bridge] delivery failed — notification remains in Workspace");
}
...
export async function recordEmailOpen(deliveryId) {
...
export async function getEmailDeliveryForNotification(notificationId) {
...
export const OPEN_TRACKING_GIF = Buffer.from("R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7", "base64");
//# sourceMappingURL=email-bridge.service.js.map