import { CreateFreightRequestPayload, SubmitFreightOfferPayload, ReviseFreightOfferPayload, WithdrawFreightOfferPayload, SelectFreightOfferPayload, CancelFreightRequestPayload, } from "@dmx/contracts/freightiq.zod";
import { deriveFreightIqExecutionState, nextFreightIqExecutionAction, } from "@dmx/contracts/freightiq-execution";
import { SocketEvents } from "@dmx/contracts/socket-events";
import { socketBus } from "../../realtime/socket-bus.js";
import { AppError } from "../../utils/httpErrors.js";
import { assertFreightActionRole, canAccessFreightForOrder, isOrderEligibleForFreight, } from "./freightiq.policy.js";
...
const TERMINAL = ["CANCELLED", "EXPIRED", "CONVERTED_TO_SHIPMENT"];
export class FreightIqService {
    ...
    async getSummary(orderId) {
        ...
        const comparable = request.offers.filter((o) => o.status === "ACTIVE" || o.status === "REVISED");
        const displayOffers = request.offers.filter((o) => o.status === "ACTIVE" || o.status === "REVISED" || o.status === "SELECTED");
    ...
    async applyFreightAction(orderId, action, actor, payload = {}, ctx) {
    ...
    async getOpsOverview() {
        ...
        const offerSelectedAwaitingBooking = [];
        const bookingAwaitingConfirmation = [];
        const confirmedAwaitingContainer = [];
        const shipmentAwaitingTracking = [];
        for (const r of selectedWithShip) {
            ...
            if (!booking || booking === "DRAFT" || booking === "REQUESTED") {
                if (!booking || booking === "DRAFT")
                    offerSelectedAwaitingBooking.push(row);
                else
                ...
                if ((sw?.containers.length ?? 0) === 0)
                    confirmedAwaitingContainer.push(row);
                if (!sw?.trackingLinkedAt)
                    shipmentAwaitingTracking.push(row);
            }
        }
        return {
    ...
    async createRequest(orderId, actor, input, ctx) {
        ...
        if (existing)
            throw new AppError(409, "FREIGHT_REQUEST_ALREADY_OPEN");
        ...
        if (!buyer || !supplier)
            throw new AppError(409, "ORDER_PARTICIPANTS_INCOMPLETE");
        await this.db.$transaction(async (tx) => {
            const orgId = await import("../mixed-container/mc-organization-sync.service.js").then((m) => m.resolveOrganizationWorkspaceId(tx, { orderId }));
            ...
            if (orgId) {
                const { bridgeModuleEventToOrganization, bindOrganizationToFreightRequest } = await import("../mixed-container/mc-organization-sync.service.js");
    ...
    async submitOffer(orderId, actor, input, ctx) {
    ...
    async reviseOffer(orderId, actor, input, ctx) {
        const fr = await this.requireOpenRequest(orderId);
        const offer = await this.db.freightOffer.findUnique({ where: { id: input.offerId } });
        if (!offer || offer.freightRequestId !== fr.id)
    ...
    async withdrawOffer(orderId, actor, input, ctx) {
        const fr = await this.requireOpenRequest(orderId);
        const offer = await this.db.freightOffer.findUnique({ where: { id: input.offerId } });
        if (!offer || offer.freightRequestId !== fr.id)
            throw new AppError(404, "OFFER_NOT_FOUND");
    ...
    async selectOffer(orderId, actor, input, ctx) {
        ...
        if (existingConverted?.selection) {
            if (existingConverted.selection.offerId === input.offerId)
                return; // idempotent
            throw new AppError(409, "ALREADY_SELECTED");
        }
        const fr = await this.requireOpenRequest(orderId);
        if (fr.selection) {
            if (fr.selection.offerId === input.offerId)
                return;
            throw new AppError(409, "ALREADY_SELECTED");
        }
        ...
        await this.db.$transaction(async (tx) => {
            ...
            const orgId = await import("../mixed-container/mc-organization-sync.service.js").then((m) => m.resolveOrganizationWorkspaceId(tx, { orderId }));
            if (orgId) {
                const { bridgeModuleEventToOrganization } = await import("../mixed-container/mc-organization-sync.service.js");
                await bridgeModuleEventToOrganization(tx, {
                    organizationWorkspaceId: orgId,
                    orderId,
                    sourceModule: "FREIGHTIQ",
    ...
    async cancelRequest(orderId, actor, input, ctx) {
        const fr = await this.requireOpenRequest(orderId);
        await this.db.$transaction(async (tx) => {
            await tx.freightRequest.update({
                where: { id: fr.id },
     ...
     */
    async proceedToBooking(orderId, actor, ctx) {
        ...
        if (!fr)
            throw new AppError(404, "FREIGHT_REQUEST_NOT_FOUND");
        if (!fr.selection)
            throw new AppError(409, "NO_SELECTION");
        const shipmentWorkspaceId = fr.selection.shipmentWorkspaceId;
        if (!shipmentWorkspaceId)
            throw new AppError(409, "SHIPMENT_NOT_LINKED");
        ...
        if (!status || status === "DRAFT") {
            ...
            const role = actor.role === "SYSTEM" || actor.role === "SUPER_ADMIN" || actor.role === "SALES_CONTROL"
                ? "ADMIN"
                : actor.role;
            await ops.transitionBooking(shipmentWorkspaceId, { id: actor.id, email: actor.email, role }, { toStatus: "REQUESTED", reason: "FreightIQ proceed_to_booking" });
        }
    ...
    async requireOpenRequest(orderId) {
        const fr = await this.activeRequest(orderId);
        if (!fr)
            throw new AppError(404, "FREIGHT_REQUEST_NOT_FOUND");
        return fr;
    }
    comparisonHints(offers) {
        if (offers.length === 0) {
            return { lowestPriceOfferId: null, fastestTransitOfferId: null, expiringSoonOfferIds: [] };
        }
        const sortedPrice = [...offers].sort((a, b) => displayPriceForOffer(a) - displayPriceForOffer(b));
    ...
    async recordAudit(tx, orderId, actor, action, payload, ctx) {
    ...
    async recordTimeline(tx, orderId, actorUserId, eventType, payload) {
...
function mapRequest(r, orderRef) {
    return {
        id: r.id,
        orderId: r.orderId,
        orderRef,
...
//# sourceMappingURL=freightiq.service.js.map