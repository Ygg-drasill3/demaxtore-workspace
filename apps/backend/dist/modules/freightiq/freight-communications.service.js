import { SendFreightCommunicationsPayload, IntakeFreightOfferPayload, MarkCommunicationRespondedPayload, } from "@dmx/contracts/freight-communications.zod";
...
export class FreightCommunicationsService {
    ...
    async enrichSummary(orderId, base, actor) {
        if (!base.request)
            return { ...base, communications: [], emailTemplate: null };
        ...
        const display = dbOffers.filter((o) => o.status === "ACTIVE" || o.status === "REVISED" || o.status === "SELECTED");
        ...
        if (actor.role === "ADMIN" && merged.request) {
            merged.marginIntakeHint = await this.marginPolicies.suggestMargin(merged.request.pol, merged.request.pod);
        }
        return this.commercial.applyRoleToSummary(merged, actor);
    }
    async sendCommunications(orderId, actor, raw, ctx) {
    ...
    async intakeOffer(orderId, actor, raw, ctx) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN_ROLE");
        if (!(await canAccessFreightForOrder(this.db, actor, orderId))) {
            throw new AppError(403, "FORBIDDEN");
        ...
        const appliedMargin = input.freightiqMarginUsd ??
        ...
        await this.db.$transaction(async (tx) => {
            ...
            await this.marginPolicies.recordMarginOverride(tx, {
                orderId,
                actor,
                offerId,
    ...
    async markCommunicationResponded(orderId, actor, raw, ctx) {
        if (actor.role !== "ADMIN")
            throw new AppError(403, "FORBIDDEN_ROLE");
        ...
        if (!comm || comm.freightRequestId !== fr.id)
            throw new AppError(404, "COMMUNICATION_NOT_FOUND");
        await this.db.$transaction(async (tx) => {
            await tx.freightRequestCommunication.update({
                where: { id: comm.id },
                data: {
    ...
    async previewTemplate(orderId, requestedReplyDate, incoterm) {
        return this.buildTemplateForOrder(orderId, requestedReplyDate, incoterm);
    }
    async buildTemplateForOrder(orderId, requestedReplyDate, incoterm) {
...
function extendedComparisonHints(offers) {
    if (offers.length === 0) {
        return {
            earliestEtdOfferId: null,
            closestCutOffOfferId: null,
        };
    }
...
//# sourceMappingURL=freight-communications.service.js.map