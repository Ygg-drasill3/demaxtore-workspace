import { CreateMarginPolicyPayload, UpdateMarginPolicyPayload, } from "@dmx/contracts/freight-analytics.zod";
...
export class FreightMarginPolicyService {
    ...
    async opsAuditWorkspace() {
        ...
        if (!ws)
            throw new AppError(500, "NO_AUDIT_WORKSPACE");
        return ws;
    }
    async listPolicies() {
    ...
    async createPolicy(actor, raw, ctx) {
        ...
        const row = await this.db.$transaction(async (tx) => {
            ...
            await tx.auditLog.create({
                data: {
                    ...
                    payload: { policyId: created.id, name: input.name },
                    ipAddress: ctx?.ip,
    ...
    async updatePolicy(policyId, actor, raw, ctx) {
        ...
        if (!existing)
            throw new AppError(404, "POLICY_NOT_FOUND");
        const anchor = await this.opsAuditWorkspace();
        const row = await this.db.$transaction(async (tx) => {
            ...
            await tx.auditLog.create({
                data: {
                    ...
                    fromState: anchor.state,
                    toState: anchor.state,
                    payload: { policyId, changes: input },
                    ipAddress: ctx?.ip,
                    userAgent: ctx?.userAgent,
                },
            });
    ...
    async recordMarginOverride(tx, params, ctx) {
        if (params.appliedUsd === params.suggestedUsd)
            return;
...
//# sourceMappingURL=freight-margin-policy.service.js.map