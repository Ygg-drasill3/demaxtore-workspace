...
import { CatalogRfqFormFields, buildCatalogProductDescription, catalogIntakeFromIngestBody, } from "@dmx/contracts/catalog-rfq-intake";
...
function extractTargetMarket(description, intake) {
    if (intake.deliveryLocation)
        return clip(intake.deliveryLocation, 120);
    const m = description.match(/Delivery location:\s*(.+)/i);
    if (m?.[1])
        return clip(m[1], 120);
    const dest = description.match(/Destination:\s*(.+)/i);
    if (dest?.[1])
        return clip(dest[1], 120);
...
export function parseLineItems(description, category, qty, unit) {
    const items = [];
    const section = description.match(new RegExp(`Line items:\\n([\\s\\S]*?)(?=${LINE_ITEMS_SECTION_END.source}|$)`, "i"));
    if (section?.[1]) {
...
function enrichIntakeFromDescription(intake, description) {
    const desc = description.trim();
    if (!desc)
        return intake;
    const pick = (re) => desc.match(re)?.[1]?.trim();
    return {
        ...intake,
...
function supplementalBlocks(rawDescription) {
...
function toDraftInput(body) {
    ...
    const title = clip(body.title?.trim()
        || (company ? `${company} — DeMaxtore catalog` : `${category} — DeMaxtore catalog`), 200);
    const productDescription = /(?:Catalog request|Price quotation Request):/i.test(rawDesc)
        ? clip(rawDesc, 5000)
        : clip(buildCatalogProductDescription(intake, supplementalBlocks(rawDesc)), 5000);
...
async function resolveBuyerActor(contactEmail) {
    ...
    if (user)
        return { id: user.id, email: user.email, role: "BUYER" };
    ...
    if (fallbackUser)
        return { id: fallbackUser.id, email: fallbackUser.email, role: "BUYER" };
    throw new AppError(503, "CATALOG_RFQ_BUYER_NOT_CONFIGURED");
}
async function findExistingWorkspaceId(sessionId) {
...
export async function ingestCatalogRfq(raw) {
    ...
    return { ok: true, duplicate: false, workspace: dto };
}
//# sourceMappingURL=catalog-rfq-ingest.service.js.map