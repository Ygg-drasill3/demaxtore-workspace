...
import { listWorkspaceRfqsForEmbed, signFreightiqEmbedListToken, } from "./freightiq-workspace-rfqs.js";
...
function buildPanelEmbedUrl(next, token, opts) {
...
function rewriteNextForSyncedRfq(next, freightiqRfqId) {
    const qIdx = next.indexOf("?");
    const path = qIdx >= 0 ? next.slice(0, qIdx) : next;
...
function workspaceRfqAccessWhere(workspaceRfqId, actorUserId, role) {
    return {
        id: workspaceRfqId,
        type: "RFQ",
...
async function syncWorkspaceRfqToFreightIq(freightToken, workspaceRfqId, actorUserId, actorRole) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            logger.warn({ workspaceRfqId, status: res.status, body: body.slice(0, 500), apiBase: API_BASE }, "FreightIQ workspace RFQ sync failed");
            return null;
        }
...
async function syncWorkspaceOrderToFreightIq(freightToken, orderWorkspaceId, actorUserId, actorRole) {
    const ws = await prisma.workspace.findFirst({
        where: {
            id: orderWorkspaceId,
            type: "ORDER",
    ...
    try {
        ...
        if (!res.ok) {
            const body = await res.text().catch(() => "");
            logger.warn({ orderWorkspaceId, status: res.status, body: body.slice(0, 500), apiBase: API_BASE }, "FreightIQ workspace order sync failed");
        }
    }
    catch (err) {
        logger.warn({ err, orderWorkspaceId, apiBase: API_BASE }, "FreightIQ workspace order sync request failed");
    }
}
/** Workspace oturumu → FreightIQ iframe SSO token */
export const freightiqSso = [
    requireAuth,
    asyncHandler(async (req, res) => {
        ...
        if (embed)
            bridgeParams.set("embed", embed);
        if (compact)
            bridgeParams.set("compact", compact);
        ...
        let workspaceRfqs = embed === "workspace" ? await listWorkspaceRfqsForEmbed(authUser.id, dbUser.role) : [];
        const wsListToken = embed === "workspace" ? signFreightiqEmbedListToken(authUser.id, dbUser.role) : "";
        ...
        try {
            ...
            if (bridgeRes.ok) {
                ...
                if (bridged.token) {
                    ...
                    if (workspaceRfqId) {
                        const fiRfqId = await syncWorkspaceRfqToFreightIq(bridged.token, workspaceRfqId, authUser.id, dbUser.role);
                        if (fiRfqId)
                            resolvedNext = rewriteNextForSyncedRfq(next, fiRfqId);
                    }
                ...
                logger.warn({ status: bridgeRes.status, apiBase: API_BASE }, "FreightIQ auth bridge returned non-OK status");
            }
        }
        catch (err) {
...
//# sourceMappingURL=freightiq-sso.js.map