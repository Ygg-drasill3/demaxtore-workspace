...
async function assertIngestToken(req) {
    const token = env.CATALOG_RFQ_INGEST_TOKEN?.trim();
    if (!token)
        throw new AppError(503, "CATALOG_RFQ_INGEST_NOT_CONFIGURED");
...
publicCatalogRfqRouter.post("/catalog-rfq", catalogRfqIngestLimiter, asyncHandler(async (req, res) => {
    const ip = clientKey(req);
...
catalogRfqContextRouter.get("/catalog-rfq/context", requireAuth, asyncHandler(async (req, res) => {
    const user = req.user;
...
//# sourceMappingURL=catalog-rfq-ingest.routes.js.map