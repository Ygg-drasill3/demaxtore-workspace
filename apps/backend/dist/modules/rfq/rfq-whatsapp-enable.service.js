...
import { participantKeyForUser, } from "../unified-messaging/unified-messaging.constants.js";
...
function resolveRfqOutreachCredentials(buyerCredentials) {
    if (buyerCredentials)
        return buyerCredentials;
...
export async function enableRfqWhatsApp(db, rfqWorkspaceId) {
    ...
    const focusedSupplier = (preferredSupplierId
    ...
    const keepUserIds = new Set([buyerOwner?.userId, supplierUserId].filter((id) => Boolean(id)));
    for (const p of workspaceParts) {
        ...
        if (phone && shouldBeActive) {
            ...
            if (p.userId === supplierUserId) {
                ...
                const canBindRfq = !existingWa?.workspaceRfqId || existingWa.workspaceRfqId === rfqWorkspaceId;
    ...
    const baseMeta = typeof conv.metadata === "object" && conv.metadata && !Array.isArray(conv.metadata)
        ? conv.metadata
        : {};
    ...
    if (hasSupplierPhone && supplierWhatsAppPhone && outreachCredentials) {
        void sendRfqWhatsAppOpeningInvite(db, rfqWorkspaceId, supplierWhatsAppPhone, ws.externalRef, outreachCredentials).catch(() => undefined);
    }
}
//# sourceMappingURL=rfq-whatsapp-enable.service.js.map