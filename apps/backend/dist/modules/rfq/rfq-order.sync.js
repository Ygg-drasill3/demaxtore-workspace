...
export async function closeParentRfqWhenOrderCloses(tx, orderWorkspaceId, orderExternalRef) {
    const orderWs = await tx.workspace.findUnique({
        where: { id: orderWorkspaceId },
        select: { spawnedFromId: true, type: true },
    });
    if (!orderWs?.spawnedFromId || orderWs.type !== "ORDER")
        return false;
    ...
    if (!parent || parent.type !== "RFQ" || parent.state !== "PO_ISSUED")
        return false;
...
export async function repairRfqStateIfOrderClosed(db, rfqWorkspaceId) {
    const rfq = await db.workspace.findUnique({
        where: { id: rfqWorkspaceId },
        select: { state: true, type: true },
    });
    if (!rfq || rfq.type !== "RFQ" || rfq.state !== "PO_ISSUED")
        return false;
    ...
    if (!order)
        return false;
    return db.$transaction((tx) => closeParentRfqWhenOrderCloses(tx, order.id, order.externalRef));
}
//# sourceMappingURL=rfq-order.sync.js.map