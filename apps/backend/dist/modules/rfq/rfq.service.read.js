...
function dtoFromWorkspace(ws) {
    ...
    const catalogRaw = meta && typeof meta === "object" && meta !== null && "catalogIntake" in meta
        ? meta.catalogIntake
        : undefined;
...
RfqService.prototype.editDraft = async function (wsId, input, actor) {
    ...
    if (ws.state !== "RFQ_DRAFT")
        throw new AppError(409, "RFQ_NOT_DRAFT");
    if (ws.createdById !== actor.id && actor.role !== "ADMIN")
...
RfqService.prototype.list = async function (query, actor) {
    ...
    let myQuotedWorkspaceIds = new Set();
...
RfqService.prototype.toDTO = async function (ws, actor) {
    const wsFull = ws;
    ...
    const rawLines = base.lineItems ?? [];
    const lineItems = await Promise.all(rawLines.map(async (li) => ({
        ...li,
    ...
    if (actor?.role === "SUPPLIER") {
        ...
        return redactRfqDtoForSupplier({ ...enriched, allowedQuoteLineItemIds }, actor.id);
    }
    return enriched;
};
RfqService.prototype.fetchDTO = async function (wsId, actor) {
    ...
    if (ws.state === "PO_ISSUED") {
        ...
        if (repaired) {
            ...
            if (refreshed)
                return this.toDTO(refreshed, actor);
        }
    }
...
RfqService.prototype.buildNextActionContext = async function (ws, actor) {
    ...
    const isCounterparty = ws.participants.some((p) => p.userId === actor.id && p.participantRole === "COUNTERPARTY");
    return {
        state: ws.state,
        actorRole,
        ...
        hasQuotationFromUser: (await this.prisma.quotation.count({
...
export { RfqService } from "./rfq.service.js";
export { canAccessRfq } from "./rfq.policy.js";
//# sourceMappingURL=rfq.service.read.js.map