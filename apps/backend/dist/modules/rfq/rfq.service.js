...
import { resolveRecipients } from "./rfq.notifications.js";
import { PRECONDITIONS } from "./rfq.preconditions.js";
...
export class RfqService {
    ...
    async applyTransition(input) {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            if (transition.requiredParticipant &&
                actor.role !== "ADMIN" &&
                actor.role !== "SYSTEM") {
                ...
                if (transition.requiredParticipant !== "ANY" &&
            ...
            if (action === "assign_suppliers" || action === "add_more_suppliers") {
                ...
                if (missing.length)
                    throw new AppError(404, "RFQ_SUPPLIER_NOT_FOUND", { missing });
            ...
            if (transition.to !== "*" && transition.to !== currentState) {
            ...
            const timelineEvent = await tx.timelineEvent.create({
                data: {
                    ...
                    payload: { ...payload, reason, idempotencyKey },
                },
            });
            ...
            const auditLog = await tx.auditLog.create({
                data: {
                    ...
                    toState: transition.to === "*" ? currentState : transition.to,
                    reason,
            ...
            const newState = transition.to === "*" ? currentState : transition.to;
            ...
            socketBus.scheduleEmit(() => {
                ...
                for (const n of createdNotifications) {
                    ...
                    if (n.userId)
                        socketBus.emitToUser(n.userId, "notification:new", { notification: dto });
                    if (n.role)
                        socketBus.emitToRole(n.role, "notification:new", { notification: dto });
                }
                void import("../notification-center/delivery.dispatcher.js").then(({ scheduleNotificationChannelDeliveries }) => {
                    scheduleNotificationChannelDeliveries(createdNotifications
                        .filter((n) => n.userId)
            ...
            void (async () => {
                try {
                    ...
                    if (payload.replyToMessageId)
                        registerWiredSurface("rfq_clarification_reply");
        ...
        void (async () => {
            const { emitFromRfqAuditEvent, bootstrapSpawnedOrdersForParent } = await import("../conversation-hub/conversation-hub.hooks.js");
     ...
     */
    async runActionSideEffects(tx, transition, ws, payload, actor) {
        switch (transition.action) {
            case "assign_suppliers":
            case "add_more_suppliers": {
                const supplierIds = payload.supplierUserIds ?? [];
            ...
            case "extend_deadline": {
                ...
                const addedDays = Math.ceil((newDeadline.getTime() - oldDeadline.getTime()) / (1000 * 60 * 60 * 24));
                await tx.workspace.update({
                    where: { id: ws.id },
                    data: {
            ...
            case "submit_proforma": {
                await tx.rfqDetails.update({
                    ...
                    data: {
                        proformaFileUrl: payload.proformaFileUrl,
            ...
            case "select_supplier": {
                await tx.rfqDetails.update({
                    where: { workspaceId: ws.id },
                    data: {
                        selectedSupplierUserId: payload.supplierUserId,
                        selectedQuotationId: payload.quotationId,
                        selectionRationale: payload.rationale ?? null,
                    },
                });
                ...
                await autoGenerateFreightEstimateInTx(tx, ws.id, actor.id, {
                    supplierId: payload.supplierUserId,
                });
                return;
            }
            case "add_observer": {
                const uid = payload.observerUserId;
                if (!uid)
                    throw new AppError(400, "OBSERVER_USER_REQUIRED");
                await tx.workspaceParticipant.upsert({
                    where: {
            ...
            case "post_clarification": {
                const body = String(payload.message ?? "").trim();
                if (!body)
                    throw new AppError(400, "CLARIFICATION_MESSAGE_REQUIRED");
                ...
                const mentionedUserIds = Array.isArray(payload.mentionedUserIds)
                    ? payload.mentionedUserIds.slice(0, 20)
                    : [];
                const attachmentIds = Array.isArray(payload.attachmentIds)
                    ? payload.attachmentIds.slice(0, 10)
                    : [];
            ...
            case "issue_po": {
                ...
                if (!details?.selectedSupplierUserId)
                    throw new AppError(400, "NO_SELECTED_SUPPLIER");
                ...
                const poId = await createPurchaseOrderOnOrderSpawn(tx, {
                    ...
                    lines: lineItems.map((li, i) => ({
                        ...
                        unitPrice: Number("unitPrice" in li ? li.unitPrice : li.targetPrice ?? 1),
                        sku: `LINE-${i + 1}`,
                    })),
                    actorUserId: actor.id,
                    actorEmail: actor.email,
                    ...
                    documentFileName: issue.mode === "manual" && issue.poFileUrl
                        ? decodeURIComponent(issue.poFileUrl.split("/").pop() ?? "uploaded-po.pdf")
                        : null,
                    issueReason: issue.mode === "manual"
