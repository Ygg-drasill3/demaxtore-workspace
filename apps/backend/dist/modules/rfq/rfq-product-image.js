...
].map((fn) => fn()).filter((p) => Boolean(p?.trim()));
...
function dedupeSubtokens(tokens) {
    const phrases = tokens.filter((t) => t.includes(" "));
    const singles = tokens.filter((t) => !t.includes(" "));
    const keptSingles = singles.filter((tok) => !singles.some((other) => other !== tok && other.length > tok.length && other.includes(tok)));
...
export function extractProductSearchTerms(input) {
    const terms = [];
    const desc = input.productDescription ?? "";
    const lineMatch = desc.match(/^- (.+?) \(product_id:/m);
    if (lineMatch?.[1])
        terms.push(lineMatch[1].trim());
    for (const li of input.lineItems ?? []) {
        if (li.description?.trim())
            terms.push(li.description.trim());
    }
    ...
    if (input.productCategory?.trim())
        terms.push(input.productCategory.trim());
    return expandSearchTerms(terms);
}
export function scoreProductMatch(productName, search) {
    ...
    for (const raw of phrases) {
        ...
        else if (norm.length >= 4 && name.includes(norm))
            best = Math.max(best, 35 + Math.min(norm.length, 20));
        else if (name.length >= 4 && norm.includes(name))
            best = Math.max(best, 25 + Math.min(name.length, 15));
    }
    for (const tok of searchTokens) {
        ...
        if (name === tok)
            ...
            best = Math.max(best, 30 + Math.min(tok.length, 10));
    }
    return best;
}
function pickBestMatch(products, search) {
    if (!products.length || !search.trim())
        return null;
    let best = null;
    for (const p of products) {
        const score = scoreProductMatch(p.name ?? "", search);
        const image = p.image?.trim();
        if (!image || score < MIN_MATCH_SCORE)
            continue;
...
export function resolveFromMetadata(metadata) {
    ...
    if (intake && typeof intake === "object") {
        const img = intake.productImageUrl;
...
async function resolveFromMixedContainerCatalog(search) {
...
async function resolveFromRfqAttachments(workspaceId) {
...
function resolveFallbackJsonPath() {
...
async function resolveFromLocalCatalogFallback(search) {
    const products = await loadFallbackProducts();
    return pickBestMatch(products, search);
}
async function resolveFromDemaxtoreCatalogApi(search) {
    const terms = expandSearchTerms([search]);
    const base = (env.DEMAXTORE_CATALOG_API_URL ?? "https://demaxtore.com").replace(/\/$/, "");
    for (const term of terms) {
...
async function resolveByTerms(input) {
...
export async function resolveLineItemProductImageUrl(workspaceId, description) {
 ...
 */
export async function resolveRfqProductImageUrl(input) {
...
//# sourceMappingURL=rfq-product-image.js.map