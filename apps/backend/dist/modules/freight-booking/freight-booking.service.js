...
import { canAccessFreightBooking, canConfirmBooking, canManageFreightBooking, canSelectCarrier, canSubmitForecast, } from "./freight-booking.policy.js";
...
function scoreCarrierOptions(options, cargoReadyDate) {
    if (options.length === 0)
        return [];
...
export class FreightBookingEngineService {
    ...
    async getById(actor, id) {
        ...
        if (!row)
            throw new AppError(404, "FREIGHT_BOOKING_NOT_FOUND");
    ...
    async create(actor, payload) {
        ...
        if (!canSubmitForecast(actor))
            throw new AppError(403, "FORBIDDEN");
        return this.submitForecast(actor, payload, ctx.supplierId);
    }
    async selectCarrier(actor, bookingId, payload) {
        if (!canSelectCarrier(actor))
            throw new AppError(403, "FORBIDDEN");
        ...
        if (!option)
            throw new AppError(404, "CARRIER_OPTION_NOT_FOUND");
        if (option.status === "EXPIRED")
            throw new AppError(409, "CARRIER_OPTION_EXPIRED");
        const run = async (tx) => {
    ...
    async confirm(actor, bookingId) {
        if (!canConfirmBooking(actor))
            throw new AppError(403, "FORBIDDEN");
        const booking = await this.db.freightBooking.findUnique({ where: { id: bookingId } });
        if (!booking)
            throw new AppError(404, "FREIGHT_BOOKING_NOT_FOUND");
        if (booking.status !== "APPROVED")
            throw new AppError(409, "BOOKING_NOT_APPROVED");
        if (!booking.selectedCarrierOptionId)
            throw new AppError(409, "NO_CARRIER_SELECTED");
        const run = async (tx) => {
            const updated = await tx.freightBooking.update({
                where: { id: bookingId },
    ...
    async resolveTradeContext(tradeId) {
    ...
    async submitForecast(actor, payload, supplierId) {
    ...
    async createBookingPlan(actor, payload, ctx) {
        ...
        const run = async (tx) => {
            ...
            const scores = scoreCarrierOptions(optionDrafts.map((o) => ({ freightAmount: o.freightAmount, transitDays: o.transitDays, cutoffDate: o.cutoffDate })), cargoReady);
            let bestIdx = 0;
            let bestScore = -1;
...
//# sourceMappingURL=freight-booking.service.js.map