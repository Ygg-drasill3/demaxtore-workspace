...
import { MessagingChannelDispatcher, WhatsAppChannelAdapter, WorkspaceChannelAdapter, } from "./unified-messaging.channel-adapter.js";
import { UnifiedMessagingErrors } from "./unified-messaging.errors.js";
import { filterMessagesForUser, mapConversationDetail, mapConversationSummary, mapMessage, } from "./unified-messaging.mapper.js";
...
import { assertWhatsAppRecipientAllowed, isSupplierConversation, resolveWhatsAppTargetPhone, } from "./unified-messaging.whatsapp-target.js";
import { assertWhatsAppOutboundAllowed } from "./unified-messaging.whatsapp-outbound.js";
import { assertBuyerCredentialIsolation, requireBuyerWhatsAppCredentials, resolveBuyerIdForConversation, } from "../whatsapp-business/whatsapp-business-credential.resolver.js";
...
export class UnifiedMessagingService {
    ...
    async listConversations(user, filters) {
        ...
        return {
            items: page.items.map((c) => mapConversationSummary(c, c.unreadCount ?? 0)),
    ...
    async getConversation(user, conversationId) {
        ...
        if (!row)
            throw UnifiedMessagingErrors.conversationNotFound();
    ...
    async listMessages(user, conversationId, cursor, limit) {
        await this.policy.assertConversationAccess(user, conversationId);
    ...
    async createMessage(user, conversationId, input) {
        await this.policy.assertConversationAccess(user, conversationId);
        if (!(await this.policy.canSendExternalMessage(user, conversationId))) {
            throw UnifiedMessagingErrors.cannotAccessConversation();
        }
        if (input.clientMessageId) {
            const existing = await this.repo.findMessageByClientId(conversationId, user.id, input.clientMessageId);
            if (existing)
        ...
        const channel = supplierConversation && user.role === "SUPPLIER"
            ? "WORKSPACE"
        ...
        if (channel === "WHATSAPP") {
            ...
            const isSupplierThreadMirror = supplierConversation &&
                user.role === "SUPPLIER" &&
                Boolean(senderWa) &&
            ...
            await assertWhatsAppOutboundAllowed(this.prisma, phoneE164, whatsappCredentials?.phoneNumberId, whatsappCredentials ?? undefined);
        }
        let message = await this.writeDispatcher.persistMessageWithOutbox(user, "unified_api", {
            conversationId,
        ...
        if (channel === "WHATSAPP") {
            ...
            try {
                ...
                const reason = err instanceof AppError
    ...
    async createInternalNote(user, conversationId, input) {
        ...
        const message = await this.writeDispatcher.persistMessageWithOutbox(user, "unified_api", {
            conversationId,
            authorUserId: user.id,
            body: input.body,
            parentMessageId: input.replyToMessageId,
    ...
    async markConversationRead(user, conversationId) {
        await this.policy.assertConversationAccess(user, conversationId);
        await this.writeDispatcher.dispatchMarkRead(user, "unified_api", "conversation_mark_read", conversationId);
        return { ok: true };
    }
    async addParticipant(user, conversationId, data) {
        ...
        if (existing)
            throw UnifiedMessagingErrors.duplicateParticipant();
        ...
        const row = await this.writeDispatcher.dispatchUnifiedFirst({
            ...
            unified: async (tx) => tx.workspaceConversationParticipant.create({
                data: {
                    conversationId,
                    participantKey,
                    userId: data.userId,
    ...
    async removeParticipant(user, conversationId, participantId) {
        if (!this.policy.canLinkContext(user))
            throw UnifiedMessagingErrors.cannotAccessConversation();
        await this.policy.assertConversationAccess(user, conversationId);
    ...
    async addContext(user, conversationId, input) {
        if (!this.policy.canLinkContext(user))
            throw UnifiedMessagingErrors.cannotLinkContext();
        await this.policy.assertConversationAccess(user, conversationId);
        ...
        const row = await this.writeDispatcher.dispatchUnifiedFirst({
            ...
            unified: async (tx) => tx.conversationContext.create({
                data: {
                    conversationId,
    ...
    async removeContext(user, conversationId, contextId) {
        if (!this.policy.canLinkContext(user))
            throw UnifiedMessagingErrors.cannotLinkContext();
        await this.policy.assertConversationAccess(user, conversationId);
    ...
    async assignConversation(user, conversationId, input) {
        ...
        await this.writeDispatcher.dispatchConversationMutation(user, "unified_api", "conversation_assignment", conversationId, `assign:${conversationId}:${input.assignedUserId}`, { assignedUserId: input.assignedUserId }, "messaging:conversation:assigned");
    ...
    async archiveConversation(user, conversationId) {
        ...
        await this.writeDispatcher.dispatchConversationMutation(user, "unified_api", "archive", conversationId, `archive:${conversationId}`, { isArchived: true, status: "ARCHIVED" }, "messaging:conversation:archived");
        return this.getConversation(user, conversationId);
    }
    async unarchiveConversation(user, conversationId) {
        ...
        await this.writeDispatcher.dispatchConversationMutation(user, "unified_api", "unarchive", conversationId, `unarchive:${conversationId}`, { isArchived: false, status: "ACTIVE" }, "messaging:conversation:updated");
        return this.getConversation(user, conversationId);
    }
    async updatePriority(user, conversationId, priority) {
        if (!this.policy.canAssignConversation(user))
            throw UnifiedMessagingErrors.cannotAssign();
        await this.policy.assertConversationAccess(user, conversationId);
        registerWiredSurface("priority_update");
        await this.writeDispatcher.dispatchConversationMutation(user, "unified_api", "priority_update", conversationId, `priority:${conversationId}:${priority}`, { priority }, "messaging:conversation:updated");
        return this.getConversation(user, conversationId);
    }
    async updateStatus(user, conversationId, status) {
        ...
        await this.writeDispatcher.dispatchConversationMutation(user, "unified_api", "conversation_status_update", conversationId, `status:${conversationId}:${status}`, { status }, "messaging:conversation:updated");
        return this.getConversation(user, conversationId);
    }
    async retryMessage(user, conversationId, messageId) {
        await this.policy.assertConversationAccess(user, conversationId);
        ...
        const retried = await this.writeDispatcher.dispatchUnifiedFirst({
            ...
            unified: async (tx) => tx.workspaceMessage.update({
                where: { id: messageId },
...
//# sourceMappingURL=unified-messaging.service.js.map