...
import { AddContextRequestSchema, AddParticipantRequestSchema, AssignConversationRequestSchema, ConversationListFiltersSchema, CreateConversationRequestSchema, CreateInternalNoteRequestSchema, CreateMessageRequestSchema, MessageListQuerySchema, UpdatePriorityRequestSchema, UpdateStatusRequestSchema, } from "@dmx/contracts/unified-messaging.zod";
const uploadAttachment = multer({
    storage: multer.memoryStorage(),
    limits: { fileSize: 25 * 1024 * 1024 },
    fileFilter: createUploadFileFilter(),
});
const uploadAttachmentSingle = uploadAttachment.single("file");
const router = Router();
...
function requireUnifiedMessagingEnabled(_req, res, next) {
...
router.get("/attachments/:attachmentId", asyncHandler(async (req, res) => {
...
router.get("/:conversationId/messages", asyncHandler(async (req, res) => {
    ...
    res.json(await service().listMessages(req.user, req.params.conversationId, q.cursor, q.limit));
}));
router.post("/:conversationId/messages", asyncHandler(async (req, res) => {
    const body = CreateMessageRequestSchema.parse(req.body);
    const { message, duplicate } = await service().createMessage(req.user, req.params.conversationId, body);
    res.status(duplicate ? 200 : 201).json(message);
}));
router.post("/:conversationId/internal-notes", asyncHandler(async (req, res) => {
...
router.post("/:conversationId/assign", asyncHandler(async (req, res) => {
...
router.post("/:conversationId/archive", asyncHandler(async (req, res) => {
    res.json(await service().archiveConversation(req.user, req.params.conversationId));
}));
router.post("/:conversationId/unarchive", asyncHandler(async (req, res) => {
    res.json(await service().unarchiveConversation(req.user, req.params.conversationId));
}));
router.post("/:conversationId/priority", asyncHandler(async (req, res) => {
...
router.post("/:conversationId/status", asyncHandler(async (req, res) => {
...
router.post("/:conversationId/messages/:messageId/retry", asyncHandler(async (req, res) => {
    res.json(await service().retryMessage(req.user, req.params.conversationId, req.params.messageId));
}));
router.post("/:conversationId/participants", asyncHandler(async (req, res) => {
...
router.delete("/:conversationId/participants/:participantId", asyncHandler(async (req, res) => {
    res.json(await service().removeParticipant(req.user, req.params.conversationId, req.params.participantId));
}));
router.post("/:conversationId/contexts", asyncHandler(async (req, res) => {
...
router.delete("/:conversationId/contexts/:contextId", asyncHandler(async (req, res) => {
    res.json(await service().removeContext(req.user, req.params.conversationId, req.params.contextId));
}));
router.post("/:conversationId/attachments", uploadLimiter, uploadAttachmentSingle, asyncHandler(async (req, res) => {
    const file = req.file;
    if (!file)
...
//# sourceMappingURL=unified-messaging.routes.js.map