...
import { logShadowCompare, recordFallback, recordLatency, recordShadowMismatch, recordShadowReadError, recordShadowReadStart, recordShadowReadSuccess, } from "./legacy-adapter.metrics.js";
...
export async function executeLegacyCompatibleRead(input) {
    ...
    const shouldShadow = cfg.shadowReadEnabled &&
        cfg.readMode === "shadow" &&
        input.unifiedReader &&
...
async function runShadowCompare(input, legacyResult, timeoutMs) {
    const cfg = getLegacyAdapterConfig();
