...
export function compareNormalized(surface, legacy, unified) {
    ...
    if (legacy.messageCount !== unified.messageCount)
        mismatchTypes.push("MESSAGE_COUNT");
    if (legacy.unreadCount !== unified.unreadCount)
        mismatchTypes.push("UNREAD_COUNT");
    if (legacy.participantCount !== unified.participantCount && legacy.participantCount > 0) {
        mismatchTypes.push("PARTICIPANT_COUNT");
    }
    if (legacy.attachmentCount !== unified.attachmentCount)
        mismatchTypes.push("ATTACHMENT_COUNT");
...
//# sourceMappingURL=legacy-adapter.comparator.js.map