...
export function getWhatsAppBusinessPhoneE164() {
...
export function maskPhoneE164(phone) {
...
export function assertWhatsAppRecipientAllowed(recipientPhone, sender, senderPhones) {
    ...
    if (recipient === business) {
        throw Validation("Cannot use the DeMaxtore WhatsApp Business line as message recipient.", { recipientMasked: maskPhoneE164(recipient), blocked: "business_line" });
    }
...
export async function isSupplierConversation(prisma, conversationId) {
...
async function resolveRfqWhatsAppTarget(prisma, workspaceId, sender) {
    ...
    const isSupplierSide = sender.role === "SUPPLIER" || senderPart?.participantRole === "COUNTERPARTY";
    if (isSupplierSide) {
...
export async function resolveWhatsAppTargetPhone(prisma, conversationId, sender) {
    const conv = await prisma.workspaceConversation.findUnique({
        where: { id: conversationId },
        include: {
    ...
    const meta = typeof conv.metadata === "object" && conv.metadata && !Array.isArray(conv.metadata)
        ? conv.metadata
        : {};
    const metaPhone = normalizePhone(typeof meta.rfqSupplierWhatsAppPhone === "string" ? meta.rfqSupplierWhatsAppPhone : null);
...
export function formatWhatsAppPhoneDisplay(phone) {
    const digits = phone.replace(/\D/g, "");
...
//# sourceMappingURL=unified-messaging.whatsapp-target.js.map