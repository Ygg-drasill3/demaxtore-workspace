...
export async function scanWorkspaceCommunicationAlerts(db) {
    let n = 0;
    const now = new Date();
    ...
    for (const q of questions) {
        const resolved = await resolveWorkspace(db, q.conversation.workspaceType, q.conversation.workspaceId);
        if (!resolved)
            continue;
        const nonAuthorReaders = q.readReceipts.filter((r) => r.userId !== q.authorUserId);
        if (nonAuthorReaders.length > 0)
            continue;
    ...
    for (const d of decisions) {
        const resolved = await resolveWorkspace(db, d.conversation.workspaceType, d.conversation.workspaceId);
        if (!resolved)
            continue;
    ...
    for (const note of notes) {
        const resolved = await resolveWorkspace(db, note.conversation.workspaceType, note.conversation.workspaceId);
        if (!resolved)
            continue;
        const followUp = await db.workspaceMessage.findFirst({
            where: {
...
//# sourceMappingURL=communication-alerts.js.map