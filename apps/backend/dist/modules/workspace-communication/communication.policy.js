...
export async function resolveWorkspace(prisma, workspaceType, workspaceId) {
...
export async function canAccessCommWorkspace(prisma, user, workspaceType, workspaceId) {
...
export async function buildVisibilityContext(prisma, resolved) {
    ...
    if (ws.createdById)
        buyerUserIds.add(ws.createdById);
    for (const p of participants) {
        if (p.participantRole === "OWNER") {
            if (p.user.role === "BUYER")
                buyerUserIds.add(p.userId);
            if (p.user.role === "SUPPLIER")
                supplierUserIds.add(p.userId);
        }
...
//# sourceMappingURL=communication.policy.js.map