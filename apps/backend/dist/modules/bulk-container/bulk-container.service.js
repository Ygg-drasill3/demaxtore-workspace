import { BC_MAX_CAPACITY_MT, computeBcCapacityWarnings, findBcTransition, isBcContainerFull, } from "@dmx/contracts/bulk-container.fsm";
...
function validateSpecValues(template, specValues) {
...
export function toBulkContainerDTO(ws) {
...
async function nextBcRef(prisma) {
...
export class BulkContainerService {
    ...
    async findIncompleteOpenContainer(actor) {
        if (actor.role !== "BUYER")
            return null;
        ...
        for (const p of parts) {
            ...
            if (!isBcContainerFull(currentMt, Number(d.maxCapacityMt)))
                return ws.id;
        }
        return null;
    }
    async ensureActiveBuilding(actor) {
        const existing = await this.findIncompleteOpenContainer(actor);
        if (existing)
            return this.fetchDTO(existing);
    ...
    async create(input, actor) {
        ...
        if (openId)
            throw new AppError(409, "OPEN_BULK_CONTAINER_EXISTS", { workspaceId: openId });
    ...
    async addLine(id, input, actor) {
        ...
        const template = product.specTemplate.schema;
        ...
        const packingLink = await assertValidPackingTypeForProduct(this.prisma, "BULK_CONTAINER", input.catalogProductId, input.packingTypeId);
        const current = await this.fetchDTO(id);
        if (current.isFull)
            throw new AppError(409, "CONTAINER_FULL");
    ...
    async updateLine(id, lineId, input, actor) {
        ...
        if (input.specValues) {
            const template = line.catalogProduct.specTemplate.schema;
        ...
        await this.prisma.$transaction(async (tx) => {
            await tx.bulkContainerLine.update({
                ...
                data: {
                    quantityMt: input.quantityMt,
                    ...(input.specValues ? { specValues: input.specValues } : {}),
                },
            });
...
//# sourceMappingURL=bulk-container.service.js.map