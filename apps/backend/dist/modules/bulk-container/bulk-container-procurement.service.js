import { BC_OFFER_VALIDITY_HOURS, findBcTransition, } from "@dmx/contracts/bulk-container.fsm";
...
function summarizeSpec(specValues, template) {
    if (!template) {
...
async function nextBcOfferRef(tx) {
...
function toOfferDTO(ws, offer) {
    const now = Date.now();
    const expiresInSeconds = offer.status === "SENT"
...
export class BulkContainerProcurementService {
    ...
    async getProcurement(id) {
        ...
        return {
            container: toBulkContainerDTO(ws),
            quotes: ws.bcProcurementQuotes.map((q) => ({
                ...
                notes: q.notes,
                specificationSnapshot: q.specificationSnapshot,
            })),
    ...
    async upsertQuote(id, input, actor) {
        ...
        const specValues = line.specValues;
        ...
        const data = {
            ...
            specificationSnapshot: specValues,
    ...
    async expireOffers(actor) {
        ...
        let count = 0;
        const transitionActor = actor.role === "ADMIN"
            ? actor
            : {
        ...
        for (const offer of expired) {
            if (offer.workspace.state !== "BC_BUYER_REVIEW")
                continue;
            await this.prisma.$transaction(async (tx) => {
...
//# sourceMappingURL=bulk-container-procurement.service.js.map