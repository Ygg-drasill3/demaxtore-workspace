...
export class BulkContainerAllocationService {
    ...
    async assertAdminBcWorkspace(id) {
    ...
    lineFullyAllocated(lineId, lineQtyMt, allocations) {
    ...
    async allocationKpis() {
        ...
        return {
            allocationsPending: (map.get("BC_APPROVED") ?? 0) + (map.get("BC_ALLOCATION_IN_PROGRESS") ?? 0),
    ...
    async completeAllocations(id, actor) {
        ...
        const incomplete = ws.bulkContainerLines.filter((l) => !this.lineFullyAllocated(l.id, Number(l.quantityMt), ws.bcSupplierAllocations));
    ...
    async tryBeginPaymentTracking(tx, id, actor) {
        ...
        if (ws.state !== "BC_PROFORMA_PENDING")
            return;
    ...
    async tryMarkExecutionReady(tx, id, actor) {
        ...
        if (ws.state !== "BC_PAYMENT_TRACKING")
            return;
        if (ws.bcSupplierAllocations.length === 0)
            return;
...
//# sourceMappingURL=bulk-container-allocation.service.js.map