...
export function computeTaskPermissions(role, opts) {
    const manager = isPlatformAdminRole(role)
        || role === "OPS_MANAGER"
        || role === "ADMIN"
        || role === "DOCUMENT_CONTROLLER"
        || role === "SALES_CONTROL";
    const partnerRole = role === "SUPPLIER"
        || role === "ORIGIN_AGENT"
        || role === "CUSTOMS_BROKER"
        || role === "TRUCKER";
...
export class OperationalTaskService {
    ...
    async loadUsers(ids) {
        ...
        return new Map(rows.map((u) => [u.id, { id: u.id, name: u.displayName, email: u.email }]));
    }
    toDto(row, users, actor) {
        ...
        const fallback = { id: row.createdById, name: "Unknown", email: null };
        return {
            id: row.id,
            orderId: row.orderId,
    ...
    async list(actor, query) {
        ...
        const users = await this.loadUsers(rows.flatMap((r) => [r.assignedToId, r.createdById, r.completedById].filter(Boolean)));
        return {
    ...
    async summary(actor) {
        const mineFilter = isPlatformAdminRole(actor.role)
            || actor.role === "OPS_MANAGER"
            || actor.role === "LOGISTICS_OPERATOR"
            || actor.role === "DOCUMENT_CONTROLLER"
            ? {}
    ...
    async get(id, actor) {
        ...
        const users = await this.loadUsers([row.assignedToId, row.createdById, row.completedById].filter(Boolean));
    ...
    async patch(id, actor, input) {
        ...
        if (!perms.canUpdateProgress && !perms.canAssign)
            throw new AppError(403, "FORBIDDEN");
    ...
    async start(id, actor) {
        ...
        if (!perms.canUpdateProgress)
            throw new AppError(403, "FORBIDDEN");
        if (!["OPEN", "ASSIGNED"].includes(row.status))
            throw new AppError(409, "TASK_NOT_STARTABLE");
        await this.prisma.operationalTask.update({
            where: { id },
            data: {
                status: "IN_PROGRESS",
    ...
    async complete(id, actor) {
        ...
        void import("../operational-issue/operational-issue.service.js")
            .then(({ OperationalIssueService }) => new OperationalIssueService(this.prisma).suggestResolutionFromTask(id, actor))
            .catch(() => undefined);
        return this.get(id, actor);
    }
    ...
    async addComment(id, actor, input) {
        const row = await this.prisma.operationalTask.findFirst({ where: { id, deletedAt: null } });
        if (!row)
            throw new AppError(404, "TASK_NOT_FOUND");
        await this.assertOrderAccess(actor, row.orderId);
    ...
    async deleteComment(taskId, commentId, actor) {
    ...
    async ensureAutomatedTask(input) {
        ...
        const dueDate = input.dueInDays != null
        ...
        try {
            ...
            await this.writeTimeline(input.orderId, { id: createdById, email: "system@demaxtore.local", role: "ADMIN" }, "task.created", {
                taskId: row.id,
                title: row.title,
                automationKey: input.automationKey,
                status: row.status,
...
//# sourceMappingURL=operational-task.service.js.map