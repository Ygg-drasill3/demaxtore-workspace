...
import { findShipmentTransition, resolveShipmentTargetState, SHIPMENT_SELF_LOOP_ACTIONS, } from "@dmx/contracts/shipment.fsm";
...
import { computeShipmentPermissions, ShipmentWorkspaceOps, } from "./shipment-workspace.ops.js";
export class ShipmentService {
    ...
    async runOneTransition(input) {
        ...
        const txnPromise = this.prisma.$transaction(async (tx) => {
            ...
            if (action === "complete_shipment") {
                const { assertShipmentCompletionAllowed } = await import("../trade-documents/compliance.js");
            ...
            if (transition.action === "resolve_exception") {
                ...
                if (!open)
                    throw new AppError(409, "NO_OPEN_EXCEPTION");
                newState = open.stateBefore;
            }
            ...
            exceptionId = await this.runActionSideEffects(tx, transition, workspaceFull, payload, actor, currentState, reason, newState);
        ...
        if (result.toState === "COMPLETED") {
            const { FreightCommercialService } = await import("../freightiq/commercial/freight-commercial.service.js");
            await new FreightCommercialService(this.prisma).realizeRevenueForShipment(workspaceId, { id: actor.id, email: actor.email, role: actor.role });
        }
        ...
        if (result.timelineEventId !== "(idempotent-replay)") {
            void (async () => {
                ...
                if (transition?.auditEvent) {
                    emitFromFsmAuditEvent(this.prisma, "SHIPMENT", workspaceId, transition.auditEvent, actor.role === "SYSTEM" ? null : actor.id);
                }
            })();
        }
        return result;
    }
    async notifyOrchestrator(shipmentId, action, payload, result) {
    ...
    async runActionSideEffects(tx, transition, ws, payload, actor, currentState, reason, newState) {
        ...
        switch (transition.action) {
            ...
            case "assign_container": {
                const containerNumber = payload.containerNumber;
                await update({
                    containerNumber,
            ...
            case "pickup_cargo":
                if (currentState === "CONTAINER_ASSIGNED")
                    break;
            ...
            case "load_vessel":
                await update({
                    ...
                    voyageNumber: payload.voyageNumber ?? null,
                    loadedAt: new Date(),
                });
                break;
            case "depart_vessel":
    ...
    async fetchDTO(workspaceId, _user) {
