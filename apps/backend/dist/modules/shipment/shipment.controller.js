import { z } from "zod";
import { ConfirmBookingPayload, AssignContainerPayload, PickupCargoPayload, ArriveOriginPortPayload, LoadVesselPayload, DepartVesselPayload, ArriveDestinationPayload, StartCustomsPayload, CompleteCustomsPayload, ReadyDeliveryPayload, ConfirmDeliveryPayload, ConfirmPartialDeliveryPayload, RejectShipmentPayload, CompleteShipmentPayload, ReportExceptionPayload, ResolveExceptionPayload, CancelShipmentPayload, UploadShipmentDocumentPayload, } from "@dmx/contracts/shipment.zod";
...
import { CancelShipmentBookingSchema, CompleteShipmentMilestoneSchema, CreateShipmentContainerSchema, CreateShipmentMilestoneSchema, ListDelayedShipmentsQuerySchema, ListUpcomingMilestonesQuerySchema, PatchShipmentContainerSchema, PatchShipmentMilestoneSchema, PatchShipmentStatusAliasSchema, PatchShipmentWorkspaceSchema, TransitionShipmentBookingSchema, UpsertShipmentBookingSchema, } from "@dmx/contracts/shipment-workspace.zod";
...
async function loadAccessible(req) {
    ...
    if (!(await canAccessShipment(prisma, req.user, ws.id)))
...
export const shipmentController = {
    ...
    async trackingEvents(req, res) {
        await loadAccessible(req);
        const dto = await trackingService.getTracking(req.params.id);
        res.json(dto.events);
    },
    async linkTracking(req, res) {
        await loadAccessible(req);
        const body = LinkTrackingPayload.parse(req.body);
        res.status(201).json(await trackingService.linkTracking(req.params.id, body, req.user));
    },
    ...
    async transitionBooking(req, res) {
        const ws = await loadAccessible(req);
        await ops.transitionBooking(ws.id, actorOf(req), TransitionShipmentBookingSchema.parse(req.body));
        res.json(await service.fetchDTO(ws.id, req.user));
    },
    ...
    async addContainer(req, res) {
        const ws = await loadAccessible(req);
        res.status(201).json(await ops.addContainer(ws.id, actorOf(req), CreateShipmentContainerSchema.parse(req.body)));
    },
    async patchContainer(req, res) {
        const ws = await loadAccessible(req);
        res.json(await ops.patchContainer(ws.id, req.params.containerId, actorOf(req), PatchShipmentContainerSchema.parse(req.body)));
    },
    ...
    async createMilestone(req, res) {
        res.status(201).json(await milestones.create(req.user, req.params.id, CreateShipmentMilestoneSchema.parse(req.body)));
    },
    async patchMilestone(req, res) {
        res.json(await milestones.patch(req.user, req.params.id, req.params.milestoneId, PatchShipmentMilestoneSchema.parse(req.body)));
    },
    async completeMilestone(req, res) {
        res.json(await milestones.complete(req.user, req.params.id, req.params.milestoneId, CompleteShipmentMilestoneSchema.parse(req.body ?? {})));
    },
    async delayedShipments(req, res) {
        res.json(await milestones.delayed(req.user, ListDelayedShipmentsQuerySchema.parse(req.query)));
    },
    async upcomingMilestones(req, res) {
        res.json(await milestones.upcoming(req.user, ListUpcomingMilestonesQuerySchema.parse(req.query)));
    },
    async milestoneDashboardSummary(req, res) {
    ...
    action(action) {
        return async (req, res) => {
            ...
            const payload = schema
                ...
                : (env.payload ?? {});
            const effectiveReason = env.reason ?? payload.reason;
