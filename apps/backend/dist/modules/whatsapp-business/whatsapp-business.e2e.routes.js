...
whatsappBusinessE2eRouter.post("/whatsapp-business/mock-connect", asyncHandler(async (req, res) => {
...
whatsappBusinessE2eRouter.post("/whatsapp-business/mock-disconnect", asyncHandler(async (req, res) => {
    const secret = req.headers["x-e2e-test-secret"];
    const provided = typeof secret === "string" ? secret : "";
    if (!isValidE2eSecretValue(provided)) {
        return res.status(403).json({ error: { code: "FORBIDDEN" } });
 ...
 */
export function shouldMountWhatsappE2eRoutes() {
    if (env.NODE_ENV === "production")
        return false;
    return Boolean(env.E2E_TEST_SECRET && env.E2E_TEST_SECRET.length >= 32);
}
//# sourceMappingURL=whatsapp-business.e2e.routes.js.map