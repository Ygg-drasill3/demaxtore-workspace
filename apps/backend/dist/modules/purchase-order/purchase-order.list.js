...
function accessibleOrderFilter(actor) {
...
function dayEndUtc(yyyyMmDd) {
    return new Date(`${yyyyMmDd}T23:59:59.999Z`);
}
function pricingStateFromLines(lines) {
    if (lines.length === 0)
        return "UNPRICED";
    let priced = 0;
    let unpriced = 0;
...
export function emptyBySource() {
    return { RFQ: 0, DIRECT: 0, REORDER: 0, API: 0, LEGACY: 0, COMMODITY_BID: 0 };
}
export async function listPurchaseOrders(db, actor, query) {
    ...
    const orderMeta = new Map(accessible.map((w) => [
        w.id,
        {
    ...
    if (query.dateFrom || query.dateTo) {
        where.issuedAt = {};
        if (query.dateFrom)
            where.issuedAt.gte = dayStartUtc(query.dateFrom);
        if (query.dateTo)
            where.issuedAt.lte = dayEndUtc(query.dateTo);
    }
    if (query.search) {
...
export async function mapRecentPurchaseOrders(db, actor, limit = 5) {
