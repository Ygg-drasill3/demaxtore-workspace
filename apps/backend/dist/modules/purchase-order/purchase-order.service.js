import { canonicalizePurchaseOrderSource, resolveCurrentRevisionNumber, } from "@dmx/contracts/purchase-order";
import { canonicalizePurchaseOrderStatus } from "@dmx/contracts/purchase-order.fsm";
import { AcknowledgePoPayload, ApproveAmendmentPayload, ApprovePoPayload, CancelPoPayload, ClosePoPayload, CompletePoPayload, RejectAmendmentPayload, RequestAmendmentPayload, StartExecutionPayload, SubmitPoPayload, UpdateDraftPurchaseOrderSchema, } from "@dmx/contracts/purchase-order.zod";
...
import { assertAcknowledgeAllowed, assertAmendmentAllowed, assertApproveAllowed, assertCancelAllowed, assertCloseAllowed, assertCompleteAllowed, assertDraftEditAllowed, assertPoActionRole, assertStartExecutionAllowed, assertSubmitAllowed, assertVersionMatch, canAccessPo, } from "./purchase-order.policy.js";
...
import { emptyBySource, listPurchaseOrders, } from "./purchase-order.list.js";
export class PurchaseOrderService {
     ...
     */
    async createForOrder(input, actor) {
        const poId = await this.db.$transaction((tx) => createPurchaseOrderForOrderTx(tx, {
            ...input,
    ...
    async getSummary(poId) {
        ...
        const actorById = new Map(revisionActors.map((u) => [
            u.id,
        ...
        return mapSummary(po, order?.externalRef ?? null, {
            ...
            rfqWorkspaceId: order?.spawnedFrom?.type === "RFQ"
                ? order.spawnedFrom.id
                : orderDetails?.parentWorkspaceType === "RFQ"
                    ? orderDetails.parentWorkspaceId
                    : null,
            commodityBidWorkspaceId: order?.spawnedFrom?.type === "COMMODITYBID"
                ? order.spawnedFrom.id
    ...
    async getDashboard(actor) {
        ...
        if (actor) {
            ...
            for (const item of items) {
                ...
                if ((item.openAmendments ?? 0) > 0)
                    totals.amendmentRequested += 1;
                if (item.pendingAcknowledgement)
        ...
        const ackPending = open.filter((p) => ["SUBMITTED", "ISSUED"].includes(p.status)).length;
        ...
        const statusCount = (s) => statusGroups.find((g) => g.status === s)?._count._all ?? 0;
        const valueMap = new Map();
        const addValue = (currency, amount, kind) => {
            const bucket = valueMap.get(currency) ?? { openTotal: 0, closedTotal: 0 };
            if (kind === "open")
                bucket.openTotal += amount;
            else
                bucket.closedTotal += amount;
    ...
    async applyPoAction(poId, action, actor, payload = {}, ctx) {
    ...
    async acknowledge(poId, actor, input, ctx) {
    ...
    async requestAmendment(poId, actor, input, ctx) {
        const po = await this.requirePo(poId);
        ...
        if (openCount > 0)
            throw new AppError(409, "AMENDMENT_ALREADY_OPEN");
        await this.db.$transaction(async (tx) => {
    ...
    async approveAmendment(poId, actor, input, ctx) {
        const po = await this.requirePo(poId);
        assertAmendmentAllowed(po.status);
        ...
        const lines = input.lines?.length
            ? input.lines
            : amendment.proposedLines ?? undefined;
        if (!lines?.length)
            throw new AppError(400, "AMENDMENT_LINES_REQUIRED");
        ...
        if (createdRevisionId) {
            void import("../operational-task/operational-task.automation.js").then(({ runOperationalTaskAutomation }) => runOperationalTaskAutomation(this.db, {
                type: "po.revised",
                orderId: po.orderId,
    ...
    async rejectAmendment(poId, actor, input, ctx) {
        const po = await this.requirePo(poId);
        const amendment = await this.db.purchaseOrderAmendment.findUnique({
            where: { id: input.amendmentId },
        });
    ...
    async closePo(poId, actor, input, ctx) {
        const po = await this.requirePo(poId);
    ...
    async cancelPo(poId, actor, input, ctx) {
        const po = await this.requirePo(poId);
    ...
    async submitPo(poId, actor, input, ctx) {
        const po = await this.requirePo(poId);
    ...
    async approvePo(poId, actor, input, ctx) {
        const po = await this.requirePo(poId);
    ...
    async startExecution(poId, actor, input, ctx) {
        const po = await this.requirePo(poId);
    ...
    async completePo(poId, actor, input, ctx) {
        const po = await this.requirePo(poId);
    ...
    async updateDraft(poId, actor, raw, ctx) {
        if (!(await canAccessPo(this.db, actor, poId)))
            throw new AppError(403, "FORBIDDEN");
    ...
    async repairPendingAckIfOrderConfirmed(poId) {
        ...
        if (po.acknowledgements.some((a) => a.status === "ACCEPTED"))
            return;
        if (!["SUBMITTED", "APPROVED"].includes(canonicalizePurchaseOrderStatus(po.status)))
            return;
        const orderWs = await this.db.orderWorkspace.findUnique({
            where: { workspaceId: po.orderId },
            select: { supplierConfirmedAt: true },
        });
        if (!orderWs?.supplierConfirmedAt)
            return;
        const supplier = await this.db.user.findUnique({
            where: { id: po.supplierId },
            select: { email: true },
        });
        await this.db.$transaction((tx) => autoAcknowledgePoOnSupplierConfirm(tx, po.orderId, {
            id: po.supplierId,
            email: supplier?.email ?? "supplier@unknown",
            role: "SUPPLIER",
        }, "Backfilled: order was confirmed before PO auto-ack"));
    }
    async audit(tx, orderId, actor, action, payload, ctx, fromStatus, toStatus) {
    ...
    async timeline(tx, orderId, actorUserId, eventType, payload) {
    ...
    emit(event, orderId, poId, extra) {
        socketBus.scheduleEmit(() => {
            socketBus.emitToWorkspace(orderId, event, { poId, orderId, ...extra });
        });
    }
}
function mapSummary(po, orderRef, parties = { buyerName: null, buyerEmail: null, supplierName: null, supplierEmail: null }, extras = {}, actorById = new Map()) {
    ...
    const snap = (latestRevision?.snapshotJson ?? {});
    ...
    return {
        purchaseOrder: {
            ...
            status: canonicalizePurchaseOrderStatus(po.status),
            version: Number(po.version ?? 1),
        ...
        lines: po.lines.map((l, i) => {
            ...
            const snapshotPriceAbsent = fromDirect != null && (fromDirect.unitPrice === null || fromDirect.unitPrice === undefined);
...
function parseComposedDirectDescription(description) {
    ...
    for (const part of parts.slice(1)) {
        if (part.startsWith("Spec:"))
            specification = part.slice(5).trim() || null;
        else if (part.startsWith("Pack:"))
            packaging = part.slice(5).trim() || null;
        else if (part.startsWith("Unit:"))
            unit = part.slice(5).trim() || null;
        else
            rest.push(part);
    }
    return {
