import { CreateDirectPurchaseOrderSchema, composeDirectPoLineDescription, } from "@dmx/contracts/purchase-order.zod";
...
export async function createDirectPurchaseOrderWorkspace(db, rawInput, actor) {
    ...
    const totalValue = input.lines.reduce((sum, l) => sum + l.quantity * (l.unitPrice ?? 0), 0);
    const revisionMetadata = {};
    if (input.expectedDeliveryDate)
        revisionMetadata.expectedDeliveryDate = input.expectedDeliveryDate;
    if (input.destinationCountryCode)
        revisionMetadata.destinationCountryCode = input.destinationCountryCode;
    if (input.buyerReference)
        revisionMetadata.buyerReference = input.buyerReference;
    if (input.notes)
        revisionMetadata.notes = input.notes;
    ...
    for (const line of input.lines) {
        ...
        if (line.quickCreateProduct) {
            if (!buyerUser?.organisationId)
                throw new AppError(400, "ORGANISATION_REQUIRED");
            productId = await productSvc.quickCreateOrGet(actor, buyerUser.organisationId, {
    ...
    try {
        ...
        const code = err?.code;
        if (code === "P2002")
            throw new AppError(409, "PURCHASE_ORDER_NUMBER_ALREADY_EXISTS");
        throw new AppError(500, "PURCHASE_ORDER_CREATION_FAILED");
    }
}
/** Convenience for tests — returns full PO summary after direct create. */
export async function createDirectPurchaseOrderWorkspaceWithSummary(db, rawInput, actor) {
...
//# sourceMappingURL=direct-purchase-order.orchestration.js.map