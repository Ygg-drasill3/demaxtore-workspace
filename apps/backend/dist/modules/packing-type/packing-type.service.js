import { BULK_CONTAINER_CATEGORY_DEFAULT_PACKING, BULK_CONTAINER_LOCKED_PACKING_TYPES, isBulkContainerPackingPrefix, isLockedBulkContainerPackingCode, lockedPackingCodesForCategory, validateBulkContainerPackingAssignment, } from "@dmx/contracts/bulk-container-packing-locked";
...
export class PackingTypeService {
    ...
    async create(input) {
        ...
        if (existing)
            throw new AppError(409, "PACKING_TYPE_CODE_EXISTS");
    ...
    async update(id, input) {
        ...
        if (isLockedBulkContainerPackingCode(current.code)) {
            ...
            const immutableChange = (input.name !== undefined && input.name !== current.name) ||
    ...
    async assignProduct(input) {
        ...
        if (!pt)
            throw new AppError(404, "PACKING_TYPE_NOT_FOUND");
    ...
    async updateProductLink(linkId, input) {
    ...
    async listProductLinks(catalogKind, productId) {
    ...
    async assertProductExists(catalogKind, productId) {
    ...
    async enforceBulkContainerLockedCatalog() {
        ...
        const ptMap = new Map((await this.prisma.packingType.findMany({
        ...
        for (const product of products) {
            ...
            const allowedIds = codes.map((c) => ptMap.get(c)).filter(Boolean);
            await this.prisma.productPackingType.updateMany({
                where: {
                    catalogKind: "BULK_CONTAINER",
                    productId: product.id,
...
//# sourceMappingURL=packing-type.service.js.map