...
import { evaluateOrderShipmentDesync, alertSeverityFromDesync, TERMINAL_ORDER_STATES, TERMINAL_SHIPMENT_STATES, } from "@dmx/contracts/order-shipment-orchestration";
...
export class AlertEngine {
    ...
    async upsertOpenAlert(input) {
        if (await isTestWorkspace(this.db, input.workspaceId))
            return false;
    ...
    async conditionStillActive(alertKey, workspaceId, state, type) {
        const now = new Date();
        switch (alertKey) {
            case AlertKey.RFQ_SUBMITTED_UNASSIGNED:
                if (state !== "RFQ_SUBMITTED")
                    return false;
            ...
            case AlertKey.ORDER_INSPECTION_SLA_PAST: {
                ...
                if (!ow?.inspectionRequestedAt || ow.inspectionCompletedAt)
                    return false;
            ...
            case AlertKey.ORDER_SHIPMENT_STATE_MISMATCH: {
                ...
                if (!shipment || TERMINAL_SHIPMENT_STATES.includes(shipment.state))
                    return false;
            ...
            case AlertKey.TRADE_DOC_MISSING_72H: {
                ...
                for (const req of requirements) {
                    ...
                    if (!doc || doc.status === "MISSING" || doc.status === "REQUESTED" || doc.status === "UPLOADED" || doc.status === "UNDER_REVIEW")
                        return true;
                }
                return false;
            }
            case AlertKey.TRADE_DOC_DELIVERED_INCOMPLETE: {
                if (type !== "SHIPMENT" || state !== "DELIVERED")
                    return false;
                const requirements = await this.db.documentRequirement.findMany({
                    where: { workspaceId, required: true },
                });
            ...
            case AlertKey.FREIGHT_ESTIMATE_REFRESH_REQUIRED: {
                ...
                if (!poReadyStates.includes(state))
                    return false;
                const active = await this.db.freightEstimate.findFirst({
                    where: { tradeId: workspaceId, status: "ACTIVE", expiresAt: { gt: now } },
                });
    ...
    async staleUpdated(workspaceId, maxAgeMs) {
    ...
    async shipmentEtaExceeded(workspaceId, state) {
        if (state !== "IN_TRANSIT")
            return false;
        const sw = await this.db.shipmentWorkspace.findUnique({ where: { workspaceId } });
        if (!sw)
    ...
    async scanShipment() {
        ...
        for (const ws of inTransit) {
            ...
            if (!order?.currentEta || order.currentEta > now)
                continue;
            if (await this.upsertOpenAlert({
                severity: "CRITICAL",
                category: "SHIPMENT",
    ...
    async enqueueOrchestratorFromAlert(orderId, metadata) {
        ...
        await orch.planFromDesyncAlert({
            ...
            laggingEntity: metadata.laggingEntity,
        });
    }
    async enqueueExceptionCase(alert) {
