...
export function shouldExcludeTestData() {
    if (env.CONTROL_TOWER_EXCLUDE_TEST_DATA === true)
        return true;
    if (env.CONTROL_TOWER_EXCLUDE_TEST_DATA === false)
        return false;
    if (isProd)
        return false;
    return env.NODE_ENV === "development";
}
export function isTestExternalRef(externalRef) {
    if (!externalRef)
        return false;
    return TEST_EXTERNAL_REF_PATTERNS.some((re) => re.test(externalRef));
}
export function isTestRfqMeta(title, productCategory) {
    if (productCategory === TEST_PRODUCT_CATEGORY)
        return true;
    const t = title?.trim();
    if (!t)
        return false;
...
export function workspaceExcludesTestData() {
...
export function alertExcludesTestData(testIds) {
    if (!shouldExcludeTestData() || testIds.length === 0)
        return {};
    return {
        OR: [
            { workspaceId: null },
...
let cachedTestIds = null;
const CACHE_MS = 60_000;
export async function getTestWorkspaceIds(db) {
    if (!shouldExcludeTestData())
        return [];
    const now = Date.now();
    if (cachedTestIds && now - cachedTestIds.at < CACHE_MS)
        return cachedTestIds.ids;
    const ids = new Set();
...
export async function isTestWorkspace(db, workspaceId) {
    if (!shouldExcludeTestData())
        return false;
    const testIds = await getTestWorkspaceIds(db);
    if (testIds.includes(workspaceId))
        return true;
    ...
    if (!ws)
        return false;
    if (isTestExternalRef(ws.externalRef))
        return true;
    if (ws.rfqDetails && isTestRfqMeta(ws.rfqDetails.title, ws.rfqDetails.productCategory))
        return true;
    if (ws.spawnedFromId && testIds.includes(ws.spawnedFromId))
        return true;
    return false;
}
export async function resolveOpenAlertsForTestWorkspaces(db) {
    if (!shouldExcludeTestData())
        return 0;
    const testIds = await getTestWorkspaceIds(db);
    if (testIds.length === 0)
