...
const ROOT_TYPES = ["RFQ", "COMMODITYBID", "MIXED_CONTAINER", "BULK_CONTAINER"];
export async function findAccessibleTradeRoots(db, actor) {
    if (hasPortfolioVisibility(actor.role)) {
        ...
        return [...rooted, ...directOrders].sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime());
    }
...
export async function getAccessibleTradeIds(db, actor) {
    const roots = await findAccessibleTradeRoots(db, actor);
    const ids = new Set();
    for (const root of roots) {
        ids.add(root.id);
        const graph = await collectTradeGraph(db, root);
        for (const id of graph.allWorkspaceIds)
            ids.add(id);
    }
    return [...ids];
}
export function canAccessImportControlTower(actor) {
    return hasPortfolioVisibility(actor.role) || actor.role === "BUYER" || actor.role === "SUPPLIER";
}
//# sourceMappingURL=control-tower.policy.js.map