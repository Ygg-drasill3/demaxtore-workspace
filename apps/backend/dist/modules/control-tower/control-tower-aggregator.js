...
function pipelineStage(ctx) {
    if (ctx.shipmentStates.some((s) => COMPLETED_STATES.has(s)))
        return "delivery";
    if (ctx.shipmentStates.length > 0 || ctx.shipmentStates.some((s) => TRANSIT_STATES.has(s)))
        return "shipment";
    if (ctx.hasBooking)
        return "booking";
    if (ctx.orderStates.some((s) => PRODUCTION_STATES.has(s)))
        return "production";
    if (ctx.hasPo)
        return "po";
    if (SUPPLIER_SEL.has(ctx.root.state))
        return "supplier";
    if (RFQ_EARLY.has(ctx.root.state) || ctx.root.type === "RFQ")
        return "rfq";
    return "rfq";
}
...
export class ControlTowerAggregator {
    ...
    async buildDashboard(actor, query = {}) {
        const roots = await findAccessibleTradeRoots(this.db, actor);
        const contexts = [];
        for (const root of roots.slice(0, 100)) {
            if (shouldExcludeTestData() && await isTestWorkspace(this.db, root.id))
                continue;
            ...
            if (query.q && !matchesQuery(ctx, query.q))
                continue;
            if (query.supplier && !(ctx.supplierName ?? "").toLowerCase().includes(query.supplier.toLowerCase()))
                continue;
            if (query.status && !ctx.root.state.toLowerCase().includes(query.status.toLowerCase()))
                continue;
            contexts.push(ctx);
        }
        ...
        const [timelineKpis, bookingKpis, activityRows, openAlerts, openExceptions, trackingSnaps, forecasts, carrierOptions,] = await Promise.all([
        ...
        for (const ctx of contexts) {
            ...
            if (ctx.orderStates.some((s) => PRODUCTION_STATES.has(s)))
                inProduction++;
            if (ctx.shipmentStates.some((s) => TRANSIT_STATES.has(s)))
                atSea++;
            if (ctx.shipmentStates.some((s) => s === "DELIVERED" || s === "CLOSED")) {
                if (ctx.root.updatedAt >= monthStart)
                    deliveredThisMonth++;
            }
        ...
        for (const alert of openAlerts.slice(0, 15)) {
            if (attentionRequired.length >= 20)
                break;
        ...
        const activityFeed = activityRows.map((e) => ({
            id: e.id,
            title: e.title,
        ...
        for (const opt of carrierOptions) {
            if (opt.cutoffDate <= now || opt.cutoffDate > weekEnd)
                continue;
            upcomingMilestones.push({
                id: `cutoff-${opt.id}`,
        ...
        const latestSnapByShipment = new Map();
        for (const s of trackingSnaps) {
            if (!latestSnapByShipment.has(s.shipmentId))
                latestSnapByShipment.set(s.shipmentId, s);
        }
        ...
        for (const snap of latestSnapByShipment.values()) {
            ...
            if (snap.delayFlag === "MINOR" || snap.delayFlag === "MAJOR")
                delayedShipments++;
            else if (snap.trackingStatus === "IN_TRANSIT")
                onTimeShipments++;
            if (snap.eta && snap.eta > now) {
                etaSum += (snap.eta.getTime() - now.getTime()) / 86_400_000;
                etaCount++;
            }
            if (snap.eta && snap.eta >= now && snap.eta <= weekEnd)
                arrivalsThisWeek++;
        }
        const carrierDistribution = [...carrierMap.entries()]
            .map(([carrier, count]) => ({ carrier, count }))
            .sort((a, b) => b.count - a.count);
        const operationalRisks = openAlerts
...
function normalizeCarrier(name) {
    ...
    if (u.includes("MSC"))
        return "MSC";
    if (u.includes("MAERSK"))
        return "Maersk";
    if (u.includes("CMA") || u.includes("CGM"))
        return "CMA CGM";
    if (u.includes("HAPAG") || u.includes("LLOYD"))
        return "Hapag-Lloyd";
    return "Others";
}
function mapExceptionKind(type) {
    if (type.includes("Inspection"))
        return "Inspection Pending";
    if (type.includes("Document") || type.includes("Missing"))
        return "Document Missing";
    if (type.includes("Shipment") || type.includes("ETA"))
        return "ETA Delay";
    if (type.includes("Production"))
        return "Production Delay";
    if (type.includes("PO") || type.includes("Payment"))
        return "Exception Open";
    return "Exception Open";
}
function mapAlertKind(key, category) {
    if (key.includes("INSPECTION"))
        return "Inspection Pending";
    if (key.includes("DOC") || category === "DOCUMENT")
        return "Document Missing";
    if (key.includes("TRACKING") || key.includes("ETA") || key.includes("SHIPMENT"))
        return "ETA Delay";
    if (key.includes("PRODUCTION"))
        return "Production Delay";
    if (key.includes("BOOKING") || key.includes("FREIGHT") || key.includes("CUTOFF"))
        return "Booking Pending";
    if (key.includes("CUTOFF"))
        return "Cut-Off Risk";
    if (key.includes("PO") || key.includes("ACK"))
        return "Exception Open";
    return "Exception Open";
}
const PRIORITY_RANK = {
...
function dedupeAttentionRequired(items) {
    const byKey = new Map();
    ...
    return [...byKey.values()].sort((a, b) => (PRIORITY_RANK[b.priority] ?? 0) - (PRIORITY_RANK[a.priority] ?? 0));
}
function mapRiskKind(key, category) {
    if (key.includes("ETA") || key.includes("TRACKING"))
        return "ETA Delay Risk";
    if (key.includes("DOC"))
        return "Missing Documents";
    if (key.includes("INSPECTION"))
        return "Inspection Overdue";
    if (key.includes("CUTOFF") || key.includes("BOOKING"))
        return "Booking Cut-Off Risk";
    if (key.includes("PRODUCTION"))
        return "Production Delay";
...
//# sourceMappingURL=control-tower-aggregator.js.map