...
import { resolveTradeRoot, tradeRefFromRoot } from "../trade/trade.resolver.js";
...
function mapAlertSeverity(severity, alertKey) {
    if (severity === "CRITICAL")
        return "Critical";
    if (alertKey.includes("72h") || alertKey.includes("delay"))
        return "High";
    if (severity === "WARNING")
        return "Medium";
    return "Low";
}
function mapExceptionType(alertKey, title) {
    return ALERT_TYPE_MAP[alertKey] ?? (title.toLowerCase().includes("document") ? "Missing Document" : "Manual Exception");
}
function defaultStatus(type, ownerRole, alertKey) {
    if (alertKey === AlertKey.PO_REJECTED)
    ...
    if (type === "PO Pending" && alertKey !== AlertKey.PO_REJECTED)
...
function defaultOwnerRole(type) {
    if (["Missing Document", "Document Revision Requested"].includes(type))
        return "SUPPLIER";
    if (["Document Rejected", "PO Pending", "Payment Pending"].includes(type))
        return "BUYER";
    return "OPERATIONS";
}
function defaultRequiredAction(type) {
    const actions = {
...
export class ExceptionHubService {
    ...
    async list(actor, query) {
        ...
        if (query.waitingForMe) {
            filtered = filtered.filter((r) => r.ownerId === actor.id ||
    ...
    async getDetail(actor, id) {
        ...
        if (!row)
            throw new AppError(404, "EXCEPTION_NOT_FOUND");
    ...
    async getTradeExceptions(actor, tradeRootId) {
    ...
    async getShipmentExceptions(actor, shipmentId) {
        if (!(await canAccessTrade(this.db, actor, shipmentId)))
            throw new AppError(403, "FORBIDDEN");
        await this.syncAlertsForActor(actor);
        return (await this.loadRows(actor)).filter((r) => r._workspaceId === shipmentId);
    }
    async assign(actor, id, ownerId, ownerRole) {
    ...
    async resolve(actor, id, resolutionNote, resolutionEta) {
        const ex = await this.requireAccessible(actor, id);
    ...
    async close(actor, id, note) {
        const ex = await this.requireAccessible(actor, id);
        if (actor.role !== "ADMIN" && actor.role !== "BUYER")
            throw new AppError(403, "FORBIDDEN");
        const now = new Date();
        await this.db.$transaction(async (tx) => {
            await tx.tradeException.update({
    ...
    async requireAccessible(actor, id) {
        const ex = await this.db.tradeException.findUnique({ where: { id } });
        if (!ex)
            throw new AppError(404, "EXCEPTION_NOT_FOUND");
        if (!(await canAccessTrade(this.db, actor, ex.tradeRootId)))
            throw new AppError(403, "FORBIDDEN");
        return ex;
    }
    async syncAlertsForActor(actor) {
    ...
    async accessibleWorkspaceFilter(actor) {
    ...
    async loadRows(actor) {
        ...
        for (const o of orderWs) {
            userIds.add(o.buyerUserId);
            userIds.add(o.supplierUserId);
        }
        for (const s of shipmentWs) {
            userIds.add(s.buyerUserId);
            userIds.add(s.supplierUserId);
        }
        ...
        return exceptions.map((ex) => {
            ...
            return {
                ...
                tradeId: root ? tradeRefFromRoot(root) : null,
    ...
    async accessibleTradeRootIds(actor) {
        const parts = await this.db.workspaceParticipant.findMany({
            where: { userId: actor.id, leftAt: null },
            select: { workspaceId: true },
        });
...
function applyFilters(rows, query, actorId) {
    let out = rows;
    if (query.severity)
        out = out.filter((r) => r.severity === query.severity);
    if (query.status)
        out = out.filter((r) => r.status === query.status);
    if (query.exceptionType)
        out = out.filter((r) => r.exceptionType === query.exceptionType);
    if (query.ownerId)
        out = out.filter((r) => r.ownerId === query.ownerId);
    if (query.supplierId)
        out = out.filter((r) => r._supplierId === query.supplierId);
    if (query.tradeType)
        out = out.filter((r) => r._tradeType === query.tradeType);
    if (query.dateFrom) {
    ...
    if (query.search?.trim()) {
        ...
        out = out.filter((r) => r.exceptionRef.toLowerCase().includes(q) ||
...
function computeKpis(rows, actorId) {
    ...
    for (const r of open)
        typeCounts.set(r.exceptionType, (typeCounts.get(r.exceptionType) ?? 0) + 1);
    return {
        openExceptions: open.length,
        criticalExceptions: open.filter((r) => r.severity === "Critical").length,
        myPendingActions: open.filter((r) => r.ownerId === actorId ||
            r.status === "Waiting For Buyer" ||
            r.status === "Waiting For Supplier").length,
...
function computeAnalytics(rows) {
    ...
    for (const r of open) {
        if (r.supplierName)
            bySupplier.set(r.supplierName, (bySupplier.get(r.supplierName) ?? 0) + 1);
        if (r._carrier)
            byCarrier.set(r._carrier, (byCarrier.get(r._carrier) ?? 0) + 1);
        if (r._tradeType)
            byTradeType.set(r._tradeType, (byTradeType.get(r._tradeType) ?? 0) + 1);
    }
    return {
        openExceptions: open.length,
        averageResolutionHours: avgHours != null ? Math.round(avgHours * 10) / 10 : null,
...
export function shipmentExceptionSummary(exceptions) {
    const open = exceptions.filter((e) => !["Resolved", "Closed"].includes(e.status));
    if (open.length === 0)
        return { count: 0, highestSeverity: null, status: "None", primaryDetailUrl: null };
...
//# sourceMappingURL=exception-hub.service.js.map