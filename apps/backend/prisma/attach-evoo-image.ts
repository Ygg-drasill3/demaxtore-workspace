...
const file = path.resolve(
  ...
  "assets/mc-products/mc-product-extra-virgin-olive-oil.jpg",
);

async function main() {
  const buf = await fs.readFile(file);
  const { storageKey } = await writeStoredFile(buf, "mc-product-extra-virgin-olive-oil.jpg");
  const updated = await prisma.catalogProduct.update({
    where: { productRef: "MC-OLO-01" },
